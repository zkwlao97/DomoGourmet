(function () {
  ...
  const STORAGE = {
    query: 'hankkiRouteJapan.query.v6',
    plan: 'hankkiRouteJapan.plan.v6',
    planStory: 'hankkiRouteJapan.planStory.v1',
    admin: 'hankkiRouteJapan.admin.v6',
    adminSession: 'hankkiRouteJapan.adminSession.v6',
    forceDemo: 'hankkiRouteJapan.forceDemo.v6',
    basePoint: 'hankkiRouteJapan.basePoint.v6',
    uiView: 'hankkiRouteJapan.uiView.v2',
  };
  ...
  const REGION_TRAVEL_SPOTS = {
    아키하바라: [
      { nameKo: '라디오회관', nameOriginal: 'ラジオ会館', lat: 35.699693, lng: 139.77158 },
      { nameKo: '센소지', nameOriginal: '浅草寺', lat: 35.714765, lng: 139.796655 },
      { nameKo: '시부야 스크램블 교차로', nameOriginal: '渋谷スクランブル交差点', lat: 35.659494, lng: 139.700553 },
    ],
    도쿄: [
      { nameKo: '라디오회관', nameOriginal: 'ラジオ会館', lat: 35.699693, lng: 139.77158 },
      { nameKo: '센소지', nameOriginal: '浅草寺', lat: 35.714765, lng: 139.796655 },
      { nameKo: '도쿄 스카이트리', nameOriginal: '東京スカイツリー', lat: 35.710063, lng: 139.8107 },
      { nameKo: '시부야 스크램블 교차로', nameOriginal: '渋谷スクランブル交差点', lat: 35.659494, lng: 139.700553 },
    ],
    오사카: [
      { nameKo: '도톤보리', nameOriginal: '道頓堀', lat: 34.668731, lng: 135.501295 },
      { nameKo: '오사카성', nameOriginal: '大阪城', lat: 34.687315, lng: 135.526202 },
    ],
    난바: [
      { nameKo: '도톤보리', nameOriginal: '道頓堀', lat: 34.668731, lng: 135.501295 },
      { nameKo: '오사카성', nameOriginal: '大阪城', lat: 34.687315, lng: 135.526202 },
    ...
    교토: [
      { nameKo: '교토역', nameOriginal: '京都駅', lat: 34.985849, lng: 135.758766 },
      { nameKo: '기요미즈데라', nameOriginal: '清水寺', lat: 34.994856, lng: 135.785046 },
      { nameKo: '후시미 이나리 신사', nameOriginal: '伏見稲荷大社', lat: 34.967146, lng: 135.772671 },
    ],
    후쿠오카: [
      { nameKo: '하카타역', nameOriginal: '博多駅', lat: 33.590355, lng: 130.420616 },
      { nameKo: '하카타역 주변', nameOriginal: '博多駅周辺', lat: 33.590355, lng: 130.420616 },
    ],
    하카타: [
      { nameKo: '하카타역', nameOriginal: '博多駅', lat: 33.590355, lng: 130.420616 },
    ],
  ...
  function cache() {
    ids([
      'mode-badge', 'api-status', 'region-input', 'base-input', 'maps-link-input', 'food-input', 'start-date-input', 'days-input',
      ...
      'search-view', 'plan-view', 'plan-summary', 'plan-story',
  ...
  function search(options) {
    ...
    const query = {
      ...
      mapsLink: String(els.mapsLinkInput.value || '').trim(),
    ...
    return resolveBasePoint(query.region, query.base, query.mapsLink).then(function (basePoint) {
      ...
      const removedClosedCount = rawPool.filter(isLikelyClosedRestaurant).length;
      ...
      let filterMessage = removedClosedCount ? '폐업 또는 장기 휴업으로 보이는 후보 ' + removedClosedCount + '곳은 제외했습니다.' : '';
  ...
  function renderResults() {
    ...
    els.resultsList.innerHTML = pageItems.map(function (r) {
      const distanceLabel = r.distanceKm != null ? '직선거리 약 ' + r.distanceKm.toFixed(2) + 'km' : '';
        ...
        '<article class="result-card" data-result-id="' + escapeHtml(r.id) + '">' +
          ...
          '<div class="result-main">' +
            ...
            '<div class="metric-strip">' +
              (distanceLabel ? '<div class="metric-card"><span>기준 장소와 거리</span><strong>' + escapeHtml(distanceLabel) + '</strong></div>' : '') +
  ...
  function renderPlanSummary() {
    ...
    els.planSummary.innerHTML = '<div class="plan-summary-box"><strong>' + escapeHtml(baseName) + '</strong><span>총 ' + filled + ' / ' + total + '칸 채움</span></div>';
  }
  ...
  function renderBasePoint() {
    ...
    const sourceLabel = state.basePoint.source === 'google-maps-link' ? 'Google Maps 링크 기준 좌표를 사용했습니다.' : '이 위치를 중심으로 주변 식당을 계산합니다.';
    els.basePointBox.innerHTML = '<div class="base-box"><strong>' + escapeHtml(title) + '</strong><br /><span class="muted">' + escapeHtml(subtitle) + '</span><div class="base-subnote">' + escapeHtml(sourceLabel) + '</div></div>';
    renderPlanSummary();
  }
  ...
  function scoreRestaurantForPlan(restaurant, slot, date, previousGenre, usedRestaurantIds) {
    ...
    if (restaurant.distanceKm != null) {
      if (restaurant.distanceKm <= 0.5) score += 9;
      else if (restaurant.distanceKm <= 1.0) score += 6;
      else if (restaurant.distanceKm <= 1.8) score += 3;
      else score -= 2;
    }
  ...
  function openPlanOptionModal() {
    ...
    if (els.planAiState) {
      els.planAiState.textContent = state.geminiAvailable
        ? 'Gemini API 키가 연결되어 있으면 여행 이유 설명을 더 자연스럽게 만들어 드립니다.'
        : 'Gemini API 키가 없어서 지금은 규칙 기반 설명으로 동작합니다.';
    }
  ...
  function generateNearbyPlacePlan(useAi) {
    ...
    const defaults = [];
    if (state.currentQuery.mapsLink) defaults.push(state.currentQuery.mapsLink);
    if (state.currentQuery.base) defaults.push(state.currentQuery.base);
    ...
    return Promise.all(inputs.map(resolvePointLine)).then(function (resolved) {
      ...
      if (!points.length) {
        throw new Error('방문 후보 장소나 Google Maps 링크를 하나 이상 넣어 주세요.');
      }
      return Promise.all(points.map(function (point) {
        return fetchCandidatesAroundPoint(point, state.currentQuery.food || '');
      })).then(function (groups) {
        return fillPlanFromPointGroups(groups, {
          type: 'nearby-multi',
          useAi: useAi,
          overviewSeed: '입력한 방문 후보를 기준으로 각 지점에서 가까운 식당을 우선 배치했습니다.',
        });
      });
    });
  }
  ...
  function generateRegionalAutoPlan(useAi) {
    let spots = REGION_TRAVEL_SPOTS[state.currentQuery.region] || REGION_TRAVEL_SPOTS[Object.keys(REGION_TRAVEL_SPOTS).find(function (key) {
      return normalize(key) === normalize(state.currentQuery.region);
    }) || ''] || [];
    if (!spots.length && state.basePoint) spots = [makePointDisplay(state.basePoint)];
    if (!spots.length) {
      throw new Error('이 지역은 아직 자동 여행 코스 템플릿이 부족합니다. 기준 장소를 먼저 입력해 주세요.');
    }
    const days = clamp(parseInt(els.daysInput.value || '3', 10), 1, 10);
    const picked = [];
    for (var i = 0; i < days; i += 1) picked.push(makePointDisplay(spots[i % spots.length]));
    return Promise.all(picked.map(function (point) {
      return fetchCandidatesAroundPoint(point, '');
    })).then(function (groups) {
      return fillPlanFromPointGroups(groups, {
        type: 'regional-auto',
        useAi: useAi,
        overviewSeed: (state.currentQuery.region || '입력한 지역') + '에서 이동 동선이 무난한 대표 방문지와 주변 식당을 함께 섞어 일정으로 만들었습니다.',
      });
    });
  }
  ...
  function fillPlanFromPointGroups(pointGroups, options) {
    ...
    for (var dayIndex = 0; dayIndex < days; dayIndex += 1) {
      const group = pointGroups[dayIndex % pointGroups.length];
      const point = group.point;
      const sourceList = group.results && group.results.length ? group.results : state.results;
      const date = addDays(tripStart, dayIndex);
      ['breakfast', 'lunch', 'dinner'].forEach(function (slot) {
        const chosen = chooseRestaurantForSlot(sourceList, slot, date, previousGenre, usedRestaurantIds);
        if (!chosen) return;
        usedRestaurantIds.add(chosen.id);
        previousGenre = chosen.genreKo || chosen.genreOriginal || previousGenre;
        state.plan[dayIndex][slot] = {
          restaurantId: chosen.id,
          restaurantName: chosen.displayName,
          restaurantUrl: chosen.url || chosen.reservationUrl || '',
          note: formatPointName(point) + ' 근처 · ' + buildPlanReason(chosen, slot),
          date: toDateInput(date),
        };
      });
      storyDays.push({
        ...
        why: formatPointName(point) + ' 주변에서 도보 부담이 작은 식당을 우선 골랐습니다.',
        visitPlace: formatPointName(point),
      });
    }
