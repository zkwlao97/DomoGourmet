  ...
  try {
    ...
    if (url.pathname === '/api/health') {
      return sendJson(res, 200, {
        ...
        geminiApiKeyPresent: Boolean(GEMINI_API_KEY),
...
function sanitizeUserAccount(user) {
  ...
  return {
    ...
    nickname: String(user.nickname || email.split('@')[0] || '여행자').trim() || '여행자',
...
async function handleAuthSignup(req, res) {
  ...
  if (!email || !password) {
    return sendJson(res, 400, { ok: false, error: 'missing_fields', message: '이메일과 비밀번호를 입력해 주세요.' });
  }
  if (!nickname) nickname = email.split('@')[0] || '여행자';
  if (email === 'admin') {
    return sendJson(res, 400, { ok: false, error: 'reserved_identifier', message: 'admin 계정은 회원가입할 수 없습니다.' });
  }
  const store = loadStore();
  if (store.users[email]) {
    return sendJson(res, 409, { ok: false, error: 'already_exists', message: '이미 존재하는 계정입니다.' });
  }
  const user = sanitizeUserAccount({ email, password, nickname, role: 'user' });
  store.users[email] = user;
  saveStore(store);
  return sendJson(res, 200, { ok: true, message: '회원가입이 완료되었습니다.', user: publicUser(user), ...summarizeStore(store) });
}
...
async function handleAuthLogin(req, res) {
  ...
  if (identifier === 'admin' && password === 'admin') {
    return sendJson(res, 200, { ok: true, message: '관리자 계정으로 로그인했습니다.', user: { id: 'admin', email: 'admin', nickname: '관리자', role: 'admin' } });
  }
  const email = identifier.toLowerCase();
  const store = loadStore();
  const user = store.users[email];
  if (!user || String(user.password || '') !== password) {
    return sendJson(res, 401, { ok: false, error: 'invalid_credentials', message: '이메일 또는 비밀번호가 다릅니다.' });
  }
  return sendJson(res, 200, { ok: true, message: '로그인되었습니다.', user: publicUser(user) });
}
