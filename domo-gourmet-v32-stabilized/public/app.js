...
function renderAdminDashboard() {
  ...
  function getDefaultThemeOptions() {
    return ['애니', '관광', '패션', '요양', '오타쿠', '쇼핑', '카페', '혼행', '가족'];
  }
...
function submitAdminLogin() {
  ...
  if (!email || !password || (mode === 'signup' && !nickname)) {
    if (els.adminLoginMessage) els.adminLoginMessage.textContent = mode === 'signup' ? '이메일, 비밀번호, 닉네임을 입력해 주세요.' : '이메일(또는 아이디)과 비밀번호를 입력해 주세요.';
    return;
  }
...
function getThemeCoverAssetV28(themes, region, title) {
  var joined = normalize([Array.isArray(themes) ? themes.join(' ') : '', region || '', title || ''].join(' '));
  if (joined.indexOf('오키나와') >= 0 || joined.indexOf('휴양') >= 0) return '/assets/thumb-okinawa.jpg';
  if (joined.indexOf('교토') >= 0) return '/assets/thumb-kyoto.jpg';
  if (joined.indexOf('후쿠오카') >= 0 || joined.indexOf('먹방') >= 0) return '/assets/thumb-fukuoka.jpg';
  if (joined.indexOf('오사카') >= 0) return '/assets/thumb-osaka.jpg';
  if (joined.indexOf('애니') >= 0 || joined.indexOf('아키하바라') >= 0 || joined.indexOf('도쿄') >= 0 || joined.indexOf('패션') >= 0) return '/assets/thumb-tokyo.jpg';
  return '/assets/hero-japan.jpg';
}

function getPlaceCoverAssetV28(place, themes, region) {
  if (place && place.photo) return place.photo;
  return getThemeCoverAssetV28((place && place.themes) || themes, (place && (place.regionKo || place.regionOriginal)) || region, (place && (place.nameKo || place.nameOriginal)) || '');
}

function buildPlaceChipListV28(points, limit) {
  return (points || []).slice(0, limit || 4).map(function (point, index) {
    var label = point.displayNameKo || point.nameKo || point.nameOriginal || point.query || ('장소 ' + (index + 1));
    return '<span class="route-pill"><strong>' + (index + 1) + '</strong><span>' + escapeHtml(label) + '</span></span>';
  }).join('');
}

var __syncHeaderStatusLightsV27 = syncHeaderStatusLightsV27;
syncHeaderStatusLightsV27 = function () {
  if (__syncHeaderStatusLightsV27) __syncHeaderStatusLightsV27();
  var maps = document.getElementById('status-maps-header');
  var hp = document.getElementById('status-hotpepper-header');
  var gm = document.getElementById('status-gemini-header');
  if (maps) maps.textContent = '연결됨';
  if (hp) hp.textContent = state.liveEnabled ? '연결됨' : '데모';
  if (gm) gm.textContent = state.geminiAvailable ? '연결됨' : '미연결';
};

function updateNavUserBadgeV28() {
  var badgeEl = document.getElementById('nav-user-badge');
  if (!badgeEl) return;
  if (!state.currentUser) {
  ...
  var label = state.currentUser.nickname || state.currentUser.email || '여행자';
  if (state.currentUser.role === 'admin') label += ' · 관리자';
  badgeEl.textContent = label;
  badgeEl.classList.remove('hidden');
}

var __persistCurrentUserV27 = persistCurrentUserV27;
persistCurrentUserV27 = function (user) {
  __persistCurrentUserV27(user);
  updateNavUserBadgeV28();
};

var __renderAdminStateV27 = renderAdminState;
renderAdminState = function () {
  __renderAdminStateV27();
  updateNavUserBadgeV28();
};

var __setAuthModeV27 = setAuthModeV27;
setAuthModeV27 = function (mode) {
  __setAuthModeV27(mode);
  var helper = document.getElementById('admin-login-message');
  if (helper && mode === 'signup') helper.textContent = '회원가입 시 닉네임을 비워두면 이메일 앞부분이 자동으로 사용됩니다.';
};

var __renderPostCardBaseV28 = renderPostCardBase;
renderPostCardBase = function (post, communityMode) {
  ...
  var isMine = communityMode && state.currentUser && (state.currentUser.role === 'admin' || (state.currentUser.email && state.currentUser.email === post.authorEmail));
  var authorText = communityMode ? (post.nickname || '익명') : '관리자';
  var cover = getPlaceCoverAssetV28(points[0], themes, region || title);
  var routeLink = points.length > 1 ? '<a class="btn btn-ghost btn-mini" target="_blank" rel="noopener noreferrer" href="' + escapeHtml(buildMultiStopDirectionsUrl(points)) + '">전체 루트 Google Maps 열기</a>' : (points[0] && points[0].mapLink ? '<a class="btn btn-ghost btn-mini" target="_blank" rel="noopener noreferrer" href="' + escapeHtml(points[0].mapLink) + '">Google Maps 열기</a>' : '');
  var placePreview = buildPlaceChipListV28(points, 4);
  return '<article class="content-post-card premium-post-card">' +
    '<div class="content-cover post-cover">' + (cover ? '<img src="' + escapeHtml(cover) + '" alt="' + escapeHtml(title) + '" />' : '<div class="placeholder-image">추천 루트</div>') + '</div>' +
    '<div class="content-post-body">' +
      '<div class="community-card-top">' +
        '<div>' +
          '<div class="badge-row">' + themes.slice(0, 4).map(function (theme) { return badge(theme); }).join('') + '</div>' +
      ...
      '</div>' +
      (description ? '<p class="content-summary">' + escapeHtml(description) + '</p>' : '') +
      (placePreview ? '<div class="route-pill-row">' + placePreview + '</div>' : '') +
      '<div class="action-row compact-top">' + routeLink + (communityMode ? '<button class="btn btn-secondary btn-mini" type="button" data-add-community-route="' + escapeHtml(post.id) + '">일정에 추가</button>' : '') + '</div>' +
    '</div>' +
  '</article>';
};

var __renderThemePlaceRegistryV28 = renderThemePlaceRegistry;
renderThemePlaceRegistry = function () {
  if (!els.themePlaceRegistry) return;
  var rows = (state.themePlaces || []).slice();
  els.themePlaceRegistry.innerHTML = rows.length ? rows.map(function (item) {
    var cover = getPlaceCoverAssetV28(item, item.themes || [], item.regionKo || item.regionOriginal || '');
    return '<article class="content-post-card compact-card premium-registry-card">' +
      '<div class="content-cover compact-cover">' + (cover ? '<img src="' + escapeHtml(cover) + '" alt="' + escapeHtml(item.nameKo || item.nameOriginal || item.id) + '" />' : '<div class="placeholder-image">장소</div>') + '</div>' +
      '<div class="content-post-body"><div class="result-top"><div><h3 class="result-title">' + escapeHtml(item.nameKo || item.nameOriginal || item.id) + '</h3><div class="subline">' + escapeHtml([item.regionKo || item.regionOriginal, item.areaKo || item.areaOriginal].filter(Boolean).join(' · ')) + '</div></div><div class="badge-row">' + (Array.isArray(item.themes) ? item.themes.map(function (theme) { return badge(theme); }).join('') : '') + '</div></div>' + (item.description ? '<p class="content-summary">' + escapeHtml(item.description) + '</p>' : '') + '</div></article>';
  }).join('') : '<div class="empty-state">아직 등록된 테마 장소가 없습니다.</div>';
};

var __renderThemeTemplateRegistryV28 = renderThemeTemplateRegistry;
renderThemeTemplateRegistry = function () {
  if (!els.themeTemplateList) return;
  var placeMap = {};
  (state.themePlaces || []).forEach(function (item) { placeMap[item.id] = item; });
  var rows = (state.themeTemplates || []).slice();
  els.themeTemplateList.innerHTML = rows.length ? rows.map(function (item) {
    ...
    var coverSource = (Array.isArray(item.placeIds) ? item.placeIds : []).map(function(id){ return placeMap[id]; }).filter(Boolean)[0];
    var cover = getPlaceCoverAssetV28(coverSource, item.themes || [], item.regionKo || item.regionOriginal || '');
    return '<article class="content-post-card compact-card premium-registry-card">' +
      '<div class="content-cover compact-cover">' + (cover ? '<img src="' + escapeHtml(cover) + '" alt="' + escapeHtml(item.title || '추천 일정') + '" />' : '<div class="placeholder-image">추천 일정</div>') + '</div>' +
      '<div class="content-post-body"><div class="result-top"><div><h3 class="result-title">' + escapeHtml(item.title || '추천 일정') + '</h3><div class="subline">' + escapeHtml(item.regionKo || item.regionOriginal || '') + '</div></div><div class="badge-row">' + (Array.isArray(item.themes) ? item.themes.map(function (theme) { return badge(theme); }).join('') : '') + '</div></div>' + (item.description ? '<p class="content-summary">' + escapeHtml(item.description) + '</p>' : '') + (placeNames.length ? '<div class="route-pill-row">' + placeNames.map(function (name, index) { return '<span class="route-pill"><strong>' + (index + 1) + '</strong><span>' + escapeHtml(name) + '</span></span>'; }).join('') + '</div>' : '') + '</div></article>';
  }).join('') : '<div class="empty-state">아직 저장된 테마 일정 템플릿이 없습니다.</div>';
};

var __renderTemplateScheduleRecommendationsV25b = renderTemplateScheduleRecommendationsV25;
renderTemplateScheduleRecommendationsV25 = function () {
  ensureTemplateScheduleSectionV25();
  var list = document.getElementById('template-schedule-list');
  if (!list) return;
  var rows = (state.themeTemplates || []).slice().sort(function (a, b) {
  ...
  list.innerHTML = rows.length ? rows.map(function (template) {
    ...
    var foods = Array.isArray(template.wantedFoods) ? template.wantedFoods.filter(Boolean) : [];
    var coverPlace = (Array.isArray(template.placeIds) ? template.placeIds : []).map(function(id){ return placeMap[id]; }).filter(Boolean)[0];
    var cover = getPlaceCoverAssetV28(coverPlace, template.themes || [], template.regionKo || template.regionOriginal || '');
    return '<article class="template-schedule-card premium-template-card">' +
      '<div class="template-cover">' + (cover ? '<img src="' + escapeHtml(cover) + '" alt="' + escapeHtml(template.title || '추천 일정') + '" />' : '<div class="placeholder-image">일정</div>') + '</div>' +
      '<div class="template-schedule-body"><div class="template-schedule-top"><div><h3>' + escapeHtml(template.title || '추천 일정') + '</h3><div class="subline">' + escapeHtml(template.regionKo || template.regionOriginal || '') + '</div></div><div class="badge-row">' + (Array.isArray(template.themes) ? template.themes.map(function (theme) { return badge(theme); }).join('') : '') + '</div></div>' +
      (template.description ? '<p class="content-summary">' + escapeHtml(template.description) + '</p>' : '') +
      (foods.length ? '<div class="meta-line">' + foods.map(function (food) { return '<span class="badge soft">' + escapeHtml(food) + '</span>'; }).join('') + '</div>' : '') +
      (placeNames.length ? '<div class="route-pill-row">' + placeNames.slice(0,4).map(function (name, index) { return '<span class="route-pill"><strong>' + (index + 1) + '</strong><span>' + escapeHtml(name) + '</span></span>'; }).join('') + '</div>' : '') +
      '<div class="action-row compact-top"><button class="btn btn-primary btn-mini" type="button" data-add-template-day="' + escapeHtml(template.id) + '">첫 빈 날에 추가</button><button class="btn btn-ghost btn-mini" type="button" data-open-template-plan="' + escapeHtml(template.id) + '">일정표 보기</button></div></div>' +
      '</article>';
  }).join('') : '<div class="empty-state">아직 공개된 추천 일정 템플릿이 없습니다.</div>';
  ...
  list.querySelectorAll('[data-open-template-plan]').forEach(function (button) {
    button.addEventListener('click', function () { setView('plan'); });
  });
};


var __submitAdminLoginV28 = submitAdminLogin;
submitAdminLogin = function () {
  ...
  var nickname = String(((document.getElementById('account-nickname-input') || {}).value) || '').trim();
  if (!email || !password) {
    if (els.adminLoginMessage) els.adminLoginMessage.textContent = mode === 'signup' ? '이메일과 비밀번호를 입력해 주세요.' : '이메일(또는 아이디)과 비밀번호를 입력해 주세요.';
    return;
  }
  if (mode === 'signup' && !nickname) nickname = email.split('@')[0] || '여행자';
  if (els.adminLoginSubmit) {
    els.adminLoginSubmit.disabled = true;
    els.adminLoginSubmit.textContent = mode === 'signup' ? '가입 중...' : '로그인 중...';
  }
  ...
  var payload = mode === 'signup' ? { email: email, password: password, nickname: nickname } : { email: email, password: password };
  postJson(endpoint, payload).then(function (json) {
    if (!json || !json.user) throw new Error(mode === 'signup' ? '회원가입 정보를 확인하지 못했습니다.' : '로그인 정보를 확인하지 못했습니다.');
    persistCurrentUserV27(json.user);
    if (els.adminLoginMessage) els.adminLoginMessage.textContent = json.message || (mode === 'signup' ? '회원가입이 완료되었습니다.' : '로그인되었습니다.');
...
// ===== v29 premium ui + auth/status reliability patch =====
(function () {
  function updateServiceBadgeText() {
    fetch('/api/health').then(function (r) { return r.json(); }).then(function (json) {
      state.apiAvailable = true;
      state.liveEnabled = Boolean(json.hotpepperConfigured);
      state.geminiAvailable = Boolean(json.geminiConfigured);
      state.geminiModels = Array.isArray(json.geminiModels) ? json.geminiModels.slice() : [];
      var maps = document.getElementById('status-maps-header');
      var hp = document.getElementById('status-hotpepper-header');
      var gm = document.getElementById('status-gemini-header');
      if (maps) maps.textContent = '연결됨';
      if (hp) hp.textContent = state.liveEnabled ? '연결됨' : '데모';
      if (gm) gm.textContent = state.geminiAvailable ? '연결됨' : '미연결';
      if (els.statusHotpepper) {
        els.statusHotpepper.textContent = state.liveEnabled ? 'Hot Pepper' : '데모 데이터';
        els.statusHotpepper.className = 'status-light ' + (state.liveEnabled ? 'on' : 'off');
      }
      if (els.statusGemini) {
        els.statusGemini.textContent = state.geminiAvailable ? 'Gemini' : 'Gemini 미연결';
        els.statusGemini.className = 'status-light ' + (state.geminiAvailable ? 'on' : 'off');
      }
      var mode = document.getElementById('mode-badge');
      if (mode) mode.textContent = state.liveEnabled ? '실시간 모드' : '데모 모드';
    }).catch(function () {});
  }

  function bindImagesFallbackV29(scope) {
    (scope || document).querySelectorAll('img').forEach(function (img) {
      if (img.dataset.v29fallback) return;
      img.dataset.v29fallback = '1';
      img.addEventListener('error', function () {
        if (img.src.indexOf('/assets/thumb-map.jpg') >= 0) return;
        img.src = '/assets/thumb-map.jpg';
      });
    });
  }

  function setAuthModeV29(mode) {
    var hidden = document.getElementById('account-auth-mode');
    var nicknameRow = document.getElementById('account-nickname-row');
    var loginTab = document.getElementById('account-mode-login');
    var signupTab = document.getElementById('account-mode-signup');
    if (hidden) hidden.value = mode;
    if (nicknameRow) nicknameRow.classList.toggle('hidden', mode !== 'signup');
    if (loginTab) loginTab.classList.toggle('active', mode === 'login');
    if (signupTab) signupTab.classList.toggle('active', mode === 'signup');
    if (els.adminLoginSubmit) els.adminLoginSubmit.textContent = mode === 'signup' ? '회원가입' : '로그인';
    if (els.adminLoginMessage) els.adminLoginMessage.textContent = mode === 'signup' ? '닉네임을 비워두면 이메일 앞부분으로 자동 생성됩니다.' : '';
  }

  function handleAuthSubmitV29() {
    var mode = ((document.getElementById('account-auth-mode') || {}).value || 'login');
  ...
  function rebindAuthV29() {
    var loginTab = document.getElementById('account-mode-login');
    var signupTab = document.getElementById('account-mode-signup');
    var submitBtn = document.getElementById('admin-login-submit');
    [loginTab, signupTab, submitBtn].forEach(function (node) {
      if (!node || !node.parentNode) return;
      var clone = node.cloneNode(true);
      node.parentNode.replaceChild(clone, node);
      if (clone.id === 'account-mode-login') loginTab = clone;
      if (clone.id === 'account-mode-signup') signupTab = clone;
      if (clone.id === 'admin-login-submit') submitBtn = clone;
    });
    if (loginTab) loginTab.addEventListener('click', function () { setAuthModeV29('login'); });
    if (signupTab) signupTab.addEventListener('click', function () { setAuthModeV29('signup'); });
    if (submitBtn) submitBtn.addEventListener('click', handleAuthSubmitV29);
    if (els) els.adminLoginSubmit = submitBtn;
    setAuthModeV29('login');
  }

  var __renderContentViewV29 = renderContentView;
  renderContentView = function () {
    __renderContentViewV29();
    bindImagesFallbackV29(document.getElementById('content-view'));
  };
  var __renderCommunityViewV29 = renderCommunityView;
  renderCommunityView = function () {
    __renderCommunityViewV29();
    bindImagesFallbackV29(document.getElementById('community-view'));
  };
  var __renderTemplateScheduleRecommendationsV29 = renderTemplateScheduleRecommendationsV25;
  renderTemplateScheduleRecommendationsV25 = function () {
    __renderTemplateScheduleRecommendationsV29();
    bindImagesFallbackV29(document.getElementById('template-schedule-section'));
  };
  var __renderThemePlaceRegistryV29 = renderThemePlaceRegistry;
  renderThemePlaceRegistry = function () {
    __renderThemePlaceRegistryV29();
    bindImagesFallbackV29(document.getElementById('theme-place-registry'));
  };
  var __renderThemeTemplateRegistryV29 = renderThemeTemplateRegistry;
  renderThemeTemplateRegistry = function () {
    __renderThemeTemplateRegistryV29();
    bindImagesFallbackV29(document.getElementById('theme-template-list'));
  };

  document.addEventListener('DOMContentLoaded', function () {
    setTimeout(function () {
      rebindAuthV29();
      updateServiceBadgeText();
      bindImagesFallbackV29(document);
      updateNavUserBadgeV28();
    }, 100);
  });
})();
...
(function(){
  function setServiceBadge(id, connected, onText, offText){
    var el=document.getElementById(id);
    if(!el) return;
    el.setAttribute('data-state', connected ? 'on' : 'off');
    var em=el.querySelector('em');
    if(em) em.textContent = connected ? (onText||'연결됨') : (offText||'미연결');
  }

  function syncServiceBadgesV30(health){
    var hp = !!(health && health.hotpepperConfigured);
    var gm = !!(health && health.geminiConfigured);
    var maps = !!(health && health.googleMapsReady);
    setServiceBadge('status-maps-header', maps, '연결 확인', '미연결');
    setServiceBadge('status-hotpepper-header', hp, '연결됨', '미연결');
    setServiceBadge('status-gemini-header', gm, '연결됨', '미연결');
    if (typeof state !== 'undefined') {
      state.apiAvailable = true;
      state.liveEnabled = hp;
      state.geminiAvailable = gm;
    }
    var mode = document.getElementById('mode-badge');
    if (mode) mode.textContent = hp ? '실시간 모드' : '데모 모드';
    var api = document.getElementById('api-status');
    if (api) api.textContent = hp ? (gm ? '실시간 검색 + AI 설명 사용 가능' : '실시간 검색 가능') : '데모 데이터 모드';
    var sh=document.getElementById('status-hotpepper'); if(sh){ sh.className='status-light '+(hp?'on':'off'); sh.textContent=hp?'Hot Pepper 연결':'Hot Pepper 미연결'; }
    var sg=document.getElementById('status-gemini'); if(sg){ sg.className='status-light '+(gm?'on':'off'); sg.textContent=gm?'Gemini 연결':'Gemini 미연결'; }
  }

  var __detectApiV30 = typeof detectApi === 'function' ? detectApi : null;
  detectApi = function(){
    return fetch('/api/health')
      .then(function(r){ return r.json(); })
      .then(function(json){
        syncServiceBadgesV30(json||{});
        if (typeof state !== 'undefined' && json && Array.isArray(json.geminiModels)) state.geminiModels = json.geminiModels.slice();
        return json;
      })
      .catch(function(err){
        if (__detectApiV30) return __detectApiV30();
        throw err;
      });
  };

  function bindAuthV30(){
    var submit = document.getElementById('admin-login-submit');
    var loginTab = document.getElementById('account-mode-login');
    var signupTab = document.getElementById('account-mode-signup');
    var cancelBtn = document.getElementById('admin-login-cancel');
    [submit,loginTab,signupTab,cancelBtn].forEach(function(node){
      if (!node) return;
      var clone=node.cloneNode(true);
      node.parentNode.replaceChild(clone,node);
      if (clone.id==='admin-login-submit') submit=clone;
      if (clone.id==='account-mode-login') loginTab=clone;
      if (clone.id==='account-mode-signup') signupTab=clone;
      if (clone.id==='admin-login-cancel') cancelBtn=clone;
    });
    function setMode(mode){
      var hidden=document.getElementById('account-auth-mode');
      var nickRow=document.getElementById('account-nickname-row');
      if(hidden) hidden.value=mode;
      if(nickRow) nickRow.classList.toggle('hidden', mode!=='signup');
      if(loginTab) loginTab.classList.toggle('active', mode==='login');
      if(signupTab) signupTab.classList.toggle('active', mode==='signup');
      var msg=document.getElementById('admin-login-message');
      if(msg) msg.textContent = mode==='signup' ? '닉네임을 비워두면 이메일 앞부분으로 자동 생성됩니다.' : '';
      if(submit) submit.textContent = mode==='signup' ? '회원가입' : '로그인';
    }
    if(loginTab) loginTab.addEventListener('click', function(){ setMode('login'); });
    if(signupTab) signupTab.addEventListener('click', function(){ setMode('signup'); });
    if(cancelBtn) cancelBtn.addEventListener('click', function(){ if (typeof closeAdminModal==='function') closeAdminModal(); });
    if(submit) submit.addEventListener('click', function(){
      ...
      var nickname=String((document.getElementById('account-nickname-input')||{}).value||'').trim();
      var msg=document.getElementById('admin-login-message');
      if(!email || !password){ if(msg) msg.textContent = mode==='signup' ? '이메일과 비밀번호를 입력해 주세요.' : '이메일(또는 아이디)과 비밀번호를 입력해 주세요.'; return; }
      if(mode==='signup' && !nickname) nickname=(email.split('@')[0]||'여행자');
      ...
      postJson(endpoint,payload).then(function(json){
        if(!json || !json.user) throw new Error(mode==='signup' ? '회원가입 정보를 확인하지 못했습니다.' : '로그인 정보를 확인하지 못했습니다.');
        if(typeof persistCurrentUserV27==='function') persistCurrentUserV27(json.user);
        if(msg) msg.textContent = json.message || (mode==='signup' ? '회원가입이 완료되었습니다.' : '로그인되었습니다.');
        setMode('login');
        setTimeout(function(){ if (typeof closeAdminModal==='function') closeAdminModal(); }, 350);
      }).catch(function(error){
        if(msg) msg.textContent = error && error.message ? error.message : (mode==='signup' ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.');
      }).finally(function(){ submit.disabled=false; submit.textContent=mode==='signup'?'회원가입':'로그인'; });
    });
    setMode('login');
  }
  ...
  function attachFallbackImages(){
    document.querySelectorAll('img').forEach(function(img){
      ...
      img.addEventListener('error', function(){
        ...
        if (img.closest('.mini-map-preview') || img.closest('.hero-side-media')) img.src='/assets/thumb-map.jpg';
        else img.src='/assets/thumb-tokyo.jpg';
      });
    });
  }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    setTimeout(function(){
      bindAuthV30();
      detectApi().catch(function(){});
      attachFallbackImages();
      var userBadge=document.getElementById('nav-user-badge');
      if(userBadge && typeof state!=='undefined' && state.currentUser){
        userBadge.classList.remove('hidden');
        userBadge.textContent = (state.currentUser.nickname || state.currentUser.email || '여행자') + (state.currentUser.role==='admin' ? ' · 관리자' : '');
      }
    }, 80);
  });
})();

/* ===== v31 refined ui + auth/status patch ===== */
(function(){
  function byId(id){ return document.getElementById(id); }
  function setBadgeState(id, connected){
    var target = byId(id);
    if(!target) return;
    var holder = target.classList && target.classList.contains('header-service-badge') ? target : (target.closest ? target.closest('.header-service-badge') : null) || target;
    holder.setAttribute('data-state', connected ? 'on' : 'off');
    if (target.tagName === 'EM' || target.tagName === 'SPAN' || target.tagName === 'DIV') {
      target.textContent = connected ? '연결됨' : '미연결';
    } else {
      var em = holder.querySelector('em');
      if (em) em.textContent = connected ? '연결됨' : '미연결';
    }
  }
  function syncHealth(health){
    health = health || {};
    var hp = !!health.hotpepperConfigured;
    var gm = !!health.geminiConfigured;
    var maps = !!health.googleMapsReady;
    setBadgeState('status-maps-header', maps);
    setBadgeState('status-hotpepper-header', hp);
    setBadgeState('status-hotpepper-header-2', hp);
    setBadgeState('status-gemini-header', gm);
    var mode = byId('mode-badge');
    if(mode) mode.textContent = hp ? '실시간 모드' : '데모 모드';
    var apiStatus = byId('api-status');
    if(apiStatus) apiStatus.textContent = hp ? (gm ? '실시간 검색 · AI 설명 사용 가능' : '실시간 검색 사용 가능') : '데모 데이터 모드';
    var sh = byId('status-hotpepper');
    if(sh){ sh.className = 'status-light ' + (hp ? 'on' : 'off'); sh.textContent = hp ? 'Hot Pepper' : 'Hot Pepper'; }
    var sg = byId('status-gemini');
    if(sg){ sg.className = 'status-light ' + (gm ? 'on' : 'off'); sg.textContent = gm ? 'Gemini' : 'Gemini'; }
  }
  function refreshHealth(){
    return fetch('/api/health', { cache: 'no-store' })
      .then(function(r){ return r.ok ? r.json() : {}; })
      .then(function(json){ syncHealth(json); return json; })
      .catch(function(){ return null; });
  }
  ...
  function rebindAuthV31(){
    ...
    var cancelBtn = byId('admin-login-cancel');
    var hiddenMode = byId('account-auth-mode');
    var nickRow = byId('account-nickname-row');
    var idInput = byId('admin-id-input');
    var pwInput = byId('admin-password-input');
    var nickInput = byId('account-nickname-input');
    var msg = byId('admin-login-message');
    [loginTab, signupTab, submit, cancelBtn].forEach(function(node){
      if(!node || !node.parentNode) return;
      var clone = node.cloneNode(true);
      node.parentNode.replaceChild(clone, node);
      if(clone.id === 'account-mode-login') loginTab = clone;
    ...
    function setMode(mode){
      ...
      if(nickRow) nickRow.classList.toggle('hidden', mode !== 'signup');
      if(submit) submit.textContent = mode === 'signup' ? '회원가입' : '로그인';
      if(msg) msg.textContent = mode === 'signup' ? '닉네임은 비워두면 이메일 앞부분으로 자동 생성됩니다.' : '관리자 계정은 admin / admin 으로 로그인합니다.';
    }
    if(loginTab) loginTab.addEventListener('click', function(){ setMode('login'); });
    if(signupTab) signupTab.addEventListener('click', function(){ setMode('signup'); });
    if(cancelBtn) cancelBtn.addEventListener('click', function(){ if(typeof closeAdminModal === 'function') closeAdminModal(); else modal.classList.add('hidden'); });
    if(submit) submit.addEventListener('click', function(){
      var mode = hiddenMode && hiddenMode.value ? hiddenMode.value : 'login';
      var email = String(idInput && idInput.value || '').trim();
      var password = String(pwInput && pwInput.value || '').trim();
      var nickname = String(nickInput && nickInput.value || '').trim();
      if(!email || !password){ if(msg) msg.textContent = mode === 'signup' ? '이메일과 비밀번호를 입력해 주세요.' : '이메일 또는 아이디와 비밀번호를 입력해 주세요.'; return; }
      if(mode === 'signup' && !nickname) nickname = (email.split('@')[0] || '여행자').slice(0, 20);
      submit.disabled = true;
      submit.textContent = mode === 'signup' ? '가입 중...' : '로그인 중...';
      fetch(mode === 'signup' ? '/api/auth/signup' : '/api/auth/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(mode === 'signup' ? { email: email, password: password, nickname: nickname } : { email: email, password: password })
      }).then(function(r){
        return r.json().catch(function(){ return {}; }).then(function(json){ if(!r.ok) throw new Error(extractMessage(json, mode === 'signup' ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.')); return json; });
      }).then(function(json){
        if(typeof persistCurrentUserV27 === 'function') persistCurrentUserV27(json.user);
        if(msg) msg.textContent = json.message || (mode === 'signup' ? '회원가입이 완료되었습니다.' : '로그인되었습니다.');
        setMode('login');
        ...
        if(mode === 'signup' && idInput) idInput.value = email;
        setTimeout(function(){
          if(typeof closeAdminModal === 'function') closeAdminModal();
          var badge = byId('nav-user-badge');
          if (badge && json.user) {
            badge.classList.remove('hidden');
            badge.textContent = (json.user.nickname || json.user.email || '여행자') + (json.user.role === 'admin' ? ' · 관리자' : '');
          }
        }, 220);
      }).catch(function(err){
        if(msg) msg.textContent = err && err.message ? err.message : (mode === 'signup' ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.');
      }).finally(function(){
        submit.disabled = false;
        submit.textContent = mode === 'signup' ? '회원가입' : '로그인';
      });
    });
    setMode((hiddenMode && hiddenMode.value) || 'login');
  }

  document.addEventListener('DOMContentLoaded', function(){
    setTimeout(function(){ rebindAuthV31(); refreshHealth(); }, 50);
    setTimeout(refreshHealth, 500);
    setTimeout(refreshHealth, 1800);
    document.addEventListener('visibilitychange', function(){ if(!document.hidden) refreshHealth(); }, { once: true });
  });
})();


/* ===== v32 layout/auth/status/reroll/community patch ===== */
(function(){
  function byId(id){ return document.getElementById(id); }
  function setBadge(id, ok){
    var node=byId(id); if(!node) return;
    var holder=node.closest ? node.closest('.header-service-badge') : null;
    if(holder) holder.setAttribute('data-state', ok ? 'on':'off');
    node.textContent = ok ? '연결됨' : '미연결';
  }
  function refreshHeaderHealthV32(){
    fetch('/api/health',{cache:'no-store'}).then(function(r){return r.ok?r.json():{};}).then(function(h){
      setBadge('status-maps-header', !!h.googleMapsReady);
      setBadge('status-hotpepper-header', !!h.hotpepperConfigured);
      setBadge('status-gemini-header', !!h.geminiConfigured);
      var mode=byId('mode-badge'); if(mode) mode.textContent = h.hotpepperConfigured ? '실시간 모드' : '데모 모드';
      var api=byId('api-status'); if(api) api.textContent = h.hotpepperConfigured ? ((h.geminiConfigured?'실시간 검색 · AI 설명 사용 가능':'실시간 검색 사용 가능')) : '데모 데이터 모드';
      var sh=byId('status-hotpepper'); if(sh) sh.className='status-light '+(h.hotpepperConfigured?'on':'off');
      var sg=byId('status-gemini'); if(sg) sg.className='status-light '+(h.geminiConfigured?'on':'off');
    }).catch(function(){});
  }

  function bindAuthV32(){
    var modal=byId('admin-login-modal'); if(!modal) return;
    var loginTab=byId('account-mode-login');
    var signupTab=byId('account-mode-signup');
    var submit=byId('admin-login-submit');
    var cancel=byId('admin-login-cancel');
    var idInput=byId('admin-id-input');
    var pwInput=byId('admin-password-input');
    var nickRow=byId('account-nickname-row');
    var nickInput=byId('account-nickname-input');
    ...
    [loginTab,signupTab,submit,cancel].forEach(function(node){
      if(!node||!node.parentNode) return;
      var clone=node.cloneNode(true); node.parentNode.replaceChild(clone,node);
      if(clone.id==='account-mode-login') loginTab=clone;
      if(clone.id==='account-mode-signup') signupTab=clone;
      if(clone.id==='admin-login-submit') submit=clone;
      if(clone.id==='admin-login-cancel') cancel=clone;
    });
    function setMode(mode){
    ...
    if(loginTab) loginTab.addEventListener('click', function(){ setMode('login'); });
    if(signupTab) signupTab.addEventListener('click', function(){ setMode('signup'); });
    if(cancel) cancel.addEventListener('click', function(){ if(typeof closeAdminModal==='function') closeAdminModal(); else modal.classList.add('hidden'); });
    if(submit) submit.addEventListener('click', function(){
      ...
      fetch(mode==='signup'?'/api/auth/signup':'/api/auth/login',{
        ...
        body:JSON.stringify(mode==='signup'?{email:email,password:password,nickname:nickname}:{email:email,password:password})
      }).then(function(r){ return r.json().catch(function(){return {};}).then(function(json){ if(!r.ok) throw new Error((json&& (json.message||json.error)) || (mode==='signup'?'회원가입에 실패했습니다.':'로그인에 실패했습니다.')); return json; }); }).then(function(json){
        if(typeof persistCurrentUserV27==='function') persistCurrentUserV27(json.user);
        if(msg) msg.textContent = json.message || (mode==='signup'?'회원가입이 완료되었습니다.':'로그인되었습니다.');
        var badge=byId('nav-user-badge');
        if(badge && json.user){ badge.classList.remove('hidden'); badge.textContent=(json.user.nickname||json.user.email||'여행자') + (json.user.role==='admin'?' · 관리자':''); }
        if(mode==='signup'){ setMode('login'); if(pwInput) pwInput.value=''; if(nickInput) nickInput.value=''; }
        setTimeout(function(){ if(typeof closeAdminModal==='function') closeAdminModal(); }, 220);
      }).catch(function(err){ if(msg) msg.textContent = err && err.message ? err.message : (mode==='signup'?'회원가입에 실패했습니다.':'로그인에 실패했습니다.'); }).finally(function(){ submit.disabled=false; submit.textContent=mode==='signup'?'회원가입':'로그인'; });
    });
    setMode('login');
  }

  function applyCommunityThemeUI(){
    var input=byId('community-themes');
    if(!input) return;
    input.classList.add('hidden-field-input');
  }

  var _oldRerollDay = typeof rerollPlanDayV12 === 'function' ? rerollPlanDayV12 : null;
  if(_oldRerollDay){
    rerollPlanDayV12 = function(dayIndex){
      var btn=document.querySelector('[data-reroll-day="'+dayIndex+'"]');
      if(btn){ btn.disabled=true; btn.classList.add('loading'); btn.dataset.originalText = btn.textContent; btn.textContent='추천 중...'; }
      return Promise.resolve(_oldRerollDay(dayIndex)).finally(function(){ if(btn){ btn.disabled=false; btn.classList.remove('loading'); btn.textContent=btn.dataset.originalText || '이 날짜 다시 추천'; }});
    };
  }
  var _oldRerollSlot = typeof rerollPlanSlotV25 === 'function' ? rerollPlanSlotV25 : null;
  if(_oldRerollSlot){
    rerollPlanSlotV25 = function(dayIndex, slot){
      var btn=document.querySelector('[data-reroll-slot="'+dayIndex+':'+slot+'"]');
      if(btn){ btn.disabled=true; btn.classList.add('loading'); btn.dataset.originalText=btn.textContent; btn.textContent='추천 중...'; }
      return Promise.resolve(_oldRerollSlot(dayIndex, slot)).finally(function(){ if(btn){ btn.disabled=false; btn.classList.remove('loading'); btn.textContent=btn.dataset.originalText || '이 식사 다시 추천'; }});
    };
  }

  document.addEventListener('DOMContentLoaded', function(){
    setTimeout(function(){ refreshHeaderHealthV32(); bindAuthV32(); applyCommunityThemeUI(); }, 60);
    setTimeout(refreshHeaderHealthV32, 800);
    setTimeout(refreshHeaderHealthV32, 1800);
  });
})();
