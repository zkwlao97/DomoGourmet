...
async function handleAuthSignup(req, res) {
  ...
  await Promise.resolve(saveStore(store));
  return sendJson(res, 200, { ok: true, user: publicUser(user), ...summarizeStore(store) });
}
...
async function handleAuthLogin(req, res) {
  ...
  if (identifier === 'admin' && password === 'admin') {
    return sendJson(res, 200, { ok: true, user: { id: 'admin', email: 'admin', nickname: '관리자', role: 'admin' } });
  }
