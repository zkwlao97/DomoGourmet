...
/* ===== v34 claude fixes ===== */
(function(){
  function updateVisibleStatusBadgesV34(){
    var hpBadge = document.querySelector('.status-badge-hotpepper');
    var gmBadge = document.querySelector('.status-badge-gemini');
    var hpText = document.getElementById('status-hotpepper-text');
    var gmText = document.getElementById('status-gemini-text');
    if (hpText) hpText.textContent = state.liveEnabled ? '연결됨' : (state.apiAvailable ? '데모' : '미연결');
    if (gmText) gmText.textContent = state.geminiAvailable ? '연결됨' : '미연결';
    if (hpBadge) hpBadge.setAttribute('data-state', state.liveEnabled ? 'on' : (state.apiAvailable ? 'demo' : 'off'));
    if (gmBadge) gmBadge.setAttribute('data-state', state.geminiAvailable ? 'on' : 'off');
  }
  var __syncHeaderStatusLightsV27 = syncHeaderStatusLightsV27;
  syncHeaderStatusLightsV27 = function(){
    __syncHeaderStatusLightsV27();
    updateVisibleStatusBadgesV34();
  };

  function ensureFloatingDockObserverV34(){
    var dock = document.querySelector('.floating-dock');
    var footer = document.querySelector('.feature-footer');
    if(!dock || !footer || dock.dataset.footerObserverBound==='1') return;
    dock.dataset.footerObserverBound='1';
  ...
  var __renderResultsV34 = renderResults;
  renderResults = function(){
    __renderResultsV34();
    updateVisibleStatusBadgesV34();
  };

  var __renderPlanV34 = renderPlan;
  renderPlan = function(){
    __renderPlanV34();
    ensureFloatingDockObserverV34();
    var cards = document.querySelectorAll('.day-route-card');
    cards.forEach(function(card){
      var btn = card.querySelector('[data-reroll-day]');
      if(btn && !btn.dataset.boundLoading){
        btn.dataset.boundLoading='1';
  ...
  var __initV18EnhancementsV34 = initV18Enhancements;
  initV18Enhancements = function(){
    __initV18EnhancementsV34();
    ['theme-place-themes','theme-template-themes','content-themes','community-themes'].forEach(syncPickerFromHidden);
    updateVisibleStatusBadgesV34();
    ensureFloatingDockObserverV34();
  };

  var __setAuthModeV27_v34 = setAuthModeV27;
  setAuthModeV27 = function(mode){
    __setAuthModeV27_v34(mode);
    var row = document.getElementById('account-nickname-row');
    if(row) row.classList.toggle('hidden', mode !== 'signup');
  };

  var __submitAdminLoginV34 = submitAdminLogin;
  submitAdminLogin = function(){
    return __submitAdminLoginV34();
  };
