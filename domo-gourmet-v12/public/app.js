...
function showModal(node) {
  ...
  function renderCuratedView() {
    ...
    if (!keys.length) {
      els.curatedList.innerHTML = '<div class="empty-state">관리자 추천 식당이 아직 없습니다.</div>';
      return;
    }
  ...
  function persistAdminRecommendation(id, reason) {
    return fetch('/api/admin/recommend', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ restaurantId: id, reason: reason })
    }).then(function (response) {
  ...
  function getAllRestaurants() {
    return mergeById(state.customRestaurants || [], mergeById(state.planPool || [], mergeById(state.pool || [], state.results || [])));
  }
  ...
  function renderAdminDashboard() {
    ...
    }).join('') : '<div class="empty-state">관리자 추천 식당이 아직 없습니다.</div>';
    var customRows = (state.customRestaurants || []).slice().sort(function (a, b) {
    ...
    }).join('') : '<div class="empty-state">수동 등록 식당이 아직 없습니다.</div>';
    els.adminGroupedList.querySelectorAll('.result-admin-recommend').forEach(function (button) {
      button.addEventListener('click', function () { openAdminRecommendModal(button.getAttribute('data-id')); });
    });
  ...
  // v12 overrides: region alias aware itinerary, clearer route map links, per-day reroll
  var PLAN_REGION_ALIASES_V12 = [
    { canonical: '도쿄', display: '시부야 · 도쿄', aliases: ['시부야', '渋谷', 'shibuya'], focus: '시부야' },
    { canonical: '도쿄', display: '신주쿠 · 도쿄', aliases: ['신주쿠', '新宿', 'shinjuku'], focus: '신주쿠' },
    { canonical: '도쿄', display: '아키하바라 · 도쿄', aliases: ['아키하바라', '秋葉原', 'akihabara'], focus: '아키하바라' },
    { canonical: '도쿄', display: '아사쿠사 · 도쿄', aliases: ['아사쿠사', '浅草', 'asakusa'], focus: '아사쿠사' },
    { canonical: '도쿄', display: '우에노 · 도쿄', aliases: ['우에노', '上野', 'ueno'], focus: '우에노' },
    { canonical: '도쿄', display: '긴자 · 도쿄', aliases: ['긴자', '銀座', 'ginza'], focus: '긴자' },
    { canonical: '도쿄', display: '오다이바 · 도쿄', aliases: ['오다이바', 'お台場', 'odaiba'], focus: '오다이바' },
    { canonical: '오사카', display: '난바 · 오사카', aliases: ['난바', '難波', 'namba'], focus: '난바' },
    { canonical: '오사카', display: '우메다 · 오사카', aliases: ['우메다', '梅田', 'umeda'], focus: '우메다' },
    { canonical: '오사카', display: '도톤보리 · 오사카', aliases: ['도톤보리', '道頓堀', 'dotonbori'], focus: '도톤보리' },
    { canonical: '오사카', display: '신사이바시 · 오사카', aliases: ['신사이바시', '心斎橋', 'shinsaibashi'], focus: '신사이바시' },
    { canonical: '후쿠오카', display: '하카타 · 후쿠오카', aliases: ['하카타', '博多', 'hakata'], focus: '하카타' },
    { canonical: '후쿠오카', display: '텐진 · 후쿠오카', aliases: ['텐진', '天神', 'tenjin'], focus: '텐진' },
    { canonical: '홋카이도', display: '삿포로 · 홋카이도', aliases: ['삿포로', '札幌', 'sapporo'], focus: '삿포로' },
    { canonical: '홋카이도', display: '스스키노 · 홋카이도', aliases: ['스스키노', 'すすきの', 'susukino'], focus: '스스키노' },
  ];

  function inferPlanRegionV12(regionText) {
    var raw = String(regionText || '').trim();
    if (!raw) return { raw: '', canonical: '', display: '', focus: '' };
    var key = normalize(raw);
    for (var i = 0; i < PLAN_REGION_ALIASES_V12.length; i += 1) {
      var row = PLAN_REGION_ALIASES_V12[i];
      if ((row.aliases || []).some(function (alias) {
        var item = normalize(alias);
        return item === key || item.indexOf(key) >= 0 || key.indexOf(item) >= 0;
      })) {
        return { raw: raw, canonical: row.canonical, display: row.display, focus: row.focus };
      }
    }
    return { raw: raw, canonical: raw, display: raw, focus: raw };
  }

  function getPlanSearchRegionV12() {
    var ctx = state.planContext || {};
    return ctx.regionCanonical || ctx.region || state.currentQuery.region || '';
  }

  function getPlanDisplayRegionV12() {
    var ctx = state.planContext || {};
    return ctx.regionDisplay || ctx.region || ctx.regionCanonical || state.currentQuery.region || '';
  }

  function findRegionCatalog(region) {
    var info = inferPlanRegionV12(region);
    var canonical = info.canonical || String(region || '').trim();
    var direct = REGION_TRAVEL_SPOTS[canonical];
    if (direct && direct.length) return direct;
    var key = Object.keys(REGION_TRAVEL_SPOTS).find(function (item) {
      return normalize(item) === normalize(canonical) || normalize(item).indexOf(normalize(canonical)) >= 0 || normalize(canonical).indexOf(normalize(item)) >= 0;
    });
    return key ? REGION_TRAVEL_SPOTS[key] : [];
  }
  ...
  function ensureRegionalSeed(region) {
    var info = inferPlanRegionV12(region);
    if (!info.canonical && !info.focus) return Promise.resolve([]);
    return resolveBasePoint(info.canonical || info.raw, info.focus || info.raw, '').then(function (point) {
      return point ? [makePointDisplay(Object.assign({}, point, { source: 'region-center' }))] : [];
    }).catch(function () {
      return [];
    });
  ...
  function normalizePlanQueryV12(query) {
    var base = Object.assign({}, query || {});
    var info = inferPlanRegionV12(base.region || '');
    base.regionInput = String(base.region || '').trim();
    base.regionCanonical = info.canonical || base.regionInput;
    base.regionDisplay = info.display || base.regionInput;
    base.focusArea = info.focus || base.regionInput;
    return base;
  }

  function buildDisplayRegionTextV12(query) {
    return query.regionDisplay || query.region || query.regionCanonical || '해당 지역';
  }

  function generateNearbyPlacePlan(query, useAi) {
    var normalized = normalizePlanQueryV12(query);
    var searchRegion = normalized.regionCanonical || normalized.region || '';
    var lines = parsePlaceLines(els.planExtraPlaces ? els.planExtraPlaces.value : '');
    var wantedFoods = parseWantedFoods();
    if (!lines.length && !searchRegion) {
      return Promise.reject(new Error('방문 후보 장소 또는 여행 지역을 입력해 주세요.'));
    }
    return Promise.all(lines.map(function (line) { return resolvePointLineForRegion(searchRegion, line); })).then(function (resolved) {
      var seedPoints = uniquePointsStrict(resolved.filter(Boolean).map(makePointDisplay));
      return persistCachedPlaces(seedPoints, { region: searchRegion, source: 'user-input' }).then(function () { return seedPoints; });
    }).then(function (seedPoints) {
      var expansion = seedPoints.length
        ? Promise.resolve({ points: seedPoints, addedCount: 0, seedCount: seedPoints.length })
        : ensureRegionalSeed(normalized.focusArea || searchRegion).then(function (seed) { return { points: seed, addedCount: 0, seedCount: seed.length }; });
      return expansion;
    }).then(function (seedPayload) {
      if (!seedPayload.points.length) throw new Error('입력한 방문 후보를 기준점으로 읽지 못했습니다. 장소명이나 Google Maps 링크를 다시 확인해 주세요.');
      var targetCount = Math.max(normalized.days * 2, seedPayload.points.length);
      return buildExpandedPointSet(seedPayload.points, searchRegion, targetCount).then(function (expanded) {
        var finalPoints = uniquePointsStrict(expanded.points || []);
        return Promise.all(finalPoints.map(function (point) {
          return fetchCandidatesAroundPoint(point, getPrimaryFoodForGather(wantedFoods), normalized);
        })).then(function (groups) {
          return fillPlanFromPointGroups(groups, {
            type: 'nearby-multi',
            useAi: useAi,
            ...
            hotelPoint: null,
            overviewSeed: expanded.addedCount > 0
              ? '입력한 장소를 우선 배치하고, 부족한 시간대는 ' + buildDisplayRegionTextV12(normalized) + ' 근처 대표 장소를 보강했습니다.'
              : '입력한 장소를 오전 / 오후 동선으로 나누고, 근처 식당과 숙소 동선을 함께 배치했습니다.',
            wantedFoods: wantedFoods,
          }).then(function (story) {
            state.planContext = savePlanContextV9(Object.assign({}, state.planContext || {}, {
              region: normalized.region || '',
              regionDisplay: normalized.regionDisplay || normalized.region || '',
              regionCanonical: searchRegion,
              focusArea: normalized.focusArea || normalized.region || '',
            }));
            return story;
          });
        });
      ...
      return resolveBasePoint(searchRegion, normalized.hotel, normalized.hotelMapsLink);
    }).then(function (hotelPoint) {
      if (hotelPoint) {
        state.planContext.hotelPoint = makePointDisplay(hotelPoint);
        state.plan.forEach(function (day) { day.hotel = state.planContext.hotelPoint; });
  ...
  function generateRegionalAutoPlan(query, useAi) {
    var normalized = normalizePlanQueryV12(query);
    var searchRegion = normalized.regionCanonical || normalized.region || '';
    var focusArea = normalized.focusArea || normalized.region || searchRegion;
    var wantedFoods = parseWantedFoods();
    var lines = parsePlaceLines(els.planExtraPlaces ? els.planExtraPlaces.value : '');
    return Promise.all([
      Promise.all(lines.map(function (line) { return resolvePointLineForRegion(searchRegion, line); })),
      ensureRegionalSeed(focusArea || searchRegion)
    ]).then(function (values) {
      var resolved = values[0];
      var regionSeed = values[1] || [];
      var regionCenter = regionSeed[0] || null;
      ...
      return persistCachedPlaces(seedPoints, { region: searchRegion, source: 'user-input' }).then(function () {
        return { seedPoints: seedPoints, regionCenter: regionCenter };
      });
    }).then(function (seedPayload) {
      var targetCount = Math.max(normalized.days * 2, 2);
      return buildExpandedPointSet(seedPayload.seedPoints, searchRegion, targetCount).then(function (expanded) {
        var finalPoints = uniquePointsStrict((expanded.points || []).filter(function (point) {
          return isPointInsideRegion(point, seedPayload.regionCenter);
        }));
        if (!finalPoints.length && seedPayload.regionCenter) finalPoints = [seedPayload.regionCenter];
        ...
        return Promise.all(finalPoints.map(function (point) {
          ...
          return fillPlanFromPointGroups(groups, {
            ...
            query: normalized,
            hotelPoint: null,
            overviewSeed: buildDisplayRegionTextV12(normalized) + ' 안에서만 대표 장소를 골라 오전 / 오후 방문지와 식당을 나눴습니다. 다른 도시는 섞지 않았습니다.',
            wantedFoods: wantedFoods,
          }).then(function (story) {
            state.planContext = savePlanContextV9(Object.assign({}, state.planContext || {}, {
              region: normalized.region || '',
  ...
  function fillBlankDaysWithAi() {
    var incomplete = getIncompletePlanDayIndices();
    var region = getPlanSearchRegionV12();
    if (!incomplete.length) {
      if (els.planWarnings) els.planWarnings.innerHTML = '<div class="message-box success">이미 채워진 일정입니다.</div>';
      return Promise.resolve();
    }
    ...
    return fetch('/api/ai/suggest-places', {
      ...
      return generateRegionalAutoPlan({
        region: getPlanDisplayRegionV12(),
        hotel: (state.planContext && state.planContext.hotel) || '',
        hotelMapsLink: (state.planContext && state.planContext.hotelMapsLink) || '',
        startDate: (state.planContext && state.planContext.startDate) || toDateInput(new Date()),
        days: getEffectivePlanDays(),
  ...
  function renderRouteStep(label, origin, destination) {
    ...
    var originName = formatPointName(origin);
    var destinationName = formatPointName(destination);
    return '<a class="route-step" target="_blank" rel="noopener noreferrer" href="' + escapeHtml(buildPointDirectionsUrl(origin, destination)) + '">' +
      '<strong>' + escapeHtml(label) + '</strong>' +
      '<span>' + escapeHtml(originName + ' → ' + destinationName) + '</span>' +
      '<em class="route-map-cta">Google Maps 열기</em>' +
    '</a>';
  }

  function mealPointV12(item) {
    if (!item) return null;
    return {
      lat: item.restaurantLat,
      lng: item.restaurantLng,
      nameKo: item.restaurantName,
      nameOriginal: item.restaurantName,
      displayNameKo: item.restaurantName,
      displayNameOriginal: item.restaurantName,
    };
  }

  function renderPlanDayCard(day, dayNumber, date) {
    var hotel = day.hotel || getPlanContextHotelPoint();
    var morningVisit = day.visits && day.visits[0] ? day.visits[0] : null;
    var afternoonVisit = day.visits && day.visits[1] ? day.visits[1] : null;
    var breakfastPoint = mealPointV12(day.breakfast);
    var lunchPoint = mealPointV12(day.lunch);
    var dinnerPoint = mealPointV12(day.dinner);
    var steps = [];
    if (hotel && breakfastPoint) steps.push(renderRouteStep('숙소 → 아침 식당 지도', hotel, breakfastPoint));
    if (breakfastPoint && morningVisit) steps.push(renderRouteStep('아침 식당 → 오전 방문지 지도', breakfastPoint, morningVisit));
    if (morningVisit && lunchPoint) steps.push(renderRouteStep('오전 방문지 → 점심 식당 지도', morningVisit, lunchPoint));
    if (lunchPoint && afternoonVisit) steps.push(renderRouteStep('점심 식당 → 오후 방문지 지도', lunchPoint, afternoonVisit));
    if (afternoonVisit && dinnerPoint) steps.push(renderRouteStep('오후 방문지 → 저녁 식당 지도', afternoonVisit, dinnerPoint));
    if (dinnerPoint && hotel) steps.push(renderRouteStep('저녁 식당 → 숙소 지도', dinnerPoint, hotel));

    var visitHtml = uniquePointsStrict((day.visits || []).filter(Boolean)).map(function (visit, idx) {
      return '<div class="route-chip">' + escapeHtml((idx === 0 ? '오전 방문지' : '오후 방문지') + ' · ' + formatPointName(visit)) + '</div>';
    }).join('');

    return '<article class="day-route-card">' +
      '<div class="day-route-head"><div><strong>Day ' + dayNumber + '</strong><span>' + escapeHtml(formatDisplayDate(date)) + '</span></div><div class="route-head-actions"><button class="small-button route-reroll-button" type="button" data-reroll-day="' + (dayNumber - 1) + '">이 날 다시 추천</button></div></div>' +
      (hotel ? '<div class="route-chip hotel">숙소 · ' + escapeHtml(formatPointName(hotel)) + '</div>' : '') +
      (visitHtml ? '<div class="route-chip-row">' + visitHtml + '</div>' : '') +
      '<div class="route-meal-list">' +
        renderMealLine('아침', day.breakfast) +
        ...
        renderMealLine('저녁', day.dinner) +
      '</div>' +
      (steps.length ? '<div class="route-step-list">' + steps.join('') + '</div>' : '<div class="muted small">숙소와 방문지, 식당이 연결되면 구간별 Google Maps 링크가 여기에 표시됩니다.</div>') +
    '</article>';
  }

  function rerollPlanStoryDayV12(dayIndex, visits) {
    if (!state.planStory || !Array.isArray(state.planStory.days)) return;
    var titles = uniquePointsStrict(visits || []).map(function (point) { return formatPointName(point); }).filter(Boolean);
    state.planStory.days[dayIndex] = {
      day: dayIndex + 1,
      title: titles.join(' → ') || ('Day ' + (dayIndex + 1)),
      why: titles.length > 1
        ? '기존 일정 대신 ' + titles[0] + '과 ' + titles[1] + ' 쪽으로 다시 묶어 이동 동선과 식당 선택 폭을 넓혔습니다.'
        : (titles[0] ? titles[0] + ' 중심으로 하루 동선을 다시 묶었습니다.' : '하루 동선을 다시 정리했습니다.'),
    };
    saveJson(STORAGE.planStory, state.planStory);
    renderPlanStory();
  }

  function bindPlanDayRerollButtonsV12() {
    if (!els.planRouteCards) return;
    els.planRouteCards.querySelectorAll('[data-reroll-day]').forEach(function (button) {
      button.onclick = function () {
        var dayIndex = parseInt(button.getAttribute('data-reroll-day') || '-1', 10);
        if (dayIndex >= 0) rerollSinglePlanDayV12(dayIndex, button);
      };
    });
  }

  function rerollSinglePlanDayV12(dayIndex, button) {
    var ctx = state.planContext || defaultPlanContextV9();
    var region = ctx.regionCanonical || ctx.region || state.currentQuery.region || '';
    if (!region) {
      if (els.planWarnings) els.planWarnings.innerHTML = '<div class="message-box info">일정을 다시 추천하려면 여행 지역이 필요합니다.</div>';
      return Promise.resolve();
    }
    state.dayRerollCounts = state.dayRerollCounts || {};
    state.dayRerollCounts[dayIndex] = (state.dayRerollCounts[dayIndex] || 0) + 1;
    var rerollCount = state.dayRerollCounts[dayIndex];
    if (button) { button.disabled = true; button.textContent = '다시 찾는 중...'; }
    if (els.planWarnings) els.planWarnings.innerHTML = '<div class="message-box info">Day ' + (dayIndex + 1) + ' 일정을 다시 찾는 중입니다...</div>';

    var existingDay = state.plan[dayIndex] || {};
    var pivot = (existingDay.visits && existingDay.visits[0]) || ctx.basePoint || state.basePoint || null;
    var seedPoints = pivot ? [makePointDisplay(pivot)] : [];
    return buildExpandedPointSet(seedPoints, region, Math.max(getEffectivePlanDays() * 3, 10)).then(function (expanded) {
      var allPoints = uniquePointsStrict((expanded.points || []).concat(seedPoints));
      var usedVisitKeys = new Set();
      state.plan.forEach(function (day, idx) {
        if (idx === dayIndex) return;
        (day.visits || []).forEach(function (point) { usedVisitKeys.add(pointKey(point)); });
      });
      var candidatePoints = allPoints.filter(function (point) {
        return !usedVisitKeys.has(pointKey(point));
      });
      if (!candidatePoints.length) candidatePoints = allPoints.slice();
      candidatePoints.sort(function (a, b) {
        var ad = pivot ? distanceBetweenPoints(pivot, a) : NaN;
        var bd = pivot ? distanceBetweenPoints(pivot, b) : NaN;
        if (Number.isFinite(ad) && Number.isFinite(bd) && ad !== bd) return ad - bd;
        return (b.priority || 0) - (a.priority || 0);
      });
      var morningPoint = candidatePoints[rerollCount % candidatePoints.length] || candidatePoints[0] || pivot;
      var afternoonPoint = candidatePoints[(rerollCount + 1) % candidatePoints.length] || null;
      if (afternoonPoint && pointKey(afternoonPoint) === pointKey(morningPoint)) afternoonPoint = null;
      return Promise.all([
        morningPoint ? fetchCandidatesAroundPoint(morningPoint, '', { region: region, sort: 'score' }) : Promise.resolve({ point: morningPoint, results: [] }),
        afternoonPoint ? fetchCandidatesAroundPoint(afternoonPoint, '', { region: region, sort: 'score' }) : Promise.resolve({ point: afternoonPoint, results: [] })
      ]).then(function (groups) {
        ...
        state.plan.forEach(function (day, idx) {
          if (idx === dayIndex) return;
          ['breakfast', 'lunch', 'dinner'].forEach(function (slot) {
            if (day[slot] && day[slot].restaurantId) usedRestaurantIds.add(day[slot].restaurantId);
          });
        });
        var previousGenre = '';
        if (dayIndex > 0 && state.plan[dayIndex - 1] && state.plan[dayIndex - 1].dinner) previousGenre = state.plan[dayIndex - 1].dinner.restaurantType || '';
        var globalPool = mergeById(groups[0].results || [], groups[1].results || []);
        var breakfastPick = chooseRestaurantForSlot(getPlanContextHotelPoint() ? globalPool : ((groups[0].results || []).length ? groups[0].results : globalPool), 'breakfast', date, previousGenre, usedRestaurantIds, '');
        if (breakfastPick && breakfastPick.restaurant) { usedRestaurantIds.add(breakfastPick.restaurant.id); previousGenre = breakfastPick.restaurant.genreKo || breakfastPick.restaurant.genreOriginal || previousGenre; }
        var lunchPick = chooseRestaurantForSlot((groups[0].results || []).length ? groups[0].results : globalPool, 'lunch', date, previousGenre, usedRestaurantIds, '');
        if (lunchPick && lunchPick.restaurant) { usedRestaurantIds.add(lunchPick.restaurant.id); previousGenre = lunchPick.restaurant.genreKo || lunchPick.restaurant.genreOriginal || previousGenre; }
        var dinnerSource = (afternoonPoint && (groups[1].results || []).length) ? groups[1].results : (((groups[0].results || []).length ? groups[0].results : globalPool));
        var dinnerPick = chooseRestaurantForSlot(dinnerSource, 'dinner', date, previousGenre, usedRestaurantIds, '');

        state.plan[dayIndex].hotel = getPlanContextHotelPoint() ? makePointDisplay(getPlanContextHotelPoint()) : (existingDay.hotel || null);
        state.plan[dayIndex].visits = uniquePointsStrict([morningPoint, afternoonPoint]);
        if (breakfastPick && breakfastPick.restaurant) state.plan[dayIndex].breakfast = makePlanMeal(breakfastPick.restaurant, date, '숙소 출발 전후 아침 식사');
        if (lunchPick && lunchPick.restaurant) state.plan[dayIndex].lunch = makePlanMeal(lunchPick.restaurant, date, (morningPoint ? formatPointName(morningPoint) : '오전 동선') + ' 근처 점심');
        if (dinnerPick && dinnerPick.restaurant) state.plan[dayIndex].dinner = makePlanMeal(dinnerPick.restaurant, date, (afternoonPoint ? formatPointName(afternoonPoint) : (morningPoint ? formatPointName(morningPoint) : '오후 동선')) + ' 근처 저녁');
        state.planPool = mergeById(state.planPool || [], globalPool);
        saveJson(STORAGE.plan, state.plan);
        rerollPlanStoryDayV12(dayIndex, state.plan[dayIndex].visits);
        renderPlan();
        renderPlanSummary();
        if (els.planWarnings) els.planWarnings.innerHTML = '<div class="message-box success">Day ' + (dayIndex + 1) + ' 일정을 새 후보로 다시 추천했습니다.</div>';
      });
    }).catch(function (error) {
      if (els.planWarnings) els.planWarnings.innerHTML = '<div class="message-box info">' + escapeHtml(error && error.message ? error.message : '이 날짜를 다시 추천하지 못했습니다.') + '</div>';
    }).finally(function () {
      if (button) { button.disabled = false; button.textContent = '이 날 다시 추천'; }
    });
  }

  var __renderPlanOriginalV12 = renderPlan;
  renderPlan = function () {
    __renderPlanOriginalV12();
    bindPlanDayRerollButtonsV12();
  };
  ...
  function renderPlanSummary() {
    ...
    var total = getEffectivePlanDays() * 3;
    var ctx = state.planContext || defaultPlanContextV9();
    var displayRegion = ctx.regionDisplay || ctx.region || ctx.regionCanonical || '';
    var baseName = ctx.basePoint ? formatPointName(ctx.basePoint) : (displayRegion ? displayRegion + ' 기준 자동 추천' : '기준 장소 미설정');
    var hotelName = ctx.hotelPoint ? formatPointName(ctx.hotelPoint) : (ctx.hotel || '숙소 미설정');
    var incomplete = getIncompletePlanDayIndices();
    els.planSummary.innerHTML = '' +
      '<div class="plan-summary-box">' +
  ...
  // ===== v12 overrides =====
  const SUBREGION_PARENT_MAP_V12 = {
    '시부야': '도쿄',
    '신주쿠': '도쿄',
    '하라주쿠': '도쿄',
    '아사쿠사': '도쿄',
  ...
  function getRegionalContextV12(rawRegion) {
    var raw = String(rawRegion || '').trim();
    if (!raw) return { input: '', parent: '', focus: '', display: '' };
    var normalized = normalize(raw);
    var matchedFocus = Object.keys(SUBREGION_PARENT_MAP_V12).find(function (item) {
      var key = normalize(item);
      return key === normalized || key.includes(normalized) || normalized.includes(key);
    }) || '';
    var parent = matchedFocus ? SUBREGION_PARENT_MAP_V12[matchedFocus] : raw;
    return {
      input: raw,
      parent: parent,
      focus: matchedFocus && normalize(parent) !== normalize(matchedFocus) ? matchedFocus : '',
      display: matchedFocus && normalize(parent) !== normalize(matchedFocus) ? (matchedFocus + ' · ' + parent) : raw,
    };
  }

  function getPlanDisplayRegionV12() {
    var ctx = state.planContext || {};
    if (ctx.regionDisplay) return ctx.regionDisplay;
    if (ctx.focusRegion && ctx.region && normalize(ctx.focusRegion) !== normalize(ctx.region)) {
      return ctx.focusRegion + ' · ' + ctx.region;
    }
    return ctx.region || state.currentQuery.region || '';
  }

  function readPlanModalQuery() {
    var rawRegion = String((els.planRegionInput && els.planRegionInput.value) || '').trim();
    var regionCtx = getRegionalContextV12(rawRegion);
    return {
      rawRegion: rawRegion,
      region: regionCtx.parent || rawRegion,
      focusRegion: regionCtx.focus,
      regionDisplay: regionCtx.display || rawRegion,
      hotel: String((els.planHotelInput && els.planHotelInput.value) || '').trim(),
      hotelMapsLink: String((els.planHotelMapsLinkInput && els.planHotelMapsLinkInput.value) || '').trim(),
      startDate: (els.planStartDateInput && els.planStartDateInput.value) || toDateInput(new Date()),
      days: clamp(parseInt((els.planDaysInput && els.planDaysInput.value) || '3', 10), 1, 10),
  ...
  function mergeCatalogEntriesV12(primaryRegion, focusRegion) {
    var rows = [];
    if (focusRegion) rows = rows.concat(findRegionCatalog(focusRegion));
    if (primaryRegion) rows = rows.concat(findRegionCatalog(primaryRegion));
    var used = {};
  ...
  function resolveCatalogPointsV12(primaryRegion, focusRegion) {
    var entries = mergeCatalogEntriesV12(primaryRegion, focusRegion);
    if (!entries.length) return Promise.resolve([]);
    return Promise.all(entries.map(function (entry) { return resolveCatalogSpot(entry, primaryRegion || focusRegion || ''); })).then(function (rows) {
      return uniquePointsStrict(rows.filter(Boolean).map(makePointDisplay));
    });
  }

  function sortPointsByAnchorV12(points, anchors) {
    var safeAnchors = (anchors || []).filter(Boolean);
    return (points || []).slice().sort(function (a, b) {
      var ad = safeAnchors.length ? Math.min.apply(null, safeAnchors.map(function (seed) { return distanceBetweenPoints(seed, a); }).filter(Number.isFinite)) : NaN;
      var bd = safeAnchors.length ? Math.min.apply(null, safeAnchors.map(function (seed) { return distanceBetweenPoints(seed, b); }).filter(Number.isFinite)) : NaN;
      if (Number.isFinite(ad) && Number.isFinite(bd) && ad !== bd) return ad - bd;
      if ((b.priority || 0) !== (a.priority || 0)) return (b.priority || 0) - (a.priority || 0);
      return String(a.area || '').localeCompare(String(b.area || ''));
    });
  ...
  function buildExpandedPointSetV12(initialPoints, primaryRegion, focusRegion, targetCount) {
    var seedPoints = uniquePointsStrict((initialPoints || []).filter(Boolean).map(makePointDisplay));
    var safeTarget = Math.max(targetCount || 0, seedPoints.length || 0, 1);
    return resolveCatalogPointsV12(primaryRegion, focusRegion).then(function (catalogPoints) {
      var used = {};
      var result = [];
      function addPoint(point, autoAdded) {
        var key = pointKey(point);
      ...
      seedPoints.forEach(function (point) { addPoint(point, false); });
      sortPointsByAnchorV12(catalogPoints.filter(function (point) { return !used[pointKey(point)]; }), seedPoints).forEach(function (point) {
        if (result.length < safeTarget) addPoint(point, true);
      });
      return { points: result, addedCount: Math.max(0, result.length - seedPoints.length), seedCount: seedPoints.length };
    });
  }

  function parseWantedFoodsFromContextV12() {
    return Array.isArray(state.planContext && state.planContext.wantedFoods) ? state.planContext.wantedFoods.slice() : [];
  }

  function getPlanQueryLikeContextV12() {
    var ctx = state.planContext || defaultPlanContextV9();
    return {
      region: ctx.region || '',
      focusRegion: ctx.focusRegion || '',
      regionDisplay: ctx.regionDisplay || getPlanDisplayRegionV12(),
      hotel: ctx.hotel || '',
      hotelMapsLink: ctx.hotelMapsLink || '',
      startDate: ctx.startDate || toDateInput(new Date()),
      days: ctx.days || getEffectivePlanDays(),
      importedFromSearch: Boolean(ctx.importedFromSearch),
      mapsLink: '',
      base: '',
      food: '',
      minScore: Number((els.minScoreInput && els.minScoreInput.value) || 3),
  ...
  function getOtherDayUsedRestaurantIdsV12(dayIndex) {
    var used = new Set();
    (state.plan || []).forEach(function (day, idx) {
      if (idx === dayIndex) return;
      ['breakfast', 'lunch', 'dinner'].forEach(function (slot) {
        if (day && day[slot] && day[slot].restaurantId) used.add(day[slot].restaurantId);
      });
    });
    return used;
  }

  function getOtherDayVisitKeysV12(dayIndex) {
    var used = new Set();
    (state.plan || []).forEach(function (day, idx) {
      if (idx === dayIndex) return;
      (day && day.visits || []).filter(Boolean).forEach(function (point) { used.add(pointKey(point)); });
    });
    return used;
  }

  function resolveFreshDayPointsV12(dayIndex) {
    var currentDay = state.plan[dayIndex] || {};
    var currentVisits = (currentDay.visits || []).filter(Boolean);
    var ctx = state.planContext || defaultPlanContextV9();
    var primaryRegion = ctx.region || '';
    var focusRegion = ctx.focusRegion || '';
    var desiredCount = Math.max(1, currentVisits.length || 2);
    var otherVisitKeys = getOtherDayVisitKeysV12(dayIndex);
    var anchors = currentVisits.length ? currentVisits : [getPlanContextHotelPoint(), getPlanContextBasePoint()].filter(Boolean);

    return resolveCatalogPointsV12(primaryRegion, focusRegion).then(function (catalogPoints) {
      var fresh = sortPointsByAnchorV12(catalogPoints.filter(function (point) {
        return !otherVisitKeys.has(pointKey(point));
      }), anchors);
      var replacementPool = fresh.filter(function (point) {
        return currentVisits.every(function (visit) { return pointKey(visit) !== pointKey(point); });
      });
      var selected = replacementPool.slice(0, desiredCount);
      if (selected.length < desiredCount) {
        currentVisits.forEach(function (point) {
          if (selected.length < desiredCount && point && !selected.some(function (row) { return pointKey(row) === pointKey(point); })) {
            selected.push(makePointDisplay(point));
          }
        });
      }
      return uniquePointsStrict(selected).slice(0, desiredCount);
    });
  }

  function rerollPlanDayV12(dayIndex) {
    if (!state.plan || !state.plan[dayIndex]) return Promise.resolve();
    var day = state.plan[dayIndex];
    var query = getPlanQueryLikeContextV12();
    var date = addDays(getEffectiveTripStartDate(), dayIndex);
    var wantedFoods = parseWantedFoodsFromContextV12();
    var lunchWanted = wantedFoods[dayIndex * 2] || wantedFoods[dayIndex] || '';
    var dinnerWanted = wantedFoods[dayIndex * 2 + 1] || wantedFoods[dayIndex + 1] || '';
    var button = document.querySelector('[data-reroll-day="' + dayIndex + '"]');
    if (button) {
      button.disabled = true;
      button.textContent = '다시 추천 중...';
    }
    if (els.planWarnings) {
      els.planWarnings.innerHTML = '<div class="message-box info">Day ' + escapeHtml(String(dayIndex + 1)) + ' 일정을 다시 고르는 중입니다.</div>';
    }

    return resolveFreshDayPointsV12(dayIndex).then(function (points) {
      var finalVisits = points.length ? points : (day.visits || []).filter(Boolean);
      if (!finalVisits.length) throw new Error('이 날짜를 다시 짤 방문 후보를 찾지 못했습니다.');
      return Promise.all(finalVisits.map(function (point) {
        return fetchCandidatesAroundPoint(point, '', query);
      })).then(function (groups) {
        var allRestaurants = getMergedGroupResults(groups);
        var usedRestaurantIds = getOtherDayUsedRestaurantIdsV12(dayIndex);
        var previousGenre = '';
        var morningPoint = finalVisits[0] || null;
        var afternoonPoint = finalVisits[1] || finalVisits[0] || null;
        var breakfastSource = getPlanContextHotelPoint() ? mergeById(state.planPool || [], allRestaurants) : ((groups[0] && groups[0].results) || mergeById(state.planPool || [], allRestaurants));
        var lunchSource = (groups[0] && groups[0].results && groups[0].results.length) ? groups[0].results : mergeById(state.planPool || [], allRestaurants);
        var dinnerSource = (groups[1] && groups[1].results && groups[1].results.length) ? groups[1].results : lunchSource;

        state.planPool = mergeById(state.planPool || [], allRestaurants);
        day.visits = uniquePointsStrict(finalVisits);
        day.hotel = getPlanContextHotelPoint() ? makePointDisplay(getPlanContextHotelPoint()) : null;
        day.breakfast = null;
        day.lunch = null;
        day.dinner = null;
        ...
        if (breakfastPick && breakfastPick.restaurant) {
          usedRestaurantIds.add(breakfastPick.restaurant.id);
          previousGenre = breakfastPick.restaurant.genreKo || breakfastPick.restaurant.genreOriginal || previousGenre;
          day.breakfast = makePlanMeal(breakfastPick.restaurant, date, '숙소 출발 전후 아침 식사');
        }
        ...
        if (lunchPick && lunchPick.restaurant) {
          usedRestaurantIds.add(lunchPick.restaurant.id);
          previousGenre = lunchPick.restaurant.genreKo || lunchPick.restaurant.genreOriginal || previousGenre;
          day.lunch = makePlanMeal(lunchPick.restaurant, date, (morningPoint ? formatPointName(morningPoint) : '오전 동선') + ' 근처 점심');
        }
        ...
        if (dinnerPick && dinnerPick.restaurant) {
          usedRestaurantIds.add(dinnerPick.restaurant.id);
          previousGenre = dinnerPick.restaurant.genreKo || dinnerPick.restaurant.genreOriginal || previousGenre;
          day.dinner = makePlanMeal(dinnerPick.restaurant, date, (afternoonPoint ? formatPointName(afternoonPoint) : (morningPoint ? formatPointName(morningPoint) : '오후 동선')) + ' 근처 저녁');
        }

        saveJson(STORAGE.plan, state.plan);
        renderPlan();
        renderPlanSummary();
        if (state.planStory && Array.isArray(state.planStory.days) && state.planStory.days[dayIndex]) {
          var titleBits = [];
          if (morningPoint) titleBits.push(formatPointName(morningPoint));
          if (afternoonPoint && normalize(formatPointName(afternoonPoint)) !== normalize(formatPointName(morningPoint))) titleBits.push(formatPointName(afternoonPoint));
          state.planStory.days[dayIndex] = {
            ...
            title: titleBits.join(' → ') || ('Day ' + (dayIndex + 1)),
            why: titleBits.length > 1
              ? '오전에는 ' + titleBits[0] + ', 오후에는 ' + titleBits[1] + ' 쪽으로 다시 묶고 각 지점 근처 식당을 새로 골랐습니다.'
              : ((titleBits[0] || '해당 지역') + ' 중심으로 다시 추천했습니다.'),
          };
          setPlanStory(state.planStory);
        }
        if (els.planWarnings) {
          els.planWarnings.innerHTML = '<div class="message-box success">Day ' + escapeHtml(String(dayIndex + 1)) + ' 일정을 새로 추천했습니다.</div>';
        }
      });
    }).catch(function (error) {
      if (els.planWarnings) {
        els.planWarnings.innerHTML = '<div class="message-box info">' + escapeHtml(error && error.message ? error.message : '이 날짜를 다시 추천하지 못했습니다.') + '</div>';
      }
    }).finally(function () {
      if (button) {
        button.disabled = false;
        button.textContent = '이 날 다시 추천';
      }
    });
  }

  function pointFromMealV12(item) {
    if (!item || !Number.isFinite(Number(item.restaurantLat)) || !Number.isFinite(Number(item.restaurantLng))) return null;
    return {
      lat: Number(item.restaurantLat),
      lng: Number(item.restaurantLng),
      nameKo: item.restaurantName || '식당',
      nameOriginal: item.restaurantName || '',
    };
  }
  ...
  function renderRouteStep(label, origin, destination) {
    return '<a class="route-step" target="_blank" rel="noopener noreferrer" href="' + escapeHtml(buildPointDirectionsUrl(origin, destination)) + '">' +
      '<div class="route-step-main"><strong>' + escapeHtml(label) + '</strong><span>' + escapeHtml(formatPointName(origin)) + ' → ' + escapeHtml(formatPointName(destination)) + '</span></div>' +
      '<span class="route-step-action">Google Maps 길찾기</span>' +
    '</a>';
  }
  ...
  function renderPlanDayCard(day, dayNumber, date) {
    ...
    var breakfastPoint = pointFromMealV12(day.breakfast);
    var lunchPoint = pointFromMealV12(day.lunch);
    var dinnerPoint = pointFromMealV12(day.dinner);
    var morningVisit = day.visits && day.visits[0] ? day.visits[0] : null;
    var afternoonVisit = day.visits && day.visits[1] ? day.visits[1] : null;
    ...
    return '<article class="day-route-card">' +
      '<div class="day-route-head"><div><strong>Day ' + dayNumber + '</strong><span>' + escapeHtml(formatDisplayDate(date)) + '</span></div><div class="day-route-actions"><button class="small-button ghost" type="button" data-reroll-day="' + escapeHtml(String(dayNumber - 1)) + '">이 날 다시 추천</button></div></div>' +
      (hotel ? '<div class="route-chip hotel">숙소 · ' + escapeHtml(formatPointName(hotel)) + '</div>' : '') +
      (visitHtml ? '<div class="route-chip-row">' + visitHtml + '</div>' : '') +
      '<div class="route-meal-list">' +
        renderMealLine('아침', day.breakfast) +
        ...
        renderMealLine('저녁', day.dinner) +
      '</div>' +
      (steps.length ? '<div class="route-step-list">' + steps.join('') + '</div>' : '<div class="muted small">숙소 / 방문지 / 식당이 채워지면 구간별 <strong>Google Maps 길찾기</strong> 버튼이 표시됩니다.</div>') +
    '</article>';
  }
  ...
  function renderPlan() {
    ...
    if (els.planRouteCards) {
      els.planRouteCards.querySelectorAll('[data-reroll-day]').forEach(function (button) {
        button.addEventListener('click', function () {
          rerollPlanDayV12(parseInt(button.getAttribute('data-reroll-day') || '0', 10));
        });
      });
    }
  ...
  function renderPlanSummary() {
    if (!els.planSummary) return;
    var ctx = state.planContext || defaultPlanContextV9();
    var baseName = getPlanContextBasePoint() ? formatPointName(getPlanContextBasePoint()) : '기준 장소 미설정';
    var hotelName = getPlanContextHotelPoint() ? formatPointName(getPlanContextHotelPoint()) : '숙소 미설정';
    var filled = (state.plan || []).reduce(function (acc, day) {
      return acc + ['breakfast', 'lunch', 'dinner'].filter(function (slot) { return Boolean(day && day[slot]); }).length;
    }, 0);
    var total = getEffectivePlanDays() * 3;
    var incomplete = getIncompletePlanDayIndices();
    var regionLabel = getPlanDisplayRegionV12() || '지역 미설정';
    els.planSummary.innerHTML = '' +
      '<div class="plan-summary-box">' +
        '<strong>여행 지역 · ' + escapeHtml(regionLabel) + '</strong>' +
        '<span>총 ' + filled + ' / ' + total + '칸 채움</span>' +
      '</div>' +
      '<div class="plan-summary-box soft">' +
        '<strong>기준 장소 · ' + escapeHtml(baseName) + '</strong>' +
        '<span>' + escapeHtml((ctx.source === 'regional-auto' ? '지역 자동 코스' : '방문 후보 중심 코스') + ' · 시작일 ' + (ctx.startDate || toDateInput(new Date())) + ' · ' + getEffectivePlanDays() + '일') + '</span>' +
      '</div>' +
      '<div class="plan-summary-box soft hotel">' +
        '<strong>숙소 · ' + escapeHtml(hotelName) + '</strong>' +
        '<span>' + escapeHtml(getPlanContextHotelPoint() ? '숙소 기준 이동 동선을 함께 계산합니다.' : '숙소를 입력하면 숙소 ↔ 식당 ↔ 방문지 링크를 함께 보여드립니다.') + '</span>' +
      '</div>' +
      (incomplete.length ? '<div class="plan-summary-box action"><div><strong>빈 일정 보강 필요</strong><span>Day ' + escapeHtml(incomplete.join(', Day ')) + '이 아직 부족합니다.</span></div><button id="plan-summary-fill-button" class="btn btn-secondary" type="button">AI로 채우기</button></div>' : '');
    var summaryButton = document.getElementById('plan-summary-fill-button');
    if (summaryButton) summaryButton.addEventListener('click', function () { fillBlankDaysWithAi(); });
  ...
  function submitPlanOptionModal() {
    ...
    var mode = getSelectedPlanMode();
    var useAi = Boolean(els.planUseAi && els.planUseAi.checked && state.geminiAvailable);
    if (mode === 'full' && !query.region && !query.rawRegion) {
      if (els.planOptionMessage) els.planOptionMessage.textContent = '지역 자동 여행 코스는 여행 지역을 먼저 입력해 주세요.';
      return;
    }
    setPlanGenerating(true, '일정을 만드는 중입니다...');
  ...
  function generateNearbyPlacePlan(query, useAi) {
    var lines = parsePlaceLines(els.planExtraPlaces ? els.planExtraPlaces.value : '');
    var wantedFoods = parseWantedFoods();
    var regionCtx = getRegionalContextV12(query.rawRegion || query.region || '');
    var primaryRegion = query.region || regionCtx.parent || '';
    var focusRegion = query.focusRegion || regionCtx.focus || '';
    if (!lines.length && !primaryRegion && !focusRegion) {
      return Promise.reject(new Error('방문 후보 장소 또는 여행 지역을 입력해 주세요.'));
    }
    var seedLinePromises = lines.map(function (line) { return resolvePointLineForRegion(primaryRegion || focusRegion, line); });
    if (focusRegion) seedLinePromises.push(resolvePointLineForRegion(primaryRegion || focusRegion, focusRegion));
    return Promise.all(seedLinePromises).then(function (resolved) {
      var seedPoints = uniquePointsStrict(resolved.filter(Boolean).map(makePointDisplay));
      return persistCachedPlaces(seedPoints, { region: primaryRegion || focusRegion, source: 'user-input' }).then(function () { return seedPoints; });
    }).then(function (seedPoints) {
      var expansion = seedPoints.length
        ? Promise.resolve({ points: seedPoints, addedCount: 0, seedCount: seedPoints.length })
        : ensureRegionalSeed(focusRegion || primaryRegion).then(function (seed) { return { points: seed, addedCount: 0, seedCount: seed.length }; });
      return expansion;
    }).then(function (seedPayload) {
      if (!seedPayload.points.length) throw new Error('입력한 방문 후보를 기준점으로 읽지 못했습니다. 장소명이나 Google Maps 링크를 다시 확인해 주세요.');
      var targetCount = Math.max(query.days * 2, seedPayload.points.length || 1);
      return buildExpandedPointSetV12(seedPayload.points, primaryRegion, focusRegion, targetCount).then(function (expanded) {
        var finalPoints = uniquePointsStrict(expanded.points || []);
        return Promise.all(finalPoints.map(function (point) {
          return fetchCandidatesAroundPoint(point, getPrimaryFoodForGather(wantedFoods), Object.assign({}, query, { region: primaryRegion }));
        })).then(function (groups) {
          return fillPlanFromPointGroups(groups, {
            type: 'nearby-multi',
            useAi: useAi,
            query: Object.assign({}, query, { region: primaryRegion, focusRegion: focusRegion, regionDisplay: regionCtx.display || primaryRegion }),
            hotelPoint: null,
            overviewSeed: expanded.addedCount > 0
              ? '입력한 장소를 우선 배치하고, 부족한 시간대는 가까운 대표 장소를 추가해 일정 밀도를 높였습니다.'
              : '입력한 장소를 오전 / 오후 동선으로 나누고, 근처 식당과 숙소 동선을 함께 배치했습니다.',
      ...
      state.planContext.focusRegion = focusRegion || '';
      state.planContext.regionDisplay = regionCtx.display || primaryRegion || focusRegion || '';
      state.planContext.wantedFoods = wantedFoods;
      savePlanContextV9(state.planContext);
      return resolveBasePoint(primaryRegion || focusRegion, query.hotel, query.hotelMapsLink);
    }).then(function (hotelPoint) {
      if (hotelPoint) {
        state.planContext.hotelPoint = makePointDisplay(hotelPoint);
        state.plan.forEach(function (day) { day.hotel = state.planContext.hotelPoint; });
  ...
  function generateRegionalAutoPlan(query, useAi) {
    ...
    var primaryRegion = query.region || regionCtx.parent || '';
    var focusRegion = query.focusRegion || regionCtx.focus || '';
    var regionDisplay = regionCtx.display || primaryRegion || focusRegion || '';
    if (!primaryRegion && !focusRegion) {
      return Promise.reject(new Error('여행 지역을 먼저 입력해 주세요.'));
    }
    var resolveTasks = lines.map(function (line) { return resolvePointLineForRegion(primaryRegion || focusRegion, line); });
    if (focusRegion) resolveTasks.push(resolvePointLineForRegion(primaryRegion || focusRegion, focusRegion));
    return Promise.all([
      Promise.all(resolveTasks),
      ensureRegionalSeed(primaryRegion || focusRegion)
    ]).then(function (values) {
      var resolved = values[0];
      var regionSeed = values[1] || [];
      var regionCenter = regionSeed[0] || null;
      ...
      return persistCachedPlaces(seedPoints, { region: primaryRegion || focusRegion, source: 'user-input' }).then(function () {
        return { seedPoints: seedPoints, regionCenter: regionCenter };
      });
    }).then(function (seedPayload) {
      var targetCount = Math.max(query.days * 2, 2);
      return buildExpandedPointSetV12(seedPayload.seedPoints, primaryRegion, focusRegion, targetCount).then(function (expanded) {
        var finalPoints = uniquePointsStrict((expanded.points || []).filter(function (point) {
          return isPointInsideRegion(point, seedPayload.regionCenter);
        }));
        if (!finalPoints.length && seedPayload.regionCenter) finalPoints = [seedPayload.regionCenter];
        ...
        return Promise.all(finalPoints.map(function (point) {
          ...
          return fillPlanFromPointGroups(groups, {
            type: 'regional-auto',
            useAi: useAi,
            query: Object.assign({}, query, { region: primaryRegion, focusRegion: focusRegion, regionDisplay: regionDisplay }),
            hotelPoint: null,
            overviewSeed: (focusRegion ? (focusRegion + '를 중심으로 ' + primaryRegion + ' 안의 가까운 대표 장소를 이어 붙였습니다.') : (primaryRegion + ' 안에서만 대표 장소를 골라 오전 / 오후 방문지와 식당을 나눴습니다.')) + ' 다른 도시는 섞지 않았습니다.',
            wantedFoods: wantedFoods,
          });
        });
      });
      ...
      state.planContext.focusRegion = focusRegion || '';
      state.planContext.regionDisplay = regionDisplay;
      state.planContext.wantedFoods = wantedFoods;
      savePlanContextV9(state.planContext);
      return resolveBasePoint(primaryRegion || focusRegion, query.hotel, query.hotelMapsLink);
    }).then(function (hotelPoint) {
  ...
  function bootstrapV12() {
    PLACE_MAP['시부야'] = PLACE_MAP['시부야'] || {
      nameKo: '시부야 스크램블 교차로', nameOriginal: '渋谷スクランブル交差点', displayNameKo: '시부야 스크램블 교차로', displayNameOriginal: '渋谷スクランブル交差点', lat: 35.659494, lng: 139.700553
    };
    PLACE_MAP['신주쿠'] = PLACE_MAP['신주쿠'] || {
      nameKo: '도쿄도청 전망대', nameOriginal: '東京都庁展望室', displayNameKo: '신주쿠 도쿄도청 전망대', displayNameOriginal: '東京都庁展望室', lat: 35.689634, lng: 139.692101
    };
    PLACE_MAP['하라주쿠'] = PLACE_MAP['하라주쿠'] || {
      nameKo: '메이지 신궁', nameOriginal: '明治神宮', displayNameKo: '하라주쿠 메이지 신궁', displayNameOriginal: '明治神宮', lat: 35.676398, lng: 139.699326
    };
    PLACE_MAP['우메다'] = PLACE_MAP['우메다'] || {
      nameKo: '우메다 스카이 빌딩', nameOriginal: '梅田スカイビル', displayNameKo: '우메다 스카이 빌딩', displayNameOriginal: '梅田スカイビル', lat: 34.705497, lng: 135.489661
    };
    PLACE_MAP['텐진'] = PLACE_MAP['텐진'] || {
      nameKo: '텐진', nameOriginal: '天神', displayNameKo: '후쿠오카 텐진', displayNameOriginal: '天神', lat: 33.590167, lng: 130.401716
    };
    REGION_TRAVEL_SPOTS['시부야'] = REGION_TRAVEL_SPOTS['시부야'] || [
      { nameKo: '시부야 스크램블 교차로', nameOriginal: '渋谷スクランブル交差点', query: '東京都 渋谷スクランブル交差点', area: '시부야', priority: 100 },
      { nameKo: '시부야 스카이', nameOriginal: 'SHIBUYA SKY', query: '東京都 SHIBUYA SKY', area: '시부야', priority: 98 },
      { nameKo: '메이지 신궁', nameOriginal: '明治神宮', query: '東京都 明治神宮', area: '하라주쿠', priority: 94 },
      { nameKo: '하라주쿠 다케시타 거리', nameOriginal: '竹下通り', query: '東京都 竹下通り', area: '하라주쿠', priority: 92 },
      { nameKo: '오모테산도', nameOriginal: '表参道', query: '東京都 表参道', area: '오모테산도', priority: 90 },
      { nameKo: '에비스 가든 플레이스', nameOriginal: '恵比寿ガーデンプレイス', query: '東京都 恵比寿ガーデンプレイス', area: '에비스', priority: 86 },
    ];
    REGION_TRAVEL_SPOTS['신주쿠'] = REGION_TRAVEL_SPOTS['신주쿠'] || [
      { nameKo: '도쿄도청 전망대', nameOriginal: '東京都庁展望室', query: '東京都庁 展望室', area: '신주쿠', priority: 100 },
      { nameKo: '신주쿠 교엔', nameOriginal: '新宿御苑', query: '東京都 新宿御苑', area: '신주쿠', priority: 96 },
      { nameKo: '가부키초', nameOriginal: '歌舞伎町', query: '東京都 歌舞伎町', area: '신주쿠', priority: 92 },
      { nameKo: '오모이데 요코초', nameOriginal: '思い出横丁', query: '東京都 思い出横丁', area: '신주쿠', priority: 90 },
      { nameKo: '메이지 신궁', nameOriginal: '明治神宮', query: '東京都 明治神宮', area: '하라주쿠', priority: 86 },
      { nameKo: '시부야 스카이', nameOriginal: 'SHIBUYA SKY', query: '東京都 SHIBUYA SKY', area: '시부야', priority: 80 },
    ];
    REGION_TRAVEL_SPOTS['우메다'] = REGION_TRAVEL_SPOTS['우메다'] || [
      { nameKo: '우메다 스카이 빌딩', nameOriginal: '梅田スカイビル', query: '大阪 梅田スカイビル', area: '우메다', priority: 100 },
      { nameKo: '그랜드 프론트 오사카', nameOriginal: 'グランフロント大阪', query: '大阪 グランフロント大阪', area: '우메다', priority: 96 },
      { nameKo: '헵파이브 대관람차', nameOriginal: 'HEP FIVE', query: '大阪 HEP FIVE', area: '우메다', priority: 92 },
      { nameKo: '오사카역 시티', nameOriginal: '大阪ステーションシティ', query: '大阪ステーションシティ', area: '우메다', priority: 88 },
      { nameKo: '오사카성', nameOriginal: '大阪城', query: '大阪城', area: '오사카성', priority: 82 },
      { nameKo: '도톤보리', nameOriginal: '道頓堀', query: '大阪 道頓堀', area: '난바', priority: 78 },
    ];
    REGION_TRAVEL_SPOTS['텐진'] = REGION_TRAVEL_SPOTS['텐진'] || [
      { nameKo: '텐진 지하상가', nameOriginal: '天神地下街', query: '福岡 天神地下街', area: '텐진', priority: 100 },
      { nameKo: '후쿠오카 파르코', nameOriginal: '福岡PARCO', query: '福岡 PARCO', area: '텐진', priority: 96 },
      { nameKo: '나카스 야타이 거리', nameOriginal: '中洲屋台', query: '福岡 中洲屋台', area: '나카스', priority: 92 },
      { nameKo: '오호리 공원', nameOriginal: '大濠公園', query: '福岡 大濠公園', area: '오호리', priority: 88 },
      { nameKo: '캐널시티 하카타', nameOriginal: 'キャナルシティ博多', query: '福岡 キャナルシティ博多', area: '하카타', priority: 84 },
    ];
    Object.keys(REGION_PARENT_CITY_V12).forEach(function (area) {
      var parent = REGION_PARENT_CITY_V12[area];
      REGION_SEARCH_ALIASES[area] = unique((REGION_SEARCH_ALIASES[area] || []).concat([area, REGION_MAP[area] || '', parent, REGION_MAP[parent] || '']).filter(Boolean));
    });
  }

  function canonicalizePlanRegionInput(rawValue) {
    var raw = String(rawValue || '').trim();
    if (!raw) return { region: '', focusRegion: '', regionDisplay: '', regionInput: '' };
    var matchedArea = Object.keys(REGION_PARENT_CITY_V12).find(function (item) {
      var ni = normalize(item);
      var nr = normalize(raw);
      return ni === nr || ni.includes(nr) || nr.includes(ni);
    }) || '';
    var matchedRegion = Object.keys(REGION_TRAVEL_SPOTS).find(function (item) {
      var ni = normalize(item);
      var nr = normalize(raw);
      return ni === nr || ni.includes(nr) || nr.includes(ni);
    }) || '';
    var focusRegion = matchedArea || (matchedRegion && REGION_PARENT_CITY_V12[matchedRegion] ? matchedRegion : '');
    var parent = focusRegion ? REGION_PARENT_CITY_V12[focusRegion] : (matchedRegion || raw);
    return {
      region: parent || raw,
      focusRegion: focusRegion || '',
      regionDisplay: focusRegion && parent ? (focusRegion + ' · ' + parent) : (matchedRegion || raw),
      regionInput: raw,
    };
  }
  ...
  function readPlanModalQuery() {
    ...
    var normalized = canonicalizePlanRegionInput(base.region);
    base.region = normalized.region;
    base.focusRegion = normalized.focusRegion;
    base.regionDisplay = normalized.regionDisplay || normalized.region;
    base.regionInput = normalized.regionInput;
    return base;
  }

  function buildPlanContextQueryV12(query) {
    var ctx = state.planContext || defaultPlanContextV9();
    return Object.assign({}, state.currentQuery || {}, {
      region: (query && query.region) || ctx.region || '',
      regionDisplay: (query && query.regionDisplay) || ctx.regionDisplay || ctx.region || '',
      focusRegion: (query && query.focusRegion) || ctx.focusRegion || '',
      hotel: (query && query.hotel) || ctx.hotel || '',
      hotelMapsLink: (query && query.hotelMapsLink) || ctx.hotelMapsLink || '',
      startDate: (query && query.startDate) || ctx.startDate || toDateInput(new Date()),
      days: (query && query.days) || ctx.days || getEffectivePlanDays(),
      importedFromSearch: Boolean((query && query.importedFromSearch) || ctx.importedFromSearch),
      minScore: Number((query && query.minScore) || (els.minScoreInput && els.minScoreInput.value) || 3),
      sort: (query && query.sort) || 'food',
      food: '',
      mapsLink: '',
      base: ''
    });
  ...
  function ensureFocusSeed(query) {
    var focus = query && (query.focusRegion || query.regionInput) || '';
    if (!focus) return Promise.resolve([]);
    return resolvePointLineForRegion(query.region || focus, focus).then(function (point) {
      return point ? [makePointDisplay(point)] : [];
  ...
  function generateNearbyPlacePlan(query, useAi) {
    var lines = parsePlaceLines(els.planExtraPlaces ? els.planExtraPlaces.value : '');
    var wantedFoods = parseWantedFoods();
    if (!lines.length && !query.region && !query.focusRegion) {
      return Promise.reject(new Error('방문 후보 장소 또는 여행 지역을 입력해 주세요.'));
    }
    if (!lines.length && query.focusRegion) lines = [query.focusRegion];
    return Promise.all(lines.map(function (line) { return resolvePointLineForRegion(query.region || query.focusRegion, line); })).then(function (resolved) {
      var seedPoints = uniquePointsStrict(resolved.filter(Boolean).map(makePointDisplay));
      return persistCachedPlaces(seedPoints, { region: query.region || query.focusRegion || '', source: 'user-input' }).then(function () { return seedPoints; });
    }).then(function (seedPoints) {
      var expansion = seedPoints.length
        ? Promise.resolve({ points: seedPoints, addedCount: 0, seedCount: seedPoints.length })
        : ensureRegionalSeed(query.focusRegion || query.region).then(function (seed) { return { points: seed, addedCount: 0, seedCount: seed.length }; });
      return expansion;
    }).then(function (seedPayload) {
      if (!seedPayload.points.length) throw new Error('입력한 방문 후보를 기준점으로 읽지 못했습니다. 장소명이나 Google Maps 링크를 다시 확인해 주세요.');
      var targetCount = Math.max(query.days * 2, seedPayload.points.length);
      return buildExpandedPointSet(seedPayload.points, query.region || query.focusRegion, targetCount).then(function (expanded) {
        var finalPoints = uniquePointsStrict(expanded.points || []);
        return Promise.all(finalPoints.map(function (point) {
          return fetchCandidatesAroundPoint(point, getPrimaryFoodForGather(wantedFoods), query);
        })).then(function (groups) {
          return fillPlanFromPointGroups(groups, {
            ...
            hotelPoint: null,
            overviewSeed: expanded.addedCount > 0
              ? '입력한 장소를 우선 배치하고, 부족한 시간대는 ' + (query.regionDisplay || query.region || '해당 지역') + '의 가까운 대표 장소를 추가했습니다.'
              : '입력한 장소를 오전 / 오후 동선으로 나누고, 근처 식당과 숙소 동선을 함께 배치했습니다.',
            wantedFoods: wantedFoods,
          });
        });
      ...
      return resolveBasePoint(query.region || query.focusRegion, query.hotel, query.hotelMapsLink);
    }).then(function (hotelPoint) {
      if (hotelPoint) {
        state.planContext.hotelPoint = makePointDisplay(hotelPoint);
        state.plan.forEach(function (day) { day.hotel = state.planContext.hotelPoint; });
  ...
  function generateRegionalAutoPlan(query, useAi) {
    ...
    if (!lines.length && query.focusRegion) lines = [query.focusRegion];
    return Promise.all([
      Promise.all(lines.map(function (line) { return resolvePointLineForRegion(query.region || query.focusRegion, line); })),
      ensureRegionalSeed(query.focusRegion || query.region)
    ]).then(function (values) {
      var resolved = values[0];
      var regionSeed = values[1] || [];
      var regionCenter = regionSeed[0] || null;
      ...
      return persistCachedPlaces(seedPoints, { region: query.region || query.focusRegion || '', source: 'user-input' }).then(function () {
        return { seedPoints: seedPoints, regionCenter: regionCenter };
      });
    }).then(function (seedPayload) {
      var targetCount = Math.max(query.days * 2, 2);
      return buildExpandedPointSet(seedPayload.seedPoints, query.region || query.focusRegion, targetCount).then(function (expanded) {
        var finalPoints = uniquePointsStrict((expanded.points || []).filter(function (point) {
          return isPointInsideRegion(point, seedPayload.regionCenter);
        }));
        if (!finalPoints.length && seedPayload.regionCenter) finalPoints = [seedPayload.regionCenter];
        ...
        return Promise.all(finalPoints.map(function (point) {
          ...
          return fillPlanFromPointGroups(groups, {
            ...
            query: query,
            hotelPoint: null,
            overviewSeed: (query.regionDisplay || query.region || '입력한 지역') + ' 안에서만 대표 장소를 골라 오전 / 오후 방문지와 식당을 나눴습니다. 다른 도시는 섞지 않았습니다.',
            wantedFoods: wantedFoods,
          });
        });
      });
  ...
  function fillPlanFromPointGroups(pointGroups, options) {
    options = options || {};
    var query = buildPlanContextQueryV12(options.query || {});
    var days = clamp(parseInt(query.days || '3', 10), 1, 10);
    var tripStart = new Date((query.startDate || toDateInput(new Date())) + 'T00:00:00');
    var groups = (pointGroups || []).filter(function (group) { return group && group.point; });
    if (!groups.length) throw new Error('추천 일정에 쓸 기준 장소를 만들지 못했습니다.');
    ...
    state.plan = createPlan(days);
    state.planPool = mergeById(state.planPool || [], allRestaurants);
    state.lastPlanQuery = Object.assign({}, query);
    state.planContext = savePlanContextV9({
      region: query.region || '',
      regionDisplay: query.regionDisplay || query.region || '',
  ...
  function renderRouteStep(label, origin, destination) {
    if (!origin || !destination) return '';
    return '<a class="route-step route-step-map" target="_blank" rel="noopener noreferrer" href="' + escapeHtml(buildPointDirectionsUrl(origin, destination)) + '">' +
      '<div class="route-step-top"><strong>' + escapeHtml(label) + '</strong><span class="route-step-action">Google Maps 열기</span></div>' +
      '<span class="route-step-path">' + escapeHtml(formatPointName(origin)) + ' → ' + escapeHtml(formatPointName(destination)) + '</span></a>';
  }
  ...
  function renderPlanDayCard(day, dayNumber, date) {
    ...
    var steps = [];
    var hotel = day.hotel || getPlanContextHotelPoint() || null;
    var breakfastPoint = mealToPoint(day.breakfast);
    var lunchPoint = mealToPoint(day.lunch);
    var dinnerPoint = mealToPoint(day.dinner);
    var morningVisit = day.visits && day.visits[0] ? day.visits[0] : null;
    var afternoonVisit = day.visits && day.visits[1] ? day.visits[1] : null;

    if (hotel && breakfastPoint) steps.push(renderRouteStep('숙소에서 아침 식당으로', hotel, breakfastPoint));
    if (breakfastPoint && morningVisit) steps.push(renderRouteStep('아침 식당에서 오전 방문지로', breakfastPoint, morningVisit));
    if (morningVisit && lunchPoint) steps.push(renderRouteStep('오전 방문지에서 점심 식당으로', morningVisit, lunchPoint));
    if (lunchPoint && afternoonVisit) steps.push(renderRouteStep('점심 식당에서 오후 방문지로', lunchPoint, afternoonVisit));
    if (afternoonVisit && dinnerPoint) steps.push(renderRouteStep('오후 방문지에서 저녁 식당으로', afternoonVisit, dinnerPoint));
    if (dinnerPoint && hotel) steps.push(renderRouteStep('저녁 식당에서 숙소로', dinnerPoint, hotel));

    return '<article class="day-route-card">' +
      '<div class="day-route-head"><div><strong>Day ' + dayNumber + '</strong><span>' + escapeHtml(formatDisplayDate(date)) + '</span></div><div class="route-head-actions"><button class="small-button refresh" type="button" data-reroll-day="' + (dayNumber - 1) + '">이 날 새로 추천</button></div></div>' +
      (hotel ? '<div class="route-chip hotel">숙소 · ' + escapeHtml(formatPointName(hotel)) + '</div>' : '') +
      (visitHtml ? '<div class="route-chip-row">' + visitHtml + '</div>' : '') +
      '<div class="route-meal-list">' +
        renderMealLine('아침', day.breakfast) +
        ...
        renderMealLine('저녁', day.dinner) +
      '</div>' +
      (steps.length ? '<div class="route-step-list">' + steps.join('') + '</div>' : '<div class="muted small">숙소나 방문지를 입력하면 구간별 Google Maps 길찾기 버튼이 함께 보입니다.</div>') +
    '</article>';
  }

  function getUsedRestaurantIdsExceptDay(dayIndex) {
    var ids = new Set();
    (state.plan || []).forEach(function (day, idx) {
      if (idx === dayIndex) return;
      ['breakfast', 'lunch', 'dinner'].forEach(function (slot) {
        var item = day && day[slot];
        if (item && item.restaurantId) ids.add(item.restaurantId);
      });
    });
    return ids;
  }

  function suggestUnusedVisitsForDay(dayIndex, query, needed) {
    var usedKeys = {};
    (state.plan || []).forEach(function (day, idx) {
      if (idx === dayIndex) return;
      (day.visits || []).forEach(function (point) { usedKeys[pointKey(point)] = true; });
    });
    return buildExpandedPointSet([], query.region || query.focusRegion, Math.max((query.days || getEffectivePlanDays()) * 2 + 4, 8)).then(function (expanded) {
      return (expanded.points || []).filter(function (point) {
        return point && !usedKeys[pointKey(point)];
      }).slice(0, needed || 2);
    });
  }

  function rerollPlanDay(dayIndex) {
    var ctx = state.planContext || defaultPlanContextV9();
    var query = buildPlanContextQueryV12(state.lastPlanQuery || ctx || {});
    var day = state.plan[dayIndex];
    if (!day) return Promise.resolve();
    var currentVisits = uniquePointsStrict(day.visits || []);
    var wantedFoods = Array.isArray(ctx.wantedFoods) ? ctx.wantedFoods.slice() : [];
    var date = addDays(getEffectiveTripStartDate(), dayIndex);
    var usedRestaurantIds = getUsedRestaurantIdsExceptDay(dayIndex);
    var previousGenre = '';
    (state.plan || []).forEach(function (row, idx) {
      if (idx >= dayIndex) return;
      ['breakfast', 'lunch', 'dinner'].forEach(function (slot) {
        var item = row && row[slot];
        if (item && item.restaurantType) previousGenre = item.restaurantType;
      });
    });
    if (els.planWarnings) els.planWarnings.innerHTML = '<div class="message-box info">Day ' + (dayIndex + 1) + ' 일정을 다시 추천하는 중입니다...</div>';

    return suggestUnusedVisitsForDay(dayIndex, query, 4).then(function (extraPoints) {
      var visits = uniquePointsStrict(extraPoints.concat(currentVisits)).slice(0, Math.max(1, Math.min(2, extraPoints.length ? 2 : currentVisits.length || 1)));
      if (!visits.length && query.focusRegion) {
        return ensureRegionalSeed(query.focusRegion).then(function (seed) {
          return uniquePointsStrict((seed || []).concat(extraPoints).concat(currentVisits)).slice(0, 2);
        });
      }
      return visits;
    }).then(function (visits) {
      day.visits = uniquePointsStrict(visits || []).slice(0, 2);
      if (!day.visits.length) throw new Error('새로 추천할 방문지를 찾지 못했습니다.');
      return Promise.all(day.visits.map(function (point) {
        return fetchCandidatesAroundPoint(point, getPrimaryFoodForGather(wantedFoods), query);
      }));
    }).then(function (groups) {
      var allRestaurants = mergeById(state.planPool || [], getMergedGroupResults(groups));
      state.planPool = allRestaurants;
      var morningGroup = groups[0] || null;
      var afternoonGroup = groups[1] || null;
      var morningPoint = morningGroup ? morningGroup.point : null;
      var afternoonPoint = afternoonGroup ? afternoonGroup.point : morningPoint;
      var globalPool = allRestaurants.map(function (item) { return augmentRestaurant(item, morningPoint || getPlanContextBasePoint(), query.food || ''); });
      var breakfastSource = getPlanContextHotelPoint() ? globalPool : ((morningGroup && morningGroup.results && morningGroup.results.length) ? morningGroup.results : globalPool);
      var lunchSource = (morningGroup && morningGroup.results && morningGroup.results.length) ? morningGroup.results : globalPool;
      var dinnerSource = (afternoonGroup && afternoonGroup.results && afternoonGroup.results.length)
      ...
      day.hotel = getPlanContextHotelPoint() ? makePointDisplay(getPlanContextHotelPoint()) : day.hotel || null;
      var breakfastPick = chooseRestaurantForSlot(breakfastSource, 'breakfast', date, previousGenre, usedRestaurantIds, '');
      if (breakfastPick && breakfastPick.restaurant) {
        usedRestaurantIds.add(breakfastPick.restaurant.id);
        previousGenre = breakfastPick.restaurant.genreKo || breakfastPick.restaurant.genreOriginal || previousGenre;
  ...
  function renderPlanSummary() {
    ...
    var total = getEffectivePlanDays() * 3;
    var ctx = state.planContext || defaultPlanContextV9();
    var regionLabel = ctx.regionDisplay || ctx.region || '';
    var baseName = ctx.basePoint ? formatPointName(ctx.basePoint) : (regionLabel ? regionLabel + ' 기준 자동 추천' : '기준 장소 미설정');
    var hotelName = ctx.hotelPoint ? formatPointName(ctx.hotelPoint) : (ctx.hotel || '숙소 미설정');
    var incomplete = getIncompletePlanDayIndices();
    els.planSummary.innerHTML = '' +
      '<div class="plan-summary-box">' +
