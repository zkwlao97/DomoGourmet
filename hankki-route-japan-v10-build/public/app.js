...
function syncModalOpenState() {
    var hasOpenModal = Array.prototype.some.call(document.querySelectorAll('.modal'), function(modal) {
...
function showModal(node) {
  ...
  function detectApi() {
    return fetch('/api/health').then(function (response) {
      ...
      if (state.forceDemo) {
        ...
        els.modeBadge.textContent = '실시간 모드';
        var aiText = state.geminiAvailable ? (' Gemini는 ' + (state.geminiModels || []).join(' → ') + ' 순서로 자동 시도합니다.') : '';
        els.apiStatus.textContent = 'Hot Pepper 실시간 검색이 연결되었습니다.' + aiText;
      } else {
        els.modeBadge.textContent = '데모 모드';
        els.apiStatus.textContent = 'API 키가 없어 데모 데이터로 동작합니다.';
      }
  ...
  function findRegionCatalog(region) {
    ...
    if (exactKey) return REGION_TRAVEL_SPOTS[exactKey];
    var partialKey = Object.keys(REGION_TRAVEL_SPOTS).find(function (item) { return normalized && (normalize(item).includes(normalized) || normalized.includes(normalize(item))); });
    return partialKey ? REGION_TRAVEL_SPOTS[partialKey] : [];
  }
  ...
  function generateNearbyPlacePlan(query, useAi) {
    ...
    return Promise.all(lines.map(function (line) { return resolvePointLineForRegion(query.region, line); })).then(function (resolved) {
      ...
      return Promise.resolve(seedPoints);
