...
// ===== v37 auth/theme/status/layout patch =====
function updateCurrentUserBadgeV37() {
  var badge = document.getElementById('current-user-badge');
  if (!badge) return;
  if (state.currentUser) {
    badge.textContent = state.currentUser.role === 'admin' ? '관리자' : (state.currentUser.nickname || state.currentUser.username || state.currentUser.email || '내 계정');
...
renderAdminState = function () {
  var out = __v37_renderAdminState.apply(this, arguments);
  updateCurrentUserBadgeV37();
  if (els.adminLoginOpen && state.currentUser) {
    els.adminLoginOpen.classList.add('hidden');
  } else if (els.adminLoginOpen) {
    els.adminLoginOpen.classList.remove('hidden');
  }
  return out;
};

function updateVisibleStatusBadgesV37FromHealth(json) {
  var hp = document.querySelector('.status-badge[data-service="hotpepper"] .status-state');
  var gm = document.querySelector('.status-badge[data-service="gemini"] .status-state');
  if (hp) {
    var hpText = json && json.hotpepperConfigured ? '연결됨' : ((json && json.mode === 'demo-only') ? '데모' : '미연결');
    hp.textContent = hpText;
    hp.className = 'status-state ' + (hpText === '연결됨' ? 'on' : (hpText === '데모' ? 'demo' : 'off'));
  }
  if (gm) {
    var gmText = json && json.geminiConfigured ? '연결됨' : '미연결';
    gm.textContent = gmText;
    gm.className = 'status-state ' + (gmText === '연결됨' ? 'on' : 'off');
  }
}

function refreshServiceHealthV37() {
  return fetch('/api/health', { cache: 'no-store' })
    .then(function (response) { if (!response.ok) throw new Error('health_failed'); return response.json(); })
    .then(function (json) {
      state.apiAvailable = true;
      state.liveEnabled = Boolean(json.hotpepperConfigured);
      state.geminiAvailable = Boolean(json.geminiConfigured);
      updateVisibleStatusBadgesV37FromHealth(json);
      updateCurrentUserBadgeV37();
      return json;
    })
    .catch(function () {
      updateVisibleStatusBadgesV37FromHealth({ hotpepperConfigured: false, geminiConfigured: false, mode: 'demo-only' });
    });
}

function bindQuickThemeChipsV37() {
  document.querySelectorAll('.theme-quick-chip').forEach(function (button) {
    if (button.dataset.v37Bound) return;
    button.dataset.v37Bound = '1';
    button.addEventListener('click', function () {
      button.classList.toggle('active');
      var selected = Array.from(document.querySelectorAll('.theme-quick-chip.active')).map(function (node) {
        return node.getAttribute('data-theme') || '';
      }).filter(Boolean);
      state.currentQuery.quickThemes = selected;
      saveJson(STORAGE.query, state.currentQuery);
    });
  });
}

function setSearchLoadingV37(active, label) {
  var row = document.getElementById('search-loading');
  if (row) row.classList.toggle('hidden', !active);
  if (els.searchButton) {
    els.searchButton.disabled = !!active;
    els.searchButton.textContent = active ? (label || '검색 중...') : '맛집 검색';
  }
}

var __v37_search = search;
search = function (options) {
  setSearchLoadingV37(true, '검색 중...');
  return Promise.resolve(__v37_search(options)).finally(function () {
    setSearchLoadingV37(false);
  });
};
...
function setAuthModeV37(mode) {
  var hidden = document.getElementById('account-auth-mode');
  var nicknameRow = document.getElementById('account-nickname-row');
  var emailRow = document.getElementById('account-email-row');
  var usernameRow = document.getElementById('account-username-row');
  var loginTab = document.getElementById('account-mode-login');
  var signupTab = document.getElementById('account-mode-signup');
  if (hidden) hidden.value = mode;
  if (nicknameRow) nicknameRow.classList.toggle('hidden', mode !== 'signup');
  if (emailRow) emailRow.classList.toggle('hidden', mode !== 'signup');
  if (usernameRow) usernameRow.classList.toggle('hidden', mode !== 'signup');
  if (loginTab) loginTab.classList.toggle('active', mode === 'login');
  if (signupTab) signupTab.classList.toggle('active', mode === 'signup');
  if (els.adminLoginSubmit) els.adminLoginSubmit.textContent = mode === 'signup' ? '회원가입' : '로그인';
  if (els.adminLoginMessage) els.adminLoginMessage.textContent = mode === 'signup' ? '이메일, 아이디, 비밀번호를 입력해 주세요.' : '';
}

function submitAuthV37() {
  var mode = ((document.getElementById('account-auth-mode') || {}).value || 'login');
  var loginIdentifier = String((document.getElementById('admin-id-input') || {}).value || '').trim();
  var signupEmail = String((document.getElementById('account-email-input') || {}).value || '').trim();
  var signupUsername = String((document.getElementById('account-username-input') || {}).value || '').trim();
  var password = String((document.getElementById('admin-password-input') || {}).value || '').trim();
  var nickname = String((document.getElementById('account-nickname-input') || {}).value || '').trim();
  var message = document.getElementById('admin-login-message');
  var submitBtn = document.getElementById('admin-login-submit');
  if (mode === 'signup') {
    if (!signupEmail || !signupUsername || !password) {
      if (message) message.textContent = '이메일, 아이디, 비밀번호를 입력해 주세요.';
      return;
    }
    if (!nickname) nickname = signupUsername || signupEmail.split('@')[0] || '여행자';
  } else {
  ...
  var endpoint = mode === 'signup' ? '/api/auth/signup' : '/api/auth/login';
  var payload = mode === 'signup'
    ? { email: signupEmail, username: signupUsername, password: password, nickname: nickname }
...
function bindAuthUiV37() {
  var loginTab = document.getElementById('account-mode-login');
  var signupTab = document.getElementById('account-mode-signup');
  var submitBtn = document.getElementById('admin-login-submit');
  var cancelBtn = document.getElementById('admin-login-cancel');
  if (loginTab) loginTab.onclick = function () { setAuthModeV37('login'); };
  if (signupTab) signupTab.onclick = function () { setAuthModeV37('signup'); };
  if (submitBtn) submitBtn.onclick = submitAuthV37;
  if (cancelBtn) cancelBtn.onclick = closeAdminModal;
  ['admin-id-input','admin-password-input','account-email-input','account-username-input','account-nickname-input'].forEach(function (id) {
...
function sanitizeQuickThemeChipBindingsV37() {
  document.querySelectorAll('.theme-quick-chip').forEach(function (button) {
    var clone = button.cloneNode(true);
    button.parentNode.replaceChild(clone, button);
  });
  bindQuickThemeChipsV37();
}

function refineCommunityLayoutV37() {
  var input = document.getElementById('community-themes');
  if (input) input.type = 'hidden';
  ['theme-place-themes','theme-template-themes','content-themes','content-category'].forEach(function(id){
    var node = document.getElementById(id);
    if (node) node.type = 'hidden';
  });
}

function clampResultTextV37() {
  document.querySelectorAll('.result-title').forEach(function(node){
...
var __v37_renderCommunityView = renderCommunityView;
renderCommunityView = function () {
  var out = __v37_renderCommunityView.apply(this, arguments);
  refineCommunityLayoutV37();
  return out;
};
...
document.addEventListener('DOMContentLoaded', function () {
  setTimeout(function () {
    ...
    sanitizeQuickThemeChipBindingsV37();
    refineCommunityLayoutV37();
...
// ===== v38 full package auth/status/layout patch =====
function v38SetBadgeState(service, text) {
  var badge = document.querySelector('.status-badge[data-service="' + service + '"]');
  if (!badge) return;
  var dot = badge.querySelector('.status-dot');
  var stateNode = badge.querySelector('.status-state');
  if (stateNode) {
    stateNode.textContent = text;
    stateNode.classList.remove('on', 'off', 'demo');
    stateNode.classList.add(text === '연결됨' ? 'on' : (text === '데모' ? 'demo' : 'off'));
  }
  if (dot) {
    dot.classList.remove('on', 'off');
    dot.classList.add(text === '연결됨' ? 'on' : 'off');
  }
  badge.title = service + ' ' + text;
}

function refreshServiceHealthV38() {
  return fetch('/api/health', { cache: 'no-store' })
    .then(function (response) { return response.json(); })
    .then(function (json) {
      state.liveEnabled = Boolean(json.hotpepperConfigured);
      state.geminiAvailable = Boolean(json.geminiConfigured);
      v38SetBadgeState('hotpepper', json && json.hotpepperConfigured ? '연결됨' : ((json && json.mode === 'demo-only') ? '데모' : '미연결'));
      v38SetBadgeState('gemini', json && json.geminiConfigured ? '연결됨' : '미연결');
      return json;
    })
    .catch(function () {
      v38SetBadgeState('hotpepper', '미연결');
      v38SetBadgeState('gemini', '미연결');
    });
}

function replaceNodeKeepId(id) {
  var node = document.getElementById(id);
...
function setAuthModeV38(mode) {
  var hidden = document.getElementById('account-auth-mode');
  if (hidden) hidden.value = mode;
  var loginRow = document.querySelector('label[for="admin-id-input"]') || document.getElementById('admin-id-input')?.closest('.form-label');
  var passwordRow = document.getElementById('admin-password-input')?.closest('.form-label');
  var emailRow = document.getElementById('account-email-row');
  var usernameRow = document.getElementById('account-username-row');
  var nicknameRow = document.getElementById('account-nickname-row');
  var loginTab = document.getElementById('account-mode-login');
  ...
  var submitBtn = document.getElementById('admin-login-submit');
  var hint = document.querySelector('.auth-hint');
  if (loginRow) loginRow.classList.toggle('hidden', mode === 'signup');
  ...
  if (loginTab) loginTab.classList.toggle('active', mode === 'login');
  if (signupTab) signupTab.classList.toggle('active', mode === 'signup');
  if (submitBtn) submitBtn.textContent = mode === 'signup' ? '회원가입' : '로그인';
  if (hint) hint.textContent = mode === 'signup' ? '이메일, 아이디, 비밀번호를 입력해 계정을 생성합니다.' : '로그인 후 저장한 일정과 커뮤니티 글을 관리할 수 있습니다.';
}

function submitAuthV38() {
  var mode = ((document.getElementById('account-auth-mode') || {}).value || 'login');
  var loginIdentifier = String((document.getElementById('admin-id-input') || {}).value || '').trim();
  var signupEmail = String((document.getElementById('account-email-input') || {}).value || '').trim();
  var signupUsername = String((document.getElementById('account-username-input') || {}).value || '').trim();
  ...
  if (mode === 'signup') {
    ...
    if (!nickname) nickname = signupUsername || signupEmail.split('@')[0] || '여행자';
    payload = { email: signupEmail, username: signupUsername, nickname: nickname, password: password };
  } else {
    if (!loginIdentifier || !password) {
      if (message) message.textContent = '이메일 또는 아이디와 비밀번호를 입력해 주세요.';
      return;
    }
    payload = { email: loginIdentifier, password: password };
  }
  if (submitBtn) {
    submitBtn.disabled = true;
    submitBtn.textContent = mode === 'signup' ? '가입 중...' : '로그인 중...';
  ...
  fetch(mode === 'signup' ? '/api/auth/signup' : '/api/auth/login', {
    ...
    .then(function (response) { return response.json().then(function (json) { if (!response.ok) throw new Error(json && json.message ? json.message : '요청에 실패했습니다.'); return json; }); })
    .then(function (json) {
      if (!json || !json.user) throw new Error(mode === 'signup' ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.');
      state.currentUser = json.user;
      saveJson(STORAGE.currentUser, state.currentUser);
      if (message) message.textContent = mode === 'signup' ? '회원가입이 완료되었습니다.' : '로그인되었습니다.';
      renderAdminState();
      updateCurrentUserBadgeV37();
      refreshServiceHealthV38();
      setTimeout(function(){ closeAdminModal(); }, 250);
    })
    .catch(function (error) {
      if (message) message.textContent = error && error.message ? error.message : (mode === 'signup' ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.');
...
function bindAuthUiV38() {
  var loginTab = replaceNodeKeepId('account-mode-login');
  var signupTab = replaceNodeKeepId('account-mode-signup');
  var submitBtn = replaceNodeKeepId('admin-login-submit');
  var cancelBtn = replaceNodeKeepId('admin-login-cancel');
  if (loginTab) loginTab.addEventListener('click', function (event) { event.preventDefault(); setAuthModeV38('login'); });
  if (signupTab) signupTab.addEventListener('click', function (event) { event.preventDefault(); setAuthModeV38('signup'); });
  if (submitBtn) submitBtn.addEventListener('click', function (event) { event.preventDefault(); submitAuthV38(); });
  if (cancelBtn) cancelBtn.addEventListener('click', function (event) { event.preventDefault(); closeAdminModal(); });
  ['admin-id-input','admin-password-input','account-email-input','account-username-input','account-nickname-input'].forEach(function (id) {
    var node = document.getElementById(id);
    if (!node) return;
    node.onkeydown = function (event) {
  ...
  setAuthModeV38('login');
}

function rebindQuickThemeChipsV38() {
  document.querySelectorAll('.theme-quick-chip').forEach(function (button) {
    var clone = button.cloneNode(true);
    button.parentNode.replaceChild(clone, button);
...
function applyResultGridFixV38() {
  var resultLists = document.querySelectorAll('.result-section-list, .results-grid');
  resultLists.forEach(function (list) {
    list.style.display = 'grid';
  });
}

function bindSearchLoadingV38() {
  var searchButton = document.getElementById('search-button');
  if (!searchButton || searchButton.dataset.v38Bound) return;
  searchButton.dataset.v38Bound = '1';
  var original = search;
  search = function (options) {
    setSearchLoadingV37(true, '검색 중...');
    return Promise.resolve(original(options)).finally(function () {
      setSearchLoadingV37(false, '맛집 검색');
      applyResultGridFixV38();
    });
  };
}

function hideBottomDockNearFooterV38() {
  var dock = document.querySelector('.bottom-dock, .mobile-dock, .dock-bar');
  if (!dock) return;
  var onScroll = function () {
    var nearBottom = window.innerHeight + window.scrollY >= document.body.offsetHeight - 120;
    dock.classList.toggle('hidden-near-footer', nearBottom);
  };
...
document.addEventListener('DOMContentLoaded', function () {
  setTimeout(function () {
    bindAuthUiV38();
    rebindQuickThemeChipsV38();
    refreshServiceHealthV38();
    bindSearchLoadingV38();
    applyResultGridFixV38();
    hideBottomDockNearFooterV38();
  }, 30);
});
...
/* ===== v39 standalone final override ===== */
(function(){
  'use strict';
  var USER_SESSION_KEY = 'domogourmet.userSession.v1';
  var QUICK_THEME_KEY = 'domogourmet.quickThemes.v1';
  function $(id){ return document.getElementById(id); }
  function by(sel,root){ return (root||document).querySelector(sel); }
  function byAll(sel,root){ return Array.prototype.slice.call((root||document).querySelectorAll(sel)); }
  function replaceNode(id){ var node=$(id); if(!node||!node.parentNode) return node; var clone=node.cloneNode(true); node.parentNode.replaceChild(clone,node); return clone; }
  ...
  function getUser(){ return readJson(USER_SESSION_KEY, null); }
  function setUser(user){ writeJson(USER_SESSION_KEY, user || null); }
  function renderHeaderAccount(){
    var user=getUser();
    var badge=$('current-user-badge');
    var loginBtn=$('admin-login-open');
    var logoutBtn=$('admin-logout-button');
    if(badge){
      if(user){
        badge.textContent = user.role === 'admin' ? '관리자' : (user.nickname || user.username || user.email || '내 계정');
        badge.classList.remove('hidden');
      } else {
        badge.textContent='';
        badge.classList.add('hidden');
    ...
    var nickInput=$('community-nickname');
    if(nickInput && user){ nickInput.value = user.nickname || user.username || user.email || ''; }
  }
  function setServiceBadge(service, text){
    var badge = by('.status-badge[data-service="'+service+'"]');
    if(!badge) return;
    var state = by('.status-state', badge);
    var dot = by('.status-dot', badge);
    if(state){ state.textContent=text; state.classList.remove('on','off','demo'); state.classList.add(text==='연결됨'?'on':(text==='데모'?'demo':'off')); }
    if(dot){ dot.classList.remove('on','off'); dot.classList.add(text==='연결됨'?'on':'off'); }
    badge.title = service + ' ' + text;
  }
  function refreshHealth(){
    return fetch('/api/health',{cache:'no-store'}).then(function(res){ if(!res.ok) throw new Error('health_failed'); return res.json(); }).then(function(json){
      setServiceBadge('hotpepper', json && json.hotpepperConfigured ? '연결됨' : ((json && /demo/i.test(String(json.mode||''))) ? '데모' : '미연결'));
      setServiceBadge('gemini', json && json.geminiConfigured ? '연결됨' : '미연결');
      return json;
    }).catch(function(){
      setServiceBadge('hotpepper','미연결');
      setServiceBadge('gemini','미연결');
    });
  }
  function setAuthMode(mode){
    var hidden=$('account-auth-mode'); if(hidden) hidden.value=mode;
    var loginRow=$('account-login-row') || ($('admin-id-input') && $('admin-id-input').closest('.form-label'));
    var emailRow=$('account-email-row');
    var usernameRow=$('account-username-row');
    var nickRow=$('account-nickname-row');
    var loginTab=$('account-mode-login');
    var signupTab=$('account-mode-signup');
    var submitBtn=$('admin-login-submit');
    var message=$('admin-login-message');
    var hint = by('.auth-hint');
    setHidden(loginRow, mode==='signup');
    setHidden(emailRow, mode!=='signup');
    setHidden(usernameRow, mode!=='signup');
    setHidden(nickRow, mode!=='signup');
    if(loginTab) loginTab.classList.toggle('active', mode==='login');
    if(signupTab) signupTab.classList.toggle('active', mode==='signup');
    if(submitBtn) submitBtn.textContent = mode==='signup' ? '회원가입' : '로그인';
    if(message) message.textContent = '';
    if(hint) hint.textContent = mode==='signup' ? '이메일, 아이디, 닉네임, 비밀번호를 입력해 계정을 생성합니다.' : '로그인하면 내 일정과 커뮤니티 글을 관리하고 저장한 추천 일정을 바로 담을 수 있습니다.';
  }
  function openAuthModal(mode){
    var modal=$('admin-login-modal');
    if(!modal) return;
    setAuthMode(mode||'login');
    modal.classList.remove('hidden');
    modal.setAttribute('aria-hidden','false');
  }
  function closeAuthModal(){ var modal=$('admin-login-modal'); if(modal){ modal.classList.add('hidden'); modal.setAttribute('aria-hidden','true'); } }
  function submitAuth(){
    var mode=(($('account-auth-mode')||{}).value||'login');
    var loginIdentifier=String(($('admin-id-input')||{}).value||'').trim();
    var signupEmail=String(($('account-email-input')||{}).value||'').trim();
    var signupUsername=String(($('account-username-input')||{}).value||'').trim();
    var nickname=String(($('account-nickname-input')||{}).value||'').trim();
    var password=String(($('admin-password-input')||{}).value||'').trim();
    var message=$('admin-login-message');
    var submitBtn=$('admin-login-submit');
    if(message) message.textContent='';
    ...
    var payload=null;
    if(mode==='signup'){
      if(!signupEmail || !signupUsername || !password){ if(message) message.textContent='이메일, 아이디, 비밀번호를 입력해 주세요.'; return; }
      if(!nickname) nickname = signupUsername || (signupEmail.split('@')[0] || '여행자');
      endpoint='/api/auth/signup';
      payload={ email: signupEmail, username: signupUsername, nickname: nickname, password: password };
    } else {
      if(!loginIdentifier || !password){ if(message) message.textContent='이메일 또는 아이디와 비밀번호를 입력해 주세요.'; return; }
      payload={ identifier: loginIdentifier, password: password };
    }
    if(submitBtn){ submitBtn.disabled=true; submitBtn.textContent = mode==='signup' ? '가입 중...' : '로그인 중...'; }
    fetch(endpoint,{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) })
      .then(function(res){ return res.json().then(function(json){ if(!res.ok) throw new Error(json && json.message ? json.message : '요청에 실패했습니다.'); return json; }); })
      .then(function(json){
        if(!json || !json.user) throw new Error(mode==='signup' ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.');
        setUser(json.user);
        renderHeaderAccount();
      ...
      .catch(function(err){ if(message) message.textContent = err && err.message ? err.message : (mode==='signup' ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.'); })
      .finally(function(){ if(submitBtn){ submitBtn.disabled=false; submitBtn.textContent = mode==='signup' ? '회원가입' : '로그인'; } });
  }
  function bindAuth(){
    var loginOpen=replaceNode('admin-login-open');
    var logoutBtn=replaceNode('admin-logout-button');
    var loginTab=replaceNode('account-mode-login');
    var signupTab=replaceNode('account-mode-signup');
    var submitBtn=replaceNode('admin-login-submit');
    var cancelBtn=replaceNode('admin-login-cancel');
    var closeBtn = by('[data-close="admin"]');
    if(loginOpen) loginOpen.addEventListener('click', function(e){ e.preventDefault(); openAuthModal('login'); });
    if(logoutBtn) logoutBtn.addEventListener('click', function(e){ e.preventDefault(); setUser(null); renderHeaderAccount(); window.location.reload(); });
    if(loginTab) loginTab.addEventListener('click', function(e){ e.preventDefault(); setAuthMode('login'); });
    if(signupTab) signupTab.addEventListener('click', function(e){ e.preventDefault(); setAuthMode('signup'); });
    if(submitBtn) submitBtn.addEventListener('click', function(e){ e.preventDefault(); submitAuth(); });
    if(cancelBtn) cancelBtn.addEventListener('click', function(e){ e.preventDefault(); closeAuthModal(); });
    if(closeBtn) closeBtn.addEventListener('click', function(e){ e.preventDefault(); closeAuthModal(); });
    ['admin-id-input','account-email-input','account-username-input','account-nickname-input','admin-password-input'].forEach(function(id){
      var node=$(id); if(!node) return; node.onkeydown=function(ev){ if(ev.key==='Enter'){ ev.preventDefault(); submitAuth(); } };
    });
    setAuthMode('login');
  }
  function bindQuickThemes(){
    var selected = readJson(QUICK_THEME_KEY, []);
    byAll('.theme-quick-chip').forEach(function(btn){
      var clone = btn.cloneNode(true);
      if(btn.parentNode) btn.parentNode.replaceChild(clone, btn);
    });
    byAll('.theme-quick-chip').forEach(function(btn){
      var t = btn.getAttribute('data-theme') || '';
      btn.classList.toggle('active', selected.indexOf(t) >= 0 || btn.classList.contains('active'));
      btn.addEventListener('click', function(ev){
        ev.preventDefault(); ev.stopPropagation();
        btn.classList.toggle('active');
        var chosen = byAll('.theme-quick-chip.active').map(function(node){ return node.getAttribute('data-theme') || ''; }).filter(Boolean);
        writeJson(QUICK_THEME_KEY, chosen);
      });
    });
  }
  function bindSearchLoading(){
    ...
    btn.addEventListener('click', function(){
      btn.disabled=true; btn.dataset.originalText = btn.dataset.originalText || btn.textContent; btn.textContent='검색 중...';
      var loading=$('search-loading'); if(loading) loading.classList.remove('hidden');
      setTimeout(function(){ btn.disabled=false; btn.textContent=btn.dataset.originalText || '맛집 검색'; if(loading) loading.classList.add('hidden'); }, 3000);
    });
  }
  function hideDockNearBottom(){
    var dock=by('.floating-dock'); if(!dock) return;
    var onScroll=function(){
      var nearBottom = window.innerHeight + window.scrollY >= document.documentElement.scrollHeight - 220;
      dock.classList.toggle('hide-at-bottom', nearBottom);
    };
    window.addEventListener('scroll', onScroll, {passive:true});
  ...
  function hydrateCommunityNickname(){
    var user=getUser(); var nick=$('community-nickname'); if(user && nick && !nick.value) nick.value = user.nickname || user.username || user.email || '';
  }
  function boot(){
    renderHeaderAccount();
    bindAuth();
    bindQuickThemes();
    bindSearchLoading();
    hydrateCommunityNickname();
    refreshHealth();
    setTimeout(refreshHealth, 800);
    setTimeout(refreshHealth, 2200);
    setInterval(refreshHealth, 30000);
    hideDockNearBottom();
  }


  // ===== v40 robust fixes: auth, themes, status =====
  function v40SetAuthMode(mode){
    var hidden = document.getElementById('account-auth-mode');
    var loginRow = document.getElementById('account-login-row');
    var emailRow = document.getElementById('account-email-row');
    var usernameRow = document.getElementById('account-username-row');
    ...
    if (hint) hint.textContent = mode === 'signup' ? '이메일, 아이디, 닉네임, 비밀번호를 입력해 계정을 생성합니다.' : '로그인하면 저장한 일정과 커뮤니티 글을 관리할 수 있습니다.';
  }

  function v40RenderHeaderAccount(){
    var user = readJson(STORAGE.userSession, null);
    state.currentUser = user || null;
    var badge = document.getElementById('current-user-badge');
    ...
    if (loginBtn) loginBtn.classList.toggle('hidden', !!user);
    if (logoutBtn) logoutBtn.classList.toggle('hidden', !user);
    var nickInput = document.getElementById('community-nickname');
    if (nickInput) {
      if (user) {
        nickInput.value = user.nickname || user.username || user.email || '';
        nickInput.disabled = true;
        nickInput.placeholder = '로그인된 닉네임이 사용됩니다';
      } else {
        nickInput.disabled = false;
        nickInput.placeholder = '예: 도쿄혼행러';
      }
    }
  }

  function v40OpenAuth(mode){
    var modal = document.getElementById('admin-login-modal');
    if (!modal) return;
    v40SetAuthMode(mode || 'login');
    modal.classList.remove('hidden');
    modal.setAttribute('aria-hidden','false');
  }

  function v40CloseAuth(){
    var modal = document.getElementById('admin-login-modal');
    if (!modal) return;
  ...
  function v40SubmitAuth(){
    ...
    if (mode === 'signup') {
      ...
      payload = { identifier: loginIdentifier, email: loginIdentifier, password: password };
    }
    if (submitBtn) { submitBtn.disabled = true; submitBtn.textContent = mode === 'signup' ? '가입 중...' : '로그인 중...'; }
    fetch(endpoint, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) })
      .then(function(res){ return res.json().then(function(json){ if(!res.ok) throw new Error(json && json.message ? json.message : '요청에 실패했습니다.'); return json; }); })
      .then(function(json){
        if (!json || !json.user) throw new Error(mode === 'signup' ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.');
        writeJson(STORAGE.userSession, json.user);
        state.currentUser = json.user;
        v40RenderHeaderAccount();
        if (message) message.textContent = mode === 'signup' ? '회원가입이 완료되었습니다.' : '로그인되었습니다.';
        setTimeout(function(){ v40CloseAuth(); }, 200);
      })
      .catch(function(err){ if (message) message.textContent = err && err.message ? err.message : (mode === 'signup' ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.'); })
      .finally(function(){ if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = mode === 'signup' ? '회원가입' : '로그인'; } });
  }

  function v40BindAuth(){
    var loginOpen = replaceNode('admin-login-open');
    var logoutBtn = replaceNode('admin-logout-button');
    var loginTab = replaceNode('account-mode-login');
    var signupTab = replaceNode('account-mode-signup');
    var submitBtn = replaceNode('admin-login-submit');
    var cancelBtn = replaceNode('admin-login-cancel');
    var closeButtons = byAll('[data-close="admin"]');
    if (loginOpen) loginOpen.addEventListener('click', function(e){ e.preventDefault(); v40OpenAuth('login'); });
    if (logoutBtn) logoutBtn.addEventListener('click', function(e){ e.preventDefault(); writeJson(STORAGE.userSession, null); state.currentUser = null; v40RenderHeaderAccount(); window.location.reload(); });
    if (loginTab) loginTab.addEventListener('click', function(e){ e.preventDefault(); v40SetAuthMode('login'); });
    if (signupTab) signupTab.addEventListener('click', function(e){ e.preventDefault(); v40SetAuthMode('signup'); });
    if (submitBtn) submitBtn.addEventListener('click', function(e){ e.preventDefault(); v40SubmitAuth(); });
    if (cancelBtn) cancelBtn.addEventListener('click', function(e){ e.preventDefault(); v40CloseAuth(); });
    closeButtons.forEach(function(btn){ btn.onclick = function(e){ e.preventDefault(); v40CloseAuth(); }; });
    ['admin-id-input','account-email-input','account-username-input','account-nickname-input','admin-password-input'].forEach(function(id){
      var node = document.getElementById(id);
      if (node) node.onkeydown = function(ev){ if (ev.key === 'Enter') { ev.preventDefault(); v40SubmitAuth(); } };
    });
    v40SetAuthMode('login');
    v40RenderHeaderAccount();
  }

  function v40SetBadge(service, text){
    var badge = document.querySelector('.status-badge[data-service="'+service+'"]');
    if (!badge) return;
    var dot = badge.querySelector('.status-dot');
    var stateNode = badge.querySelector('.status-state');
    if (stateNode) {
      stateNode.textContent = text;
      stateNode.className = 'status-state ' + (text === '연결됨' ? 'on' : (text === '데모' ? 'demo' : 'off'));
    }
    if (dot) dot.className = 'status-dot ' + (text === '연결됨' ? 'on' : 'off');
    badge.title = (service === 'hotpepper' ? 'Hot Pepper ' : 'Gemini ') + text;
  }

  function v40RefreshHealth(){
    return fetch('/api/health', { cache:'no-store' })
      .then(function(res){ if(!res.ok) throw new Error('health_failed'); return res.json(); })
      .then(function(json){
        state.liveEnabled = !!(json && json.hotpepperConfigured);
        state.geminiAvailable = !!(json && json.geminiConfigured);
        v40SetBadge('hotpepper', json && json.hotpepperConfigured ? '연결됨' : ((json && /demo/i.test(String(json.mode||''))) ? '데모' : '미연결'));
        v40SetBadge('gemini', json && json.geminiConfigured ? '연결됨' : '미연결');
      })
      .catch(function(){
        v40SetBadge('hotpepper', '미연결');
        v40SetBadge('gemini', '미연결');
      });
  }

  function v40BindQuickThemes(){
    var row = document.getElementById('quick-chip-row');
    if (!row) return;
    var selected = readJson(QUICK_THEME_KEY, []);
    row.querySelectorAll('.theme-quick-chip').forEach(function(btn){
      var clone = btn.cloneNode(true);
      if (btn.parentNode) btn.parentNode.replaceChild(clone, btn);
    });
    row.querySelectorAll('.theme-quick-chip').forEach(function(btn){
      var theme = btn.getAttribute('data-theme') || '';
      btn.classList.toggle('active', selected.indexOf(theme) >= 0);
      btn.onclick = function(ev){
        ev.preventDefault();
        ev.stopPropagation();
        btn.classList.toggle('active');
        var chosen = Array.prototype.slice.call(row.querySelectorAll('.theme-quick-chip.active')).map(function(node){ return node.getAttribute('data-theme') || ''; }).filter(Boolean);
        state.currentQuery.quickThemes = chosen;
        writeJson(QUICK_THEME_KEY, chosen);
        saveJson(STORAGE.query, state.currentQuery);
      };
    });
  ...
  function v40HydrateCommunityNickname(){
    var user = readJson(STORAGE.userSession, null);
    var nick = document.getElementById('community-nickname');
    if (nick && user) {
      nick.value = user.nickname || user.username || user.email || '';
      nick.disabled = true;
    }
  }
  ...
  function v40Boot(){
    v40BindAuth();
    v40BindQuickThemes();
    v40HydrateCommunityNickname();
    v40RefreshHealth();
    setTimeout(v40RefreshHealth, 800);
    setTimeout(v40RefreshHealth, 2200);
    setInterval(v40RefreshHealth, 30000);
  }

  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', function(){ setTimeout(v40Boot, 120); });
  else setTimeout(v40Boot, 120);
