(function () {
  ...
  const STORAGE = {
  query: 'hankkiRouteJapan.query.v7',
  plan: 'hankkiRouteJapan.plan.v7',
  planStory: 'hankkiRouteJapan.planStory.v2',
  admin: 'hankkiRouteJapan.admin.v7',
  adminSession: 'hankkiRouteJapan.adminSession.v7',
  forceDemo: 'hankkiRouteJapan.forceDemo.v7',
  basePoint: 'hankkiRouteJapan.basePoint.v7',
  hotelPoint: 'hankkiRouteJapan.hotelPoint.v1',
  uiView: 'hankkiRouteJapan.uiView.v3',
...
'plan-extra-places', 'plan-use-ai', 'plan-ai-state'
    ...
    return Promise.all(inputs.map(resolvePointLine)).then(function (resolved) {
      ...
      return Promise.all(points.map(function (point) {
        ...
        return fillPlanFromPointGroups(groups, {
          type: 'nearby-multi',
          useAi: useAi,
          overviewSeed: '입력한 방문 후보를 오전 / 오후 방문지로 나누고, 각 지점 근처 식당과 숙소 동선을 함께 묶어 일정으로 만들었습니다.',
        });
      });
    });
  }
  ...
  function generateRegionalAutoPlan(useAi) {
    ...
    const days = clamp(parseInt(els.daysInput.value || '3', 10), 1, 10);
    const picked = [];
    for (var i = 0; i < days * 2; i += 1) picked.push(makePointDisplay(spots[i % spots.length]));
    return Promise.all(picked.map(function (point) {
      ...
      return fillPlanFromPointGroups(groups, {
        type: 'regional-auto',
        useAi: useAi,
        overviewSeed: (state.currentQuery.region || '입력한 지역') + '에서 오전 / 오후 방문지를 나누고, 근처 식당과 숙소 동선을 함께 섞어 일정으로 만들었습니다.',
      });
    });
  }
