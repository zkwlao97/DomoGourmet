...
function renderAdminDashboard() {
  ...
  var THEME_EMOJI_MAP = { '관광': '🗾', '휴양': '♨️', '애니': '🎌', '패션': '👗', '먹방': '🍱' };
  ...
  function renderThemeSelectors() {
    ...
    // Only re-render if there are extra themes beyond the 5 defaults (admin-added themes)
    var defaults = ['관광', '휴양', '애니', '패션', '먹방'];
    var extras = rows.filter(function (theme) { return defaults.indexOf(theme) < 0; });
    // Always preserve existing checkboxes state; only add missing extras
    if (extras.length === 0) {
      // Just sync checked state from saved selection
      els.planThemeOptions.querySelectorAll('input[type="checkbox"]').forEach(function (node) {
        var val = String(node.value || '').trim();
        node.checked = selected.indexOf(val) >= 0;
      });
      return;
    }
    // If there are admin-added themes, rebuild including extras
    els.planThemeOptions.innerHTML = rows.map(function (theme) {
      var checked = selected.indexOf(theme) >= 0 ? ' checked' : '';
      var emoji = THEME_EMOJI_MAP[theme] ? THEME_EMOJI_MAP[theme] + ' ' : '';
      return '<label class="theme-check-chip"><input type="checkbox" value="' + escapeHtml(theme) + '"' + checked + ' /><span>' + emoji + escapeHtml(theme) + '</span></label>';
    }).join('');
  }
...
function saveCommunityPost() {
  ...
  // Re-render plan UI with v12 components only after cache() has populated els.
  // Calling renderPlan() here before init/cache() stops the rest of app.js,
  // so login/signup patches below never get installed.
  if (typeof renderPlan === 'function' && els && els.planTableBody) {
    renderPlan();
...
/* ===== V41 FINAL AUTH/THEME/STATUS PATCH ===== */
function updateStatusBadgesV41(payload) {
  var hpText = document.getElementById('status-hotpepper-header-2');
  var gmText = document.getElementById('status-gemini-header');
  var hpDot = document.getElementById('status-hotpepper');
  var gmDot = document.getElementById('status-gemini');
  var hpBadge = document.querySelector('.status-badge[data-service="hotpepper"]');
  var gmBadge = document.querySelector('.status-badge[data-service="gemini"]');
  state.liveEnabled = !!(payload && payload.hotpepperConfigured);
  state.geminiAvailable = !!(payload && payload.geminiConfigured);
  if (hpText) {
    hpText.textContent = state.liveEnabled ? '연결됨' : '데모';
    hpText.className = 'status-state ' + (state.liveEnabled ? 'on' : 'demo');
  }
  if (gmText) {
    gmText.textContent = state.geminiAvailable ? '연결됨' : '미연결';
    gmText.className = 'status-state ' + (state.geminiAvailable ? 'on' : 'off');
  }
  if (hpDot) hpDot.className = 'status-dot ' + (state.liveEnabled ? 'on' : 'off');
  if (gmDot) gmDot.className = 'status-dot ' + (state.geminiAvailable ? 'on' : 'off');
  if (hpBadge) hpBadge.title = state.liveEnabled ? 'Hot Pepper 연결됨' : 'Hot Pepper 데모';
  if (gmBadge) gmBadge.title = state.geminiAvailable ? 'Gemini 연결됨' : 'Gemini 미연결';
}

function refreshServiceHealthV41() {
  return fetch('/api/health', { cache: 'no-store' })
    .then(function (r) { if (!r.ok) throw new Error('health_failed'); return r.json(); })
    .then(function (json) { updateStatusBadgesV41(json); return json; })
    .catch(function () {
      updateStatusBadgesV41({ hotpepperConfigured: !!state.liveEnabled, geminiConfigured: !!state.geminiAvailable });
    });
}

function setQuickThemeValueV41(theme) {
  state.currentQuery = state.currentQuery || {};
  state.currentQuery.themes = theme ? [theme] : [];
  saveJson(STORAGE.query, state.currentQuery);
  document.querySelectorAll('#quick-chip-row [data-theme]').forEach(function (btn) {
    var active = String(btn.getAttribute('data-theme') || '') === theme;
    btn.classList.toggle('active', active);
    btn.setAttribute('aria-pressed', active ? 'true' : 'false');
  });
}

function bindQuickThemeRowV41() {
  var row = document.getElementById('quick-chip-row');
  if (!row) return;
  Array.from(row.querySelectorAll('[data-theme]')).forEach(function (oldBtn) {
    var btn = oldBtn.cloneNode(true);
    oldBtn.parentNode.replaceChild(btn, oldBtn);
    btn.addEventListener('click', function (event) {
      event.preventDefault();
      event.stopPropagation();
      setQuickThemeValueV41(String(btn.getAttribute('data-theme') || ''));
    });
  });
  var current = (state.currentQuery && Array.isArray(state.currentQuery.themes) && state.currentQuery.themes[0]) || '';
  setQuickThemeValueV41(current || '');
}

function setAuthModeV41(mode) {
  var loginMode = mode !== 'signup';
  var modeInput = document.getElementById('account-auth-mode');
  if (modeInput) modeInput.value = loginMode ? 'login' : 'signup';
  toggleHidden(document.getElementById('auth-login-id-row'), !loginMode);
  toggleHidden(document.getElementById('auth-login-password-row'), !loginMode);
  toggleHidden(document.getElementById('auth-signup-email-row'), loginMode);
  toggleHidden(document.getElementById('auth-signup-username-row'), loginMode);
  toggleHidden(document.getElementById('account-nickname-row'), loginMode);
  toggleHidden(document.getElementById('auth-signup-password-row'), loginMode);
  var loginTab = document.getElementById('account-mode-login');
  var signupTab = document.getElementById('account-mode-signup');
  if (loginTab) loginTab.classList.toggle('active', loginMode);
  if (signupTab) signupTab.classList.toggle('active', !loginMode);
  var submit = document.getElementById('admin-login-submit');
  if (submit) submit.textContent = loginMode ? '로그인' : '회원가입';
  var message = document.getElementById('admin-login-message');
  if (message) message.textContent = '';
}

function updateCurrentUserHeaderV41() {
  var display = document.getElementById('account-display');
  var loginBtn = document.getElementById('admin-login-open');
  var logoutBtn = document.getElementById('admin-logout-button');
  var nickname = state.currentUser ? (state.currentUser.role === 'admin' ? '관리자' : (state.currentUser.nickname || state.currentUser.username || state.currentUser.email || '내 계정')) : '';
  if (display) {
    if (nickname) {
  ...
  if (loginBtn) loginBtn.classList.toggle('hidden', !!nickname);
  if (logoutBtn) logoutBtn.classList.toggle('hidden', !nickname);
  if (els && els.navAdminView) toggleHidden(els.navAdminView, !(state.currentUser && state.currentUser.role === 'admin'));
  if (els && els.dockAdminView) toggleHidden(els.dockAdminView, !(state.currentUser && state.currentUser.role === 'admin'));
  var communityNickname = document.getElementById('community-nickname');
  if (communityNickname) {
    if (nickname) {
      communityNickname.value = nickname;
      communityNickname.disabled = true;
    } else {
      communityNickname.disabled = false;
      if (!communityNickname.value) communityNickname.placeholder = '예: 도쿄혼행러';
    }
  }
}

function persistCurrentUserV41(user) {
  state.currentUser = user || null;
  saveJson(STORAGE.userSession, state.currentUser);
  updateCurrentUserHeaderV41();
  renderCommunityView();
}

function resetAuthFormV41() {
  ['admin-id-input','admin-password-input','account-signup-email','account-signup-username','account-nickname-input','account-signup-password'].forEach(function (id) {
    var node = document.getElementById(id);
    if (node) node.value = '';
  });
  setAuthModeV41('login');
}

function submitAccountV41() {
  var mode = ((document.getElementById('account-auth-mode') || {}).value || 'login');
  var isSignup = mode === 'signup';
  var submitBtn = document.getElementById('admin-login-submit');
  var message = document.getElementById('admin-login-message');
  var endpoint = isSignup ? '/api/auth/signup' : '/api/auth/login';
  var payload;
  if (isSignup) {
    var email = String((document.getElementById('account-signup-email') || {}).value || '').trim();
    var username = String((document.getElementById('account-signup-username') || {}).value || '').trim();
    ...
    if (!email || !username || !password) {
      if (message) message.textContent = '이메일, 아이디, 비밀번호를 입력해 주세요.';
      return;
    }
    if (!nickname) nickname = username || email.split('@')[0] || '여행자';
    payload = { email: email, username: username, nickname: nickname, password: password };
  } else {
    var identifier = String((document.getElementById('admin-id-input') || {}).value || '').trim();
    var loginPassword = String((document.getElementById('admin-password-input') || {}).value || '').trim();
    if (!identifier || !loginPassword) {
      if (message) message.textContent = '이메일 또는 아이디와 비밀번호를 입력해 주세요.';
      return;
    }
    payload = { email: identifier, password: loginPassword };
  }
  if (submitBtn) { submitBtn.disabled = true; submitBtn.textContent = isSignup ? '가입 중...' : '로그인 중...'; }
  if (message) message.textContent = '';
  postJson(endpoint, payload).then(function (json) {
    if (!json || !json.user) throw new Error(isSignup ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.');
    persistCurrentUserV41(json.user);
    if (message) message.textContent = isSignup ? '회원가입이 완료되었습니다.' : '로그인되었습니다.';
    setTimeout(function () { closeAdminModal(); }, 180);
  }).catch(function (error) {
    if (message) message.textContent = error && error.message ? error.message : (isSignup ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.');
  }).finally(function () {
    if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = isSignup ? '회원가입' : '로그인'; }
  });
}

function bindAuthUiV41() {
  ['account-mode-login','account-mode-signup','admin-login-submit','admin-login-cancel','admin-login-open','admin-logout-button'].forEach(function(id){
    var node = document.getElementById(id); if (!node || !node.parentNode) return;
    var clone = node.cloneNode(true); node.parentNode.replaceChild(clone, node);
  });
  var loginTab = document.getElementById('account-mode-login');
  var signupTab = document.getElementById('account-mode-signup');
  var submitBtn = document.getElementById('admin-login-submit');
  var cancelBtn = document.getElementById('admin-login-cancel');
  var openBtn = document.getElementById('admin-login-open');
  var logoutBtn = document.getElementById('admin-logout-button');
  if (loginTab) loginTab.addEventListener('click', function (e) { e.preventDefault(); setAuthModeV41('login'); });
  if (signupTab) signupTab.addEventListener('click', function (e) { e.preventDefault(); setAuthModeV41('signup'); });
  if (submitBtn) submitBtn.addEventListener('click', function (e) { e.preventDefault(); submitAccountV41(); });
  if (cancelBtn) cancelBtn.addEventListener('click', function (e) { e.preventDefault(); closeAdminModal(); });
  if (openBtn) openBtn.addEventListener('click', function (e) { e.preventDefault(); resetAuthFormV41(); showModal(els.adminLoginModal); });
  if (logoutBtn) logoutBtn.addEventListener('click', function (e) { e.preventDefault(); persistCurrentUserV41(null); });
  ['admin-id-input','admin-password-input','account-signup-email','account-signup-username','account-nickname-input','account-signup-password'].forEach(function (id) {
    var node = document.getElementById(id); if (!node) return;
    node.addEventListener('keydown', function (event) { if (event.key === 'Enter') { event.preventDefault(); submitAccountV41(); } });
  });
  setAuthModeV41('login');
  updateCurrentUserHeaderV41();
}

function wrapResultNameV41() {
  var cards = document.querySelectorAll('#results-list .result-card, #results-list .compact-card');
  cards.forEach(function (card) {
    card.classList.add('v41-card-ready');
  });
}

var __v41_renderResultsWrap = renderResults;
renderResults = function () {
  var out = __v41_renderResultsWrap.apply(this, arguments);
  wrapResultNameV41();
  refreshServiceHealthV41();
  return out;
};

var __v41_renderCommunityWrap = renderCommunityView;
renderCommunityView = function () {
  var out = __v41_renderCommunityWrap.apply(this, arguments);
  updateCurrentUserHeaderV41();
  return out;
};

var __v41_searchWrap = search;
search = function (options) {
  var btn = document.getElementById('search-button');
  var original = btn ? btn.textContent : '';
  if (btn) { btn.disabled = true; btn.textContent = '검색 중...'; }
  return Promise.resolve(__v41_searchWrap(options)).finally(function () {
    if (btn) { btn.disabled = false; btn.textContent = original || '맛집 검색'; }
  });
};

if (typeof runAiTripPlanner === 'function') {
  var __v41_aiWrap = runAiTripPlanner;
  runAiTripPlanner = function (mode) {
    var btn = document.getElementById('schedule-run-button');
    var original = btn ? btn.textContent : '';
    if (btn) { btn.disabled = true; btn.textContent = '일정 만드는 중...'; }
    return Promise.resolve(__v41_aiWrap(mode)).finally(function () {
      if (btn) { btn.disabled = false; btn.textContent = original || '이 방식으로 일정 만들기'; }
    });
  };
}
...
/* ===== V42 FINAL AUTH/THEME/STATUS FIX ===== */
function updateStatusBadgesV42(payload) {
  state.apiAvailable = true;
  state.liveEnabled = !!(payload && payload.hotpepperConfigured);
  state.geminiAvailable = !!(payload && payload.geminiConfigured);
  document.querySelectorAll('.status-badge[data-service]').forEach(function (badge) {
    var service = badge.getAttribute('data-service');
    var textNode = badge.querySelector('.status-state');
    var dotNode = badge.querySelector('.status-dot');
    var text = '확인 중';
    var cls = 'pending';
    if (service === 'hotpepper') {
      text = state.liveEnabled ? '연결됨' : '데모';
      cls = state.liveEnabled ? 'on' : 'demo';
      badge.title = state.liveEnabled ? 'Hot Pepper 연결됨' : 'Hot Pepper 데모';
    } else if (service === 'gemini') {
      text = state.geminiAvailable ? '연결됨' : '미연결';
      cls = state.geminiAvailable ? 'on' : 'off';
      badge.title = state.geminiAvailable ? 'Gemini 연결됨' : 'Gemini 미연결';
    }
    if (textNode) {
      textNode.textContent = text;
      textNode.className = 'status-state ' + cls;
    }
    if (dotNode) dotNode.className = 'status-dot ' + (cls === 'demo' ? 'off' : cls);
  });
}

function refreshServiceHealthV42() {
  return fetch('/api/health', { cache: 'no-store' })
    .then(function (r) { if (!r.ok) throw new Error('health_failed'); return r.json(); })
    .then(function (json) { updateStatusBadgesV42(json); return json; })
    .catch(function () { updateStatusBadgesV42({ hotpepperConfigured: !!state.liveEnabled, geminiConfigured: !!state.geminiAvailable }); });
}

function setQuickThemeValueV42(theme) {
  state.currentQuery = state.currentQuery || {};
  state.currentQuery.themes = theme ? [theme] : [];
  saveJson(STORAGE.query, state.currentQuery);
  var foodInput = document.getElementById('food-input');
  var currentFood = foodInput ? foodInput.value : '';
  document.querySelectorAll('#quick-chip-row [data-theme]').forEach(function (btn) {
    var active = String(btn.getAttribute('data-theme') || '') === String(theme || '');
    btn.classList.toggle('active', active);
    btn.setAttribute('aria-pressed', active ? 'true' : 'false');
  });
  if (foodInput && foodInput.value !== currentFood) foodInput.value = currentFood;
}

function bindQuickThemeRowV42() {
  var row = document.getElementById('quick-chip-row');
  if (!row) return;
  Array.from(row.querySelectorAll('[data-theme]')).forEach(function (oldBtn) {
    var btn = oldBtn.cloneNode(true);
    ...
    btn.addEventListener('click', function (e) {
      e.preventDefault();
      e.stopPropagation();
      setQuickThemeValueV42(String(btn.getAttribute('data-theme') || ''));
    });
  });
  var current = state.currentQuery && Array.isArray(state.currentQuery.themes) ? state.currentQuery.themes[0] : '';
  if (current) setQuickThemeValueV42(current);
}

function setAuthModeV42(mode) {
  var isSignup = mode === 'signup';
  var modeInput = document.getElementById('account-auth-mode');
  if (modeInput) modeInput.value = isSignup ? 'signup' : 'login';
  toggleHidden(document.getElementById('auth-login-id-row'), isSignup);
  toggleHidden(document.getElementById('auth-login-password-row'), isSignup);
  toggleHidden(document.getElementById('auth-signup-email-row'), !isSignup);
  toggleHidden(document.getElementById('auth-signup-username-row'), !isSignup);
  toggleHidden(document.getElementById('account-nickname-row'), !isSignup);
  toggleHidden(document.getElementById('auth-signup-password-row'), !isSignup);
  var loginTab = document.getElementById('account-mode-login');
  var signupTab = document.getElementById('account-mode-signup');
  if (loginTab) loginTab.classList.toggle('active', !isSignup);
  if (signupTab) signupTab.classList.toggle('active', isSignup);
  var submit = document.getElementById('admin-login-submit');
  if (submit) submit.textContent = isSignup ? '회원가입' : '로그인';
  var message = document.getElementById('admin-login-message');
  if (message) message.textContent = '';
}

function updateCurrentUserHeaderV42() {
  var display = document.getElementById('account-display');
  var loginBtn = document.getElementById('admin-login-open');
  var logoutBtn = document.getElementById('admin-logout-button');
  var nickname = state.currentUser ? (state.currentUser.role === 'admin' ? '관리자' : (state.currentUser.nickname || state.currentUser.username || state.currentUser.email || '내 계정')) : '';
  if (display) {
    if (nickname) { display.textContent = nickname; display.classList.remove('hidden'); }
    else { display.textContent = ''; display.classList.add('hidden'); }
  }
  if (loginBtn) loginBtn.classList.toggle('hidden', !!nickname);
  if (logoutBtn) logoutBtn.classList.toggle('hidden', !nickname);
...
function persistCurrentUserV42(user) {
  state.currentUser = user || null;
  saveJson(STORAGE.userSession, state.currentUser);
  state.adminSession = { loggedIn: Boolean(user && user.role === 'admin') };
  saveJson(STORAGE.adminSession, state.adminSession);
  updateCurrentUserHeaderV42();
  try { renderCommunityView(); } catch (_) {}
}

function resetAuthFormV42() {
  ['admin-id-input','admin-password-input','account-signup-email','account-signup-username','account-nickname-input','account-signup-password'].forEach(function (id) {
    var node = document.getElementById(id);
    if (node) node.value = '';
  });
  setAuthModeV42('login');
}

function submitAccountV42() {
  var isSignup = ((document.getElementById('account-auth-mode') || {}).value || 'login') === 'signup';
  var submitBtn = document.getElementById('admin-login-submit');
  var message = document.getElementById('admin-login-message');
  var endpoint = isSignup ? '/api/auth/signup' : '/api/auth/login';
  var payload;
  ...
  postJson(endpoint, payload).then(function (json) {
    if (!json || !json.user) throw new Error(isSignup ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.');
    persistCurrentUserV42(json.user);
    if (message) message.textContent = isSignup ? '회원가입이 완료되었습니다.' : '로그인되었습니다.';
    setTimeout(function () { closeAdminModal(); }, 180);
  }).catch(function (error) {
    if (message) message.textContent = error && error.message ? error.message : (isSignup ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.');
...
function bindAuthUiV42() {
  ...
  var openBtn = document.getElementById('admin-login-open');
  var logoutBtn = document.getElementById('admin-logout-button');
  if (loginTab) loginTab.addEventListener('click', function (e) { e.preventDefault(); setAuthModeV42('login'); });
  if (signupTab) signupTab.addEventListener('click', function (e) { e.preventDefault(); setAuthModeV42('signup'); });
  if (submitBtn) submitBtn.addEventListener('click', function (e) { e.preventDefault(); submitAccountV42(); });
  if (cancelBtn) cancelBtn.addEventListener('click', function (e) { e.preventDefault(); closeAdminModal(); });
  if (openBtn) openBtn.addEventListener('click', function (e) { e.preventDefault(); resetAuthFormV42(); showModal(els.adminLoginModal); });
  if (logoutBtn) logoutBtn.addEventListener('click', function (e) { e.preventDefault(); persistCurrentUserV42(null); });
  ['admin-id-input','admin-password-input','account-signup-email','account-signup-username','account-nickname-input','account-signup-password'].forEach(function (id) {
    var node = document.getElementById(id); if (!node) return;
    node.addEventListener('keydown', function (event) { if (event.key === 'Enter') { event.preventDefault(); submitAccountV42(); } });
  });
  setAuthModeV42('login');
  updateCurrentUserHeaderV42();
}
...
document.addEventListener('DOMContentLoaded', function () {
  setTimeout(function () {
    try { bindQuickThemeRowV42(); } catch (_) {}
    try { bindAuthUiV42(); } catch (_) {}
    try { refreshServiceHealthV42(); } catch (_) {}
    try { updateCurrentUserHeaderV42(); } catch (_) {}
  }, 180);
});
/* ===== END V42 FIX ===== */

/* ===== END V41 PATCH ===== */
/* ===== V44 FINAL AUTH STABILIZER =====
   로그인 / 회원가입 UI는 이전 버전 패치들이 버튼을 cloneNode로 교체하며
   이벤트가 사라질 수 있어서, 캡처 단계에서 최종 인증 핸들러가 먼저 처리한다. */
(function () {
  var AUTH_INPUT_IDS = [
    'admin-id-input',
    'admin-password-input',
    'account-signup-email',
    'account-signup-username',
    'account-nickname-input',
    'account-signup-password'
  ];
  ...
  function getCurrentNicknameV44() {
    if (!state.currentUser) return '';
    if (state.currentUser.role === 'admin') return '관리자';
    return state.currentUser.nickname || state.currentUser.username || state.currentUser.email || '내 계정';
  }

  function setAuthModeV44(mode) {
    var isSignup = mode === 'signup';
    var modeInput = byId('account-auth-mode');
    if (modeInput) modeInput.value = isSignup ? 'signup' : 'login';

    setHidden('auth-login-id-row', isSignup);
    setHidden('auth-login-password-row', isSignup);
    setHidden('auth-signup-email-row', !isSignup);
    setHidden('auth-signup-username-row', !isSignup);
    setHidden('account-nickname-row', !isSignup);
    setHidden('auth-signup-password-row', !isSignup);

    var loginTab = byId('account-mode-login');
    var signupTab = byId('account-mode-signup');
    if (loginTab) {
    ...
    var submit = byId('admin-login-submit');
    if (submit) submit.textContent = isSignup ? '회원가입' : '로그인';
    setText('admin-login-message', '');
  }

  function resetAuthFormV44() {
    AUTH_INPUT_IDS.forEach(function (id) {
      var node = byId(id);
      if (node) node.value = '';
    });
    setAuthModeV44('login');
  }

  function openAuthModalV44() {
    resetAuthFormV44();
    var modal = byId('admin-login-modal');
  ...
  function updateCurrentUserHeaderV44() {
    var nickname = getCurrentNicknameV44();
    var display = byId('account-display');
    var loginBtn = byId('admin-login-open');
    var logoutBtn = byId('admin-logout-button');

    if (display) {
      display.textContent = nickname;
      display.classList.toggle('hidden', !nickname);
    }
    if (loginBtn) loginBtn.classList.toggle('hidden', Boolean(nickname));
    if (logoutBtn) logoutBtn.classList.toggle('hidden', !nickname);

    if (els && els.adminPanel) toggleHidden(els.adminPanel, !(state.currentUser && state.currentUser.role === 'admin'));
    if (els && els.navAdminView) toggleHidden(els.navAdminView, !(state.currentUser && state.currentUser.role === 'admin'));
    if (els && els.dockAdminView) toggleHidden(els.dockAdminView, !(state.currentUser && state.currentUser.role === 'admin'));

    var communityNickname = byId('community-nickname');
    if (communityNickname) {
      if (nickname) {
        communityNickname.value = nickname;
        communityNickname.disabled = true;
        communityNickname.placeholder = '로그인된 닉네임이 사용됩니다';
      } else {
        communityNickname.disabled = false;
        if (!communityNickname.value) communityNickname.placeholder = '예: 도쿄혼행러';
      }
  ...
  function persistCurrentUserV44(user) {
    state.currentUser = user || null;
    saveJson(STORAGE.userSession, state.currentUser);
    state.adminSession = { loggedIn: Boolean(user && user.role === 'admin') };
    saveJson(STORAGE.adminSession, state.adminSession);
    updateCurrentUserHeaderV44();
    try { renderAdminState(); } catch (_) {}
    updateCurrentUserHeaderV44();
    try { renderCommunityView(); } catch (_) {}
    try { renderContentView(); } catch (_) {}
    try { renderResults(); } catch (_) {}
  }

  function submitAuthV44() {
    var isSignup = ((byId('account-auth-mode') || {}).value || 'login') === 'signup';
    var submitBtn = byId('admin-login-submit');
    var message = byId('admin-login-message');
    var endpoint = isSignup ? '/api/auth/signup' : '/api/auth/login';
    var payload;

    if (isSignup) {
      var email = String((byId('account-signup-email') || {}).value || '').trim();
      var username = String((byId('account-signup-username') || {}).value || '').trim();
      var nickname = String((byId('account-nickname-input') || {}).value || '').trim();
      var password = String((byId('account-signup-password') || {}).value || '').trim();
      if (!email || !username || !password) {
        if (message) message.textContent = '이메일, 아이디, 비밀번호를 입력해 주세요.';
        return;
      }
      ...
      var identifier = String((byId('admin-id-input') || {}).value || '').trim();
      var loginPassword = String((byId('admin-password-input') || {}).value || '').trim();
      if (!identifier || !loginPassword) {
        if (message) message.textContent = '이메일 또는 아이디와 비밀번호를 입력해 주세요.';
        return;
      }
      payload = { identifier: identifier, email: identifier, username: identifier, password: loginPassword };
    }
    ...
    postJson(endpoint, payload).then(function (json) {
      if (!json || !json.user) throw new Error(isSignup ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.');
      persistCurrentUserV44(json.user);
      if (message) message.textContent = isSignup ? '회원가입이 완료되었습니다.' : '로그인되었습니다.';
      setTimeout(closeAuthModalV44, 180);
    }).catch(function (error) {
      if (message) message.textContent = error && error.message ? error.message : (isSignup ? '회원가입에 실패했습니다.' : '로그인에 실패했습니다.');
    }).finally(function () {
      if (submitBtn) {
  ...
  function logoutV44() {
    persistCurrentUserV44(null);
  }

  function handleAuthClickV44(event) {
    var source = event.target && event.target.nodeType === 1 ? event.target : (event.target && event.target.parentElement);
    var target = source && source.closest ? source.closest('#admin-login-open,#admin-logout-button,#account-mode-login,#account-mode-signup,#admin-login-submit,#admin-login-cancel,[data-close="admin"]') : null;
    if (!target) return;
    event.preventDefault();
    event.stopImmediatePropagation();

    if (target.id === 'admin-login-open') return openAuthModalV44();
    if (target.id === 'admin-logout-button') return logoutV44();
    if (target.id === 'account-mode-login') return setAuthModeV44('login');
    if (target.id === 'account-mode-signup') return setAuthModeV44('signup');
    if (target.id === 'admin-login-submit') return submitAuthV44();
    if (target.id === 'admin-login-cancel' || target.getAttribute('data-close') === 'admin') return closeAuthModalV44();
  }

  function handleAuthKeydownV44(event) {
    if (event.key !== 'Enter') return;
    var target = event.target;
    if (!target || AUTH_INPUT_IDS.indexOf(target.id) < 0) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    submitAuthV44();
  }

  function installAuthStabilizerV44() {
    if (window.__domoAuthV44Installed) {
      setAuthModeV44(((byId('account-auth-mode') || {}).value || 'login'));
      updateCurrentUserHeaderV44();
      return;
    }
    window.__domoAuthV44Installed = true;
    document.addEventListener('click', handleAuthClickV44, true);
    document.addEventListener('keydown', handleAuthKeydownV44, true);
    setAuthModeV44('login');
    updateCurrentUserHeaderV44();
  }

  openAdminModal = openAuthModalV44;
  closeAdminModal = closeAuthModalV44;
  submitAdminLogin = submitAuthV44;
  logoutAdmin = logoutV44;
  persistCurrentUserV42 = persistCurrentUserV44;
  updateCurrentUserHeaderV42 = updateCurrentUserHeaderV44;
  resetAuthFormV42 = resetAuthFormV44;
  submitAccountV42 = submitAuthV44;
  setAuthModeV42 = setAuthModeV44;

  document.addEventListener('DOMContentLoaded', function () {
    setTimeout(installAuthStabilizerV44, 0);
    setTimeout(installAuthStabilizerV44, 320);
    setTimeout(installAuthStabilizerV44, 900);
  });
...
/* ===== END V44 FINAL AUTH STABILIZER ===== */

/* ===== V46 PRODUCT CLARITY PATCH ===== */
(function () {
  var publicCounts = {
    recommended: 0,
    custom: 0,
  ...
  function hasMeaningfulQueryV46() {
    var current = state.currentQuery || {};
    return Boolean(
      String(current.region || '').trim() ||
      String(current.base || '').trim() ||
      String(current.food || '').trim() ||
      String(current.mapsLink || '').trim() ||
      String(current.hotel || '').trim()
    );
  }

  function metricV46(label, value) {
    return '<div class="trust-metric"><span>' + escapeHtml(label) + '</span><strong>' + escapeHtml(String(value)) + '</strong></div>';
  }

  function starterButtonV46(label, region, base, food) {
    return '<button class="onboarding-action" type="button" data-v46-start data-region="' + escapeHtml(region) + '" data-base="' + escapeHtml(base) + '" data-food="' + escapeHtml(food) + '">' + escapeHtml(label) + '</button>';
  }

  function renderSearchOnboardingV46() {
    var list = q('results-list');
    if (!list || hasMeaningfulQueryV46() || (state.results && state.results.length)) return;

    var curatedTotal = publicCounts.recommended || (state.recommendedRestaurants || []).length || 0;
    var customTotal = publicCounts.custom || (state.customRestaurants || []).length || 0;
    var templateTotal = publicCounts.templates || (state.themeTemplates || []).length || 0;
    var placeTotal = publicCounts.themePlaces || (state.themePlaces || []).length || 0;

    list.innerHTML =
      '<div class="onboarding-grid">' +
        '<section class="onboarding-card primary">' +
          '<p class="onboarding-kicker">Beta guide</p>' +
          '<h3 class="onboarding-title">여행지와 먹고 싶은 메뉴를 넣으면, 식사 동선을 먼저 정리해드려요.</h3>' +
          '<p class="onboarding-copy">지금은 무료 베타라서 Hot Pepper 공식 API와 운영자가 직접 저장한 장소를 중심으로 보여줍니다. 구글맵 링크를 붙이면 기준점 정확도가 더 좋아져요.</p>' +
          '<div class="onboarding-actions">' +
            starterButtonV46('시부야 라멘으로 시작', '도쿄', '시부야', '라멘') +
            starterButtonV46('난바 오코노미야키', '오사카', '난바', '오코노미야키') +
            '<button class="onboarding-action primary" type="button" data-v46-plan>AI 일정 만들기</button>' +
          '</div>' +
        '</section>' +
        '<aside class="onboarding-card">' +
          '<p class="onboarding-kicker">How it helps</p>' +
          '<div class="onboarding-steps">' +
            '<div class="onboarding-step"><strong>1</strong><span>지역, 기준 장소, 음식 키워드를 입력합니다.</span></div>' +
            '<div class="onboarding-step"><strong>2</strong><span>거리, 식사 시간, 예약 가능 여부를 한 화면에서 비교합니다.</span></div>' +
            '<div class="onboarding-step"><strong>3</strong><span>마음에 드는 식당을 일정표에 넣고 Google Maps 길찾기로 확인합니다.</span></div>' +
          '</div>' +
          '<div class="trust-metric-grid">' +
            metricV46('운영자 추천', curatedTotal + '곳') +
            metricV46('직접 등록', customTotal + '곳') +
            metricV46('테마 장소', placeTotal + '곳') +
            metricV46('일정 템플릿', templateTotal + '개') +
          '</div>' +
          '<div class="product-note">타베로그/구글 리뷰 수집 없이도, 무료 운영 범위 안에서 검색과 일정 작성에 집중한 베타 버전입니다.</div>' +
        '</aside>' +
      '</div>';
    bindV46StartButtons(list);
  }

  function applyStarterV46(region, base, food, runSearch) {
    var regionInput = q('region-input');
    var baseInput = q('base-input');
    var foodInput = q('food-input');
    if (regionInput) regionInput.value = region || '';
    if (baseInput) baseInput.value = base || '';
    if (foodInput) foodInput.value = food || '';
    if (typeof syncQueryFromInputs === 'function') syncQueryFromInputs(true);
    if (runSearch && typeof search === 'function') search({ silent: true });
  }

  function bindV46StartButtons(scope) {
    var root = scope || document;
    root.querySelectorAll('[data-v46-start], .starter-chip').forEach(function (button) {
      if (button.__v46Bound) return;
      button.__v46Bound = true;
      button.addEventListener('click', function () {
        applyStarterV46(
          button.getAttribute('data-region') || '',
          button.getAttribute('data-base') || '',
          button.getAttribute('data-food') || '',
          true
        );
      });
    });
    root.querySelectorAll('[data-v46-plan]').forEach(function (button) {
      if (button.__v46Bound) return;
      button.__v46Bound = true;
      button.addEventListener('click', function () {
        var planButton = q('auto-plan-button');
        if (planButton) planButton.click();
      });
    });
  }

  function bindSearchKeyboardV46() {
    ['region-input', 'base-input', 'hotel-input', 'food-input', 'maps-link-input', 'hotel-maps-link-input'].forEach(function (id) {
      var input = q(id);
      if (!input || input.__v46EnterBound) return;
      input.__v46EnterBound = true;
      input.addEventListener('keydown', function (event) {
        if (event.key !== 'Enter') return;
        event.preventDefault();
        if (typeof search === 'function') search({ silent: true });
      });
    });
  }

  function improveEmptyStatesV46() {
    var contentList = q('content-list');
    if (contentList && contentList.textContent.indexOf('등록된 콘텐츠가 아직 없습니다') >= 0) {
      contentList.innerHTML = '<div class="empty-state">가이드 콘텐츠는 준비 중입니다. 베타 기간에는 맛집 검색과 AI 일정표를 먼저 사용해 보세요.</div>';
    }
    var communityList = q('community-list');
    if (communityList && communityList.textContent.indexOf('아직 등록된 커뮤니티 글이 없습니다') >= 0) {
      communityList.innerHTML = '<div class="empty-state">첫 여행 루트를 남겨보세요. 좋은 동선은 다른 사용자도 일정표에 바로 담을 수 있습니다.</div>';
    }
  }

  function loadPublicCountsV46() {
    return fetch('/api/store-public', { cache: 'no-store' })
      .then(function (response) { if (!response.ok) throw new Error('store_failed'); return response.json(); })
      .then(function (json) {
        publicCounts = {
          recommended: Array.isArray(json.recommendedRestaurants) ? json.recommendedRestaurants.length : 0,
          custom: Array.isArray(json.customRestaurants) ? json.customRestaurants.length : 0,
          templates: Array.isArray(json.themeTemplates) ? json.themeTemplates.length : 0,
          themePlaces: Array.isArray(json.themePlaces) ? json.themePlaces.length : 0,
          community: Array.isArray(json.communityPosts) ? json.communityPosts.length : 0,
        };
        renderSearchOnboardingV46();
      }).catch(function () {
        renderSearchOnboardingV46();
      });
  }

  var previousRenderResultsV46 = renderResults;
  renderResults = function () {
    var out = previousRenderResultsV46.apply(this, arguments);
    renderSearchOnboardingV46();
    improveEmptyStatesV46();
    return out;
  };

  document.addEventListener('DOMContentLoaded', function () {
    bindV46StartButtons(document);
    bindSearchKeyboardV46();
    setTimeout(function () {
      bindV46StartButtons(document);
      bindSearchKeyboardV46();
      loadPublicCountsV46();
...
/* ===== END V46 PRODUCT CLARITY PATCH ===== */

/* ===== V47 APP EXPERIENCE REDESIGN ===== */
(function () {
  function q(id) { return document.getElementById(id); }

  function lineIconV47(kind) {
    if (kind === 'distance') return '동선';
    if (kind === 'time') return '시간';
    if (kind === 'food') return '메뉴';
    return '정보';
  }

  function compactBadgesV47(r) {
    var items = [];
    items.push('<span class="badge score">추천 ' + escapeHtml(String((r.travelScore || 0).toFixed(1))) + '점</span>');
    if (r.adminNote) items.push('<span class="badge admin">운영자 픽</span>');
    if (r.foodExact) items.push('<span class="badge match">음식 일치</span>');
    if (r.nameExact) items.push('<span class="badge match">식당명 일치</span>');
    return items.join('');
  }

  function signalV47(label, value, kind) {
    if (!value) return '';
    return '<div class="v47-signal"><span>' + escapeHtml(lineIconV47(kind)) + '</span><strong>' + escapeHtml(label) + '</strong><em>' + escapeHtml(value) + '</em></div>';
  }

  function actionLinkV47(label, href, className) {
    if (!href) return '';
    return '<a class="btn ' + escapeHtml(className || 'btn-secondary') + '" target="_blank" rel="noopener noreferrer" href="' + escapeHtml(href) + '">' + escapeHtml(label) + '</a>';
  }

  function renderResultCardV47(r) {
    var photo = r.photo ? '<img src="' + escapeHtml(r.photo) + '" alt="' + escapeHtml(r.displayName) + '" />' : '<div class="placeholder-image">Domo</div>';
    var regionLine = [r.regionKo || r.regionOriginal, r.areaKo || r.areaOriginal].filter(Boolean).join(' · ');
    var foodLine = [r.genreKo || r.genreOriginal, r.subGenreKo || r.subGenreOriginal, r.signatureMenu].filter(Boolean).slice(0, 3).join(' · ');
    var distanceText = r.distanceKm != null ? r.distanceKm.toFixed(2) + 'km' : '';
    var hotelText = r.hotelDistanceKm != null ? r.hotelDistanceKm.toFixed(2) + 'km' : '';
    var walkText = r.estimatedWalkMinutes ? formatMinutes(r.estimatedWalkMinutes) : '';
    var slots = r.recommendedSlots && r.recommendedSlots.length ? r.recommendedSlots.join(' / ') : '확인 필요';
    var good = (r.goodPoints && r.goodPoints[0]) || '여행 동선과 함께 비교하기 좋은 후보입니다.';
    var caution = (r.badPoints && r.badPoints[0]) || '방문 전 영업시간과 휴무를 다시 확인해 주세요.';
    var baseDirections = buildDirectionsUrl(r, state.basePoint);
    var hotelDirections = state.hotelPoint ? buildDirectionsUrl(r, state.hotelPoint) : '';

    return '' +
      '<article class="result-card compact-card v47-result-card" data-result-id="' + escapeHtml(r.id) + '">' +
        '<div class="result-image v47-result-media">' + photo + '</div>' +
        '<div class="result-main compact-main v47-result-main">' +
          '<div class="result-top v47-result-top">' +
            '<div class="title-wrap">' +
              '<div class="v47-eyebrow">' + escapeHtml(regionLine || '일본') + '</div>' +
              '<h3 class="result-title clamp-2">' + escapeHtml(r.displayName) + '</h3>' +
              '<div class="subline clamp-2">' + escapeHtml(foodLine || '식당 정보') + '</div>' +
            '</div>' +
            '<div class="badge-row v47-badge-row">' + compactBadgesV47(r) + '</div>' +
          '</div>' +
          '<div class="v47-signal-grid">' +
            signalV47('기준 장소', distanceText, 'distance') +
            signalV47('숙소 기준', hotelText, 'distance') +
            signalV47('도보 추정', walkText, 'time') +
            signalV47('추천 시간', slots, 'food') +
          '</div>' +
          '<div class="v47-reason-grid">' +
            '<p><strong>추천 이유</strong><span>' + escapeHtml(good) + '</span></p>' +
            '<p><strong>체크 포인트</strong><span>' + escapeHtml(caution) + '</span></p>' +
          '</div>' +
          (r.adminNote ? '<div class="compact-note v47-admin-note"><strong>운영자 메모</strong><span>' + escapeHtml(r.adminNote) + '</span></div>' : '') +
          '<div class="action-row compact-top v47-card-actions">' +
            '<button class="btn btn-primary result-add" data-id="' + escapeHtml(r.id) + '">일정에 추가</button>' +
            actionLinkV47('길찾기', baseDirections, 'btn-secondary') +
            actionLinkV47('숙소에서', hotelDirections, 'btn-secondary') +
            actionLinkV47('식당 보기', r.reservationUrl || r.url || '', 'btn-ghost-sm') +
            '<button class="btn btn-ghost-sm result-recommend-meal" data-id="' + escapeHtml(r.id) + '">시간 선택</button>' +
            (state.adminSession && state.adminSession.loggedIn ? '<button class="btn btn-admin result-admin-recommend" data-id="' + escapeHtml(r.id) + '">' + (r.adminNote ? '추천 수정' : '운영자 추천') + '</button>' : '') +
          '</div>' +
        '</div>' +
      '</article>';
  }

  renderResultCardHtml = renderResultCardV47;

  function syncDockLabelV47() {
    var labels = {
      'dock-search-view': '검색',
      'dock-plan-view': '일정표',
      'dock-curated-view': '추천',
      'dock-content-view': '가이드',
      'dock-community-view': '커뮤니티',
      'dock-admin-view': '관리자',
    };
  ...
  function renderPlanShareTextV47() {
    var plan = Array.isArray(state.plan) ? state.plan : [];
    var lines = ['도모구르메 여행 식사 일정'];
    var ctx = state.planContext || {};
    if (ctx.region) lines.push('지역: ' + ctx.region);
    plan.forEach(function (day, index) {
      lines.push('');
      lines.push('Day ' + (index + 1));
      if (day.hotel) lines.push('숙소: ' + formatPointName(day.hotel));
      if (Array.isArray(day.visits) && day.visits.length) {
        lines.push('방문지: ' + day.visits.map(formatPointName).join(' → '));
      }
      [['아침', day.breakfast], ['점심', day.lunch], ['저녁', day.dinner]].forEach(function (row) {
        if (row[1]) lines.push(row[0] + ': ' + (row[1].restaurantName || '미정') + (row[1].note ? ' - ' + row[1].note : ''));
      });
    });
    return lines.join('\n').trim();
  }

  function installPlanShareButtonV47() {
    var actions = document.querySelector('#plan-view .page-header-actions');
    if (!actions || q('plan-copy-button')) return;
    var button = document.createElement('button');
    button.id = 'plan-copy-button';
    button.className = 'btn btn-outline-sm';
    button.type = 'button';
    button.textContent = '일정 요약 복사';
    button.addEventListener('click', function () {
      var text = renderPlanShareTextV47();
      var done = function () {
        var warnings = q('plan-warnings');
        if (warnings) warnings.innerHTML = '<div class="message-box success">일정 요약을 복사했습니다.</div>';
      };
  ...
  function refineMetaTextV47() {
    var meta = q('results-meta');
    if (meta && (!state.results || !state.results.length)) {
      meta.textContent = '지역과 음식 키워드를 입력하면 검색 결과와 추천 동선을 보여드립니다.';
    }
  }

  function buildDemoFallbackV47(query) {
    var rows = Array.isArray(window.DEMO_RESTAURANTS) ? window.DEMO_RESTAURANTS : [];
    if (!rows.length) return [];
    var region = normalize(query && query.region || '');
    var food = normalize(query && query.food || '');
    var filtered = rows.filter(function (item) {
      var hay = normalize([
        item.regionKo, item.regionOriginal, item.areaKo, item.areaOriginal,
        item.nameKo, item.nameOriginal, item.genreKo, item.genreOriginal,
        item.subGenreKo, item.subGenreOriginal, item.signatureMenu, item.catchCopyKo
      ].join(' '));
      var regionOk = !region || hay.includes(region) || region.includes(normalize(item.regionKo || '')) || region.includes(normalize(item.regionOriginal || ''));
      var foodOk = !food || hay.includes(food);
      return regionOk && foodOk;
    });
    if (!filtered.length && region) {
      filtered = rows.filter(function (item) {
        var hay = normalize([item.regionKo, item.regionOriginal, item.areaKo, item.areaOriginal].join(' '));
        return hay.includes(region) || region.includes(normalize(item.regionKo || '')) || region.includes(normalize(item.regionOriginal || ''));
      });
    }
    ...
    return filtered.map(function (item) {
      ...
      copy.searchText = copy.searchText || [
        copy.nameKo, copy.nameOriginal, copy.genreKo, copy.genreOriginal,
        copy.subGenreKo, copy.signatureMenu, copy.regionKo, copy.areaKo, copy.catchCopyKo
      ].filter(Boolean).join(' | ');
      return augmentRestaurant(copy, state.basePoint, query && query.food || '');
    }).filter(function (item) {
      return !isLikelyClosedRestaurant(item);
    }).sort(sortRestaurantsForDisplay(query && query.food || '', query && query.sort || 'food'));
  }

  var previousSearchV47 = search;
  search = function (options) {
    return Promise.resolve(previousSearchV47.apply(this, arguments)).then(function (result) {
      var query = state.currentQuery || syncQueryFromInputs(true);
      if ((!state.results || !state.results.length) && Array.isArray(window.DEMO_RESTAURANTS) && window.DEMO_RESTAURANTS.length) {
        var fallback = buildDemoFallbackV47(query);
        if (fallback.length) {
          state.pool = fallback.slice();
          state.results = fallback.slice();
          state.sourceLabel = '데모 데이터';
          state.pagination.page = 1;
          renderEverything();
          showFeedback('실시간 검색 결과가 없어서 데모 데이터로 화면을 보여드립니다. 배포 환경에서는 Hot Pepper 결과가 우선 사용됩니다.', 'info');
        }
      }
      return result;
    });
  ...
  var previousRenderResultsV47 = renderResults;
  renderResults = function () {
    var out = previousRenderResultsV47.apply(this, arguments);
    refineMetaTextV47();
    return out;
  };

  var previousRenderPlanV47 = renderPlan;
  renderPlan = function () {
    var out = previousRenderPlanV47.apply(this, arguments);
    installPlanShareButtonV47();
    return out;
  };
...
/* ===== END V47 APP EXPERIENCE REDESIGN ===== */

/* ===== V48 PRODUCT WORKSPACE REDESIGN ===== */
(function () {
  function q(id) { return document.getElementById(id); }
  ...
  function mealCountV48() {
    return (state.plan || []).reduce(function (sum, day) {
      return sum + ['breakfast', 'lunch', 'dinner'].filter(function (slot) { return Boolean(day && day[slot]); }).length;
    }, 0);
  }

  function planTotalV48() {
    return Math.max(1, getEffectivePlanDays ? getEffectivePlanDays() : ((state.plan || []).length || 1)) * 3;
  }

  function statV48(label, value) {
    return '<span class="v48-stat"><strong>' + escapeHtml(String(value)) + '</strong><em>' + escapeHtml(label) + '</em></span>';
  }

  function viewMetaV48(view) {
    var filled = mealCountV48();
    var total = planTotalV48();
    var region = (state.currentQuery && state.currentQuery.region) || (state.planContext && state.planContext.region) || '일본';
    var base = (state.currentQuery && state.currentQuery.base) || (state.planContext && state.planContext.base) || '';
    var food = (state.currentQuery && state.currentQuery.food) || '';
    var map = {
      search: {
        kicker: 'Search Studio',
        title: '맛집을 찾고, 비교하고, 바로 일정에 담기',
        copy: '지역과 음식만 넣어도 결과를 동선 기준으로 정리합니다.',
        stats: [
          statV48('검색 결과', (state.results || []).length + '곳'),
          statV48('기준 지역', region || '미설정'),
          statV48('일정 채움', filled + '/' + total)
        ],
        actions: '<button class="btn btn-primary v48-action" type="button" data-v48-run-search>검색 실행</button><button class="btn btn-secondary-sm v48-action" type="button" data-v48-open-plan>일정표</button>'
      },
      plan: {
        kicker: 'Route Board',
        title: '아침부터 저녁까지 한 눈에 보는 식사 동선',
        copy: '빈 시간대는 AI 보강으로 채우고, 식당은 바로 다시 비교할 수 있습니다.',
        stats: [
          statV48('여행 일수', Math.max(1, getEffectivePlanDays ? getEffectivePlanDays() : (state.plan || []).length || 1) + '일'),
          statV48('채운 식사', filled + '칸'),
          statV48('남은 식사', Math.max(0, total - filled) + '칸')
        ],
        actions: '<button class="btn btn-ai-sm v48-action" type="button" data-v48-open-ai>AI 일정</button><button class="btn btn-secondary-sm v48-action" type="button" data-v48-open-search>검색</button>'
      },
      curated: {
        kicker: 'Curated Picks',
        title: '운영자가 모아둔 지역별 추천 식당',
        copy: '검색 결과가 애매할 때 바로 참고할 수 있는 직접 저장 후보입니다.',
        stats: [
          statV48('운영자 추천', ((state.recommendedRestaurants || []).length || (window.DEMO_RESTAURANTS || []).length || 0) + '곳'),
          statV48('직접 등록', (state.customRestaurants || []).length + '곳'),
          statV48('현재 지역', region || '전체')
        ],
        actions: '<button class="btn btn-secondary-sm v48-action" type="button" data-v48-refresh-curated>새로고침</button>'
      },
      content: {
        kicker: 'Travel Guides',
        title: '테마별 루트와 장소 가이드',
        copy: '장소 묶음과 전체 길찾기를 중심으로 가이드를 읽고 일정에 반영합니다.',
        stats: [
          statV48('가이드', (state.contentPosts || []).length + '개'),
          statV48('템플릿', (state.themeTemplates || []).length + '개'),
          statV48('테마 장소', (state.themePlaces || []).length + '곳')
        ],
        actions: '<button class="btn btn-secondary-sm v48-action" type="button" data-v48-open-plan>일정표</button>'
      },
      community: {
        kicker: 'Community Routes',
        title: '다른 여행자가 남긴 루트를 일정으로 가져오기',
        copy: '좋은 동선은 Google Maps 경로와 함께 확인하고 바로 내 일정에 담습니다.',
        stats: [
          statV48('공유 글', (state.communityPosts || []).length + '개'),
          statV48('로그인', state.currentUser ? '완료' : '필요'),
          statV48('관심 지역', region || base || '전체')
        ],
        actions: '<button class="btn btn-primary v48-action" type="button" data-v48-open-compose>글쓰기</button><button class="btn btn-secondary-sm v48-action" type="button" data-v48-open-search>검색</button>'
      }
    };
    return map[view] || null;
  }

  function installIntroV48(view) {
    var root = q(view + '-view');
    var meta = viewMetaV48(view);
    if (!root || !meta) return;
    var id = 'v48-' + view + '-intro';
    var box = q(id);
    if (!box) {
      box = document.createElement('section');
      box.id = id;
      box.className = 'v48-view-intro';
      root.insertBefore(box, root.firstChild);
    }
    box.innerHTML =
      '<div class="v48-intro-copy">' +
        '<p>' + escapeHtml(meta.kicker) + '</p>' +
        '<h2>' + escapeHtml(meta.title) + '</h2>' +
        '<span>' + escapeHtml(meta.copy) + '</span>' +
      '</div>' +
      '<div class="v48-intro-side">' +
        '<div class="v48-stat-row">' + meta.stats.join('') + '</div>' +
        '<div class="v48-intro-actions">' + meta.actions + '</div>' +
      '</div>';
    bindIntroActionsV48(box);
  }

  function bindIntroActionsV48(root) {
    root.querySelectorAll('[data-v48-run-search]').forEach(function (button) {
      button.onclick = function () { if (typeof search === 'function') search({ silent: true }); };
    });
    root.querySelectorAll('[data-v48-open-plan]').forEach(function (button) {
      button.onclick = function () { setView('plan'); };
    });
    root.querySelectorAll('[data-v48-open-search]').forEach(function (button) {
      button.onclick = function () { setView('search'); };
    });
    root.querySelectorAll('[data-v48-open-ai]').forEach(function (button) {
      button.onclick = function () { var ai = q('auto-plan-button'); if (ai) ai.click(); };
    });
    root.querySelectorAll('[data-v48-refresh-curated]').forEach(function (button) {
      button.onclick = function () { var refresh = q('curated-refresh-button'); if (refresh) refresh.click(); };
    });
    root.querySelectorAll('[data-v48-open-compose]').forEach(function (button) {
      button.onclick = function () {
        var toggle = q('community-compose-toggle');
        if (toggle) toggle.click();
        var card = document.querySelector('.community-write-card');
        if (card) card.scrollIntoView({ behavior: 'smooth', block: 'start' });
      };
    });
  }

  function syncShellV48() {
    if (document.body) document.body.classList.add('domo-v48');
    var labels = {
      'nav-search-view': ['찾기', '검색'],
      'nav-plan-view': ['일정', '일정표'],
      'nav-curated-view': ['픽', '추천'],
      'nav-content-view': ['루트', '가이드'],
      'nav-community-view': ['공유', '커뮤니티'],
      'nav-admin-view': ['관리', '관리자'],
      'dock-search-view': ['찾기', '검색'],
      'dock-plan-view': ['일정', '일정표'],
      'dock-curated-view': ['픽', '추천'],
      'dock-content-view': ['루트', '가이드'],
      'dock-community-view': ['공유', '커뮤니티'],
      'dock-admin-view': ['관리', '관리자']
    };
    Object.keys(labels).forEach(function (id) {
      var node = q(id);
      if (!node) return;
      node.setAttribute('data-chip', labels[id][0]);
      node.textContent = labels[id][1];
    });
  }

  function refreshWorkspaceV48() {
    syncShellV48();
    ['search', 'plan', 'curated', 'content', 'community'].forEach(installIntroV48);
    if (document.body) document.body.setAttribute('data-current-view', state.uiView || 'search');
  }

  function linkV48(label, href, className) {
    if (!href) return '';
    return '<a class="btn ' + escapeHtml(className || 'btn-secondary-sm') + '" target="_blank" rel="noopener noreferrer" href="' + escapeHtml(href) + '">' + escapeHtml(label) + '</a>';
  }

  function signalV48(label, value) {
    if (!value) return '';
    return '<span class="v48-signal"><em>' + escapeHtml(label) + '</em><strong>' + escapeHtml(value) + '</strong></span>';
  }

  function renderResultCardV48(r) {
    var name = r.displayName || r.nameKo || r.nameOriginal || '이름 없는 식당';
    var original = r.nameOriginal && r.nameOriginal !== name ? r.nameOriginal : '';
    var regionLine = [r.regionKo || r.regionOriginal, r.areaKo || r.areaOriginal].filter(Boolean).join(' · ') || '일본';
    var foodLine = [r.genreKo || r.genreOriginal, r.subGenreKo || r.subGenreOriginal, r.signatureMenu].filter(Boolean).slice(0, 3).join(' · ') || '식당';
    var score = asNumber(r.travelScore, 3).toFixed(1);
    var distance = r.distanceKm != null ? asNumber(r.distanceKm, 0).toFixed(2) + 'km' : '';
    var hotelDistance = r.hotelDistanceKm != null ? asNumber(r.hotelDistanceKm, 0).toFixed(2) + 'km' : '';
    var walk = r.estimatedWalkMinutes ? (typeof formatMinutes === 'function' ? formatMinutes(r.estimatedWalkMinutes) : (r.estimatedWalkMinutes + '분')) : '';
    var slots = Array.isArray(r.recommendedSlots) && r.recommendedSlots.length ? r.recommendedSlots.join(' / ') : '확인 필요';
    var reason = (Array.isArray(r.goodPoints) && r.goodPoints[0]) || r.catchCopyKo || r.catchCopy || '현재 검색 조건에서 동선과 메뉴를 함께 비교하기 좋은 후보입니다.';
    var caution = (Array.isArray(r.badPoints) && r.badPoints[0]) || '방문 전 영업시간과 휴무를 다시 확인해 주세요.';
    var baseDirections = buildDirectionsUrl(r, state.basePoint);
    var hotelDirections = state.hotelPoint ? buildDirectionsUrl(r, state.hotelPoint) : '';
    var photo = r.photo
      ? '<img src="' + escapeHtml(r.photo) + '" alt="' + escapeHtml(name) + '" />'
      : '<img src="/assets/domo-gourmet-hero.png" alt="" />';
    var badges = [
      '<span class="badge score">추천 ' + escapeHtml(score) + '</span>',
      r.adminNote ? '<span class="badge admin">운영자 픽</span>' : '',
      r.foodExact || r.nameExact ? '<span class="badge match">검색어 일치</span>' : ''
    ].filter(Boolean).join('');

    return '' +
      '<article class="result-card compact-card v48-result-card" data-result-id="' + escapeHtml(r.id) + '">' +
        '<div class="result-image v48-result-photo">' +
          photo +
          '<div class="v48-score-chip"><strong>' + escapeHtml(score) + '</strong><span>AI score</span></div>' +
        '</div>' +
        '<div class="result-main compact-main v48-result-body">' +
          '<div class="v48-result-head">' +
            '<div class="title-wrap">' +
              '<div class="v48-eyebrow">' + escapeHtml(regionLine) + '</div>' +
              '<h3 class="result-title clamp-2">' + escapeHtml(name) + '</h3>' +
              (original ? '<div class="subline clamp-1">' + escapeHtml(original) + '</div>' : '') +
              '<div class="v48-food-line">' + escapeHtml(foodLine) + '</div>' +
            '</div>' +
            '<div class="badge-row v48-badge-row">' + badges + '</div>' +
          '</div>' +
          '<div class="v48-signal-grid">' +
            signalV48('기준', distance) +
            signalV48('숙소', hotelDistance) +
            signalV48('도보', walk) +
            signalV48('시간', slots) +
          '</div>' +
          '<div class="v48-insight-row">' +
            '<p><strong>왜 추천?</strong><span>' + escapeHtml(reason) + '</span></p>' +
            '<p><strong>체크</strong><span>' + escapeHtml(caution) + '</span></p>' +
          '</div>' +
          (r.adminNote ? '<p class="v48-admin-note"><strong>운영자 메모</strong><span>' + escapeHtml(r.adminNote) + '</span></p>' : '') +
          '<div class="action-row compact-top v48-card-actions">' +
            '<button class="btn btn-primary result-add" data-id="' + escapeHtml(r.id) + '">일정에 추가</button>' +
            linkV48('길찾기', baseDirections, 'btn-secondary-sm') +
            linkV48('숙소에서', hotelDirections, 'btn-secondary-sm') +
            linkV48('식당 보기', r.reservationUrl || r.url || '', 'btn-ghost-sm') +
            '<button class="btn btn-ghost-sm result-recommend-meal" data-id="' + escapeHtml(r.id) + '">시간 선택</button>' +
            (state.adminSession && state.adminSession.loggedIn ? '<button class="btn btn-admin result-admin-recommend" data-id="' + escapeHtml(r.id) + '">' + (r.adminNote ? '추천 수정' : '운영자 추천') + '</button>' : '') +
          '</div>' +
        '</div>' +
  ...
  renderResultCardHtml = renderResultCardV48;

  renderMealLine = function (label, item) {
    var slotClass = label.indexOf('아침') >= 0 ? 'breakfast' : label.indexOf('점심') >= 0 ? 'lunch' : 'dinner';
    if (!item) {
      return '<div class="route-meal v48-meal empty ' + slotClass + '">' +
        '<strong class="route-meal-slot">' + escapeHtml(label) + '</strong>' +
        '<span class="v48-meal-name">비어 있음</span>' +
        '<small>검색 결과에서 추가</small>' +
        '<div class="route-inline-actions"></div>' +
      '</div>';
    }
    return '<div class="route-meal v48-meal filled ' + slotClass + '">' +
      '<strong class="route-meal-slot">' + escapeHtml(label) + '</strong>' +
      '<span class="v48-meal-name">' + escapeHtml(item.restaurantName + (item.restaurantType ? ' · ' + item.restaurantType : '')) + '</span>' +
      (item.note ? '<small>' + escapeHtml(item.note) + '</small>' : '<small>일정에 담긴 식당</small>') +
      '<div class="route-inline-actions"><button class="small-button" data-open-restaurant="' + escapeHtml(item.restaurantId) + '" data-open-url="' + escapeHtml(item.restaurantUrl || '') + '" data-open-name="' + escapeHtml(item.restaurantName || '') + '">식당 보기</button></div>' +
    '</div>';
  };

  renderPlanDayCard = function (day, dayNumber, date) {
    var hotel = day.hotel || getPlanContextHotelPoint() || null;
    var visits = Array.isArray(day.visits) ? day.visits : [];
    var filled = ['breakfast', 'lunch', 'dinner'].filter(function (slot) { return Boolean(day[slot]); }).length;
    var visitHtml = visits.slice(0, 4).map(function (visit, idx) {
      return '<span class="route-chip"><strong>' + (idx + 1) + '</strong>' + escapeHtml(formatPointName(visit)) + '</span>';
    }).join('');
    var breakfastPoint = day.breakfast && day.breakfast.restaurantLat && day.breakfast.restaurantLng
      ? { lat: day.breakfast.restaurantLat, lng: day.breakfast.restaurantLng, nameKo: day.breakfast.restaurantName, nameOriginal: day.breakfast.restaurantName }
      : null;
    ...
    var dinnerPoint = day.dinner && day.dinner.restaurantLat && day.dinner.restaurantLng
      ? { lat: day.dinner.restaurantLat, lng: day.dinner.restaurantLng, nameKo: day.dinner.restaurantName, nameOriginal: day.dinner.restaurantName }
      : null;
    var morningVisit = visits[0] || null;
    var afternoonVisit = visits[1] || null;
    var steps = [];
    if (hotel && breakfastPoint) steps.push(renderRouteStep('숙소 → 아침', hotel, breakfastPoint));
    if (breakfastPoint && morningVisit) steps.push(renderRouteStep('아침 → 오전', breakfastPoint, morningVisit));
    if (morningVisit && lunchPoint) steps.push(renderRouteStep('오전 → 점심', morningVisit, lunchPoint));
    if (lunchPoint && afternoonVisit) steps.push(renderRouteStep('점심 → 오후', lunchPoint, afternoonVisit));
    if (afternoonVisit && dinnerPoint) steps.push(renderRouteStep('오후 → 저녁', afternoonVisit, dinnerPoint));
    if (dinnerPoint && hotel) steps.push(renderRouteStep('저녁 → 숙소', dinnerPoint, hotel));

    return '<article class="day-route-card v48-day-card">' +
      '<div class="day-route-head v48-day-head"><div><span>Day ' + dayNumber + '</span><strong>' + escapeHtml(formatDisplayDate(date)) + '</strong></div><div class="v48-day-actions"><em>' + filled + '/3 식사</em><button class="small-button" type="button" data-reroll-day="' + (dayNumber - 1) + '">다시 추천</button></div></div>' +
      '<div class="v48-route-context">' +
        (hotel ? '<span class="route-chip hotel"><strong>숙소</strong>' + escapeHtml(formatPointName(hotel)) + '</span>' : '<span class="route-chip muted-chip"><strong>숙소</strong>미설정</span>') +
        (visitHtml ? '<div class="route-chip-row">' + visitHtml + '</div>' : '<div class="route-chip-row"><span class="route-chip muted-chip"><strong>방문지</strong>입력하면 동선이 좋아집니다</span></div>') +
      '</div>' +
      '<div class="route-meal-list v48-meal-grid">' + renderMealLine('아침', day.breakfast) + renderMealLine('점심', day.lunch) + renderMealLine('저녁', day.dinner) + '</div>' +
      (steps.length ? '<div class="route-step-list v48-step-list">' + steps.join('') + '</div>' : '<div class="v48-route-empty">숙소와 방문지를 넣으면 구간별 Google Maps 길찾기가 표시됩니다.</div>') +
    '</article>';
  };

  renderCuratedCard = function (item, adminMode) {
    var name = item.displayName || item.nameKo || item.nameOriginal || item.id;
    var regionLine = [item.regionKo || item.regionOriginal, item.areaKo || item.areaOriginal].filter(Boolean).join(' · ') || '일본';
    var foodLine = [item.genreKo || item.genreOriginal, item.signatureMenu].filter(Boolean).join(' · ') || '추천 식당';
    var photo = item.photo ? '<img src="' + escapeHtml(item.photo) + '" alt="' + escapeHtml(name) + '" />' : '<img src="/assets/domo-gourmet-hero.png" alt="" />';
    return '<article class="curated-card v48-curated-card">' +
      '<div class="v48-curated-media">' + photo + '</div>' +
      '<div class="v48-curated-body">' +
        '<div class="v48-eyebrow">' + escapeHtml(regionLine) + '</div>' +
        '<h3 class="result-title">' + escapeHtml(name) + '</h3>' +
        '<p class="subline">' + escapeHtml(foodLine) + '</p>' +
        (item.adminNote ? '<p class="v48-admin-note"><strong>추천 메모</strong><span>' + escapeHtml(item.adminNote) + '</span></p>' : '') +
        '<div class="action-row compact-top">' +
          '<a class="btn btn-secondary-sm" target="_blank" rel="noopener noreferrer" href="' + escapeHtml(buildDirectionsUrl(item, state.basePoint || state.hotelPoint || null)) + '">길찾기</a>' +
          ((item.url || item.reservationUrl) ? '<a class="btn btn-ghost-sm" target="_blank" rel="noopener noreferrer" href="' + escapeHtml(item.url || item.reservationUrl) + '">식당 보기</a>' : '') +
          (adminMode ? '<button class="btn btn-admin result-admin-recommend" data-id="' + escapeHtml(item.id) + '" type="button">추천 수정</button>' : '') +
        '</div>' +
      '</div>' +
    '</article>';
  ...
  renderPostCardBase = function (post, communityMode) {
    var points = Array.isArray(post.routePlaces) ? post.routePlaces : [];
    var themes = Array.isArray(post.themes) ? post.themes : [];
    var region = post.regionKo || post.regionOriginal || '';
    var title = post.title || '제목 없음';
    ...
    var isMine = communityMode && state.currentUser && (state.currentUser.role === 'admin' || (state.currentUser.email && state.currentUser.email === post.authorEmail));
    var authorText = communityMode ? (post.nickname || '익명') : '관리자';
    var routeLink = points.length > 1 ? '<a class="btn btn-ghost-sm small-inline" target="_blank" rel="noopener noreferrer" href="' + escapeHtml(buildMultiStopDirectionsUrl(points)) + '">전체 루트 열기</a>' : (points[0] && points[0].mapLink ? '<a class="btn btn-ghost-sm small-inline" target="_blank" rel="noopener noreferrer" href="' + escapeHtml(points[0].mapLink) + '">지도 열기</a>' : '');
    var headerBadges = (post.category ? '<span class="badge soft">' + escapeHtml(post.category) + '</span>' : '') + themes.slice(0, 4).map(function (theme) { return badge(theme); }).join('');
    var placePreview = points.slice(0, 5).map(function (point, index) {
      var label = point.displayNameKo || point.nameKo || point.nameOriginal || point.query || ('장소 ' + (index + 1));
      return '<span class="route-pill"><strong>' + (index + 1) + '</strong><span>' + escapeHtml(label) + '</span></span>';
    }).join('');
    return '<article class="content-post-card community-card v48-post-card">' +
      '<div class="v48-post-rail"><span>' + escapeHtml(communityMode ? '공유' : '가이드') + '</span><strong>' + escapeHtml(String(points.length || 0)) + '</strong><em>stops</em></div>' +
      '<div class="content-post-body v48-post-body">' +
        '<div class="community-card-top v48-post-top">' +
          '<div>' +
            '<div class="badge-row">' + headerBadges + '</div>' +
            '<h3 class="result-title">' + escapeHtml(title) + '</h3>' +
            '<div class="subline">' + escapeHtml([authorText, region, dateText].filter(Boolean).join(' · ')) + '</div>' +
          '</div>' +
          (isMine ? '<div class="community-owner-actions"><button class="small-button danger" data-community-delete="' + escapeHtml(post.id) + '">삭제</button></div>' : '') +
        '</div>' +
        (description ? '<p class="content-summary">' + escapeHtml(description) + '</p>' : '') +
        (points.length ? '<div class="route-overview-card v48-route-overview">' + (typeof renderMiniRouteSvg === 'function' ? renderMiniRouteSvg(points) : '') + '<div class="route-pill-row">' + placePreview + '</div></div>' : '') +
        '<div class="action-row compact-top">' +
          routeLink +
          (communityMode ? '<button class="btn btn-secondary-sm btn-mini" type="button" data-add-community-route="' + escapeHtml(post.id) + '">일정에 추가</button>' : '') +
        '</div>' +
      '</div>' +
    '</article>';
  };

  function startSearchV48(region, base, food) {
    var regionInput = q('region-input');
    var baseInput = q('base-input');
    var foodInput = q('food-input');
    if (regionInput) regionInput.value = region || '';
  ...
  function bindSampleCardsV48(root) {
    (root || document).querySelectorAll('[data-v48-sample]').forEach(function (button) {
      if (button.__v48SampleBound) return;
      button.__v48SampleBound = true;
      button.addEventListener('click', function () {
        startSearchV48(
          button.getAttribute('data-region') || '',
          button.getAttribute('data-base') || '',
  ...
  function renderCuratedDemoV48() {
    if (!els.curatedList) return;
    if ((state.recommendedRestaurants || []).length || (state.customRestaurants || []).length) return;
    var rows = Array.isArray(window.DEMO_RESTAURANTS) ? window.DEMO_RESTAURANTS.slice(0, 8) : [];
    if (!rows.length) return;
    var grouped = {};
    rows.forEach(function (item) {
      var region = item.regionKo || item.regionOriginal || '일본';
      if (!grouped[region]) grouped[region] = [];
      grouped[region].push(typeof augmentRestaurant === 'function' ? augmentRestaurant(Object.assign({}, item), state.basePoint, state.currentQuery && state.currentQuery.food || '') : Object.assign({}, item));
    });
    els.curatedList.innerHTML = Object.keys(grouped).map(function (region) {
      return '<section class="curated-region"><div class="result-section-head"><strong>' + escapeHtml(region) + '</strong><span>베타 데모 추천</span></div><div class="result-section-list">' + grouped[region].map(function (item) { return renderCuratedCard(item, false); }).join('') + '</div></section>';
    }).join('');
  }

  function guideSampleV48(title, region, base, food, copy) {
    return '<article class="content-post-card v48-post-card v48-sample-card">' +
      '<div class="v48-post-rail"><span>샘플</span><strong>3</strong><em>steps</em></div>' +
      '<div class="content-post-body v48-post-body">' +
        '<div class="badge-row"><span class="badge soft">' + escapeHtml(region) + '</span><span class="badge match">' + escapeHtml(food) + '</span></div>' +
        '<h3 class="result-title">' + escapeHtml(title) + '</h3>' +
        '<p class="content-summary">' + escapeHtml(copy) + '</p>' +
        '<div class="route-pill-row">' +
          '<span class="route-pill"><strong>1</strong><span>' + escapeHtml(base) + '</span></span>' +
          '<span class="route-pill"><strong>2</strong><span>' + escapeHtml(food + ' 검색') + '</span></span>' +
          '<span class="route-pill"><strong>3</strong><span>일정 담기</span></span>' +
        '</div>' +
        '<div class="action-row compact-top"><button class="btn btn-secondary-sm" type="button" data-v48-sample data-region="' + escapeHtml(region) + '" data-base="' + escapeHtml(base) + '" data-food="' + escapeHtml(food) + '">이 루트로 검색</button></div>' +
      '</div>' +
    '</article>';
  }

  function renderContentSamplesV48() {
    if (!els.contentList || (state.contentPosts || []).length) return;
    var templateSection = q('template-schedule-section');
    if (templateSection && !(state.themeTemplates || []).length) templateSection.style.display = 'none';
    els.contentList.innerHTML =
      guideSampleV48('시부야에서 라멘으로 시작하는 첫날 저녁', '도쿄', '시부야', '라멘', '도착 첫날에는 이동이 짧고 늦게까지 열 가능성이 높은 후보를 먼저 비교합니다.') +
      guideSampleV48('난바·도톤보리 오코노미야키 동선', '오사카', '난바', '오코노미야키', '쇼핑과 야경 동선을 유지하면서 점심 또는 저녁 식당을 빠르게 고르는 흐름입니다.') +
      guideSampleV48('아키하바라 카레와 애니 쇼핑 반나절', '도쿄', '아키하바라', '카레', '방문지와 식사 후보를 한 화면에 놓고 걷는 시간을 줄이는 예시입니다.');
    bindSampleCardsV48(els.contentList);
  }

  function renderCommunitySamplesV48() {
    if (!els.communityList || (state.communityPosts || []).length) return;
    els.communityList.innerHTML =
      '<div class="empty-state v48-community-empty">' +
        '<strong>아직 공유된 루트가 없습니다.</strong>' +
        '<span>베타 기간에는 첫 루트가 곧 콘텐츠가 됩니다. 장소 1개만 넣어도 다른 여행자가 일정에 가져갈 수 있어요.</span>' +
        '<div class="action-row compact-top"><button class="btn btn-primary" type="button" data-v48-open-compose>첫 루트 쓰기</button><button class="btn btn-secondary-sm" type="button" data-v48-sample data-region="도쿄" data-base="하라주쿠" data-food="카페">예시 검색</button></div>' +
      '</div>';
    bindIntroActionsV48(els.communityList);
    bindSampleCardsV48(els.communityList);
  }

  var previousSetViewV48 = setView;
  setView = function (view, updateHash) {
    var out = previousSetViewV48.apply(this, arguments);
    refreshWorkspaceV48();
    return out;
  };

  var previousRenderResultsV48 = renderResults;
  renderResults = function () {
    var out = previousRenderResultsV48.apply(this, arguments);
    refreshWorkspaceV48();
    return out;
  };

  var previousRenderPlanV48 = renderPlan;
  renderPlan = function () {
    var out = previousRenderPlanV48.apply(this, arguments);
    refreshWorkspaceV48();
    return out;
  };

  var previousRenderCuratedV48 = renderCuratedView;
  renderCuratedView = function () {
    var out = previousRenderCuratedV48.apply(this, arguments);
    renderCuratedDemoV48();
    refreshWorkspaceV48();
    return out;
  };

  var previousRenderContentV48 = renderContentView;
  renderContentView = function () {
    var out = previousRenderContentV48.apply(this, arguments);
    renderContentSamplesV48();
    refreshWorkspaceV48();
    return out;
  };

  var previousRenderCommunityV48 = renderCommunityView;
  renderCommunityView = function () {
    var out = previousRenderCommunityV48.apply(this, arguments);
    renderCommunitySamplesV48();
    refreshWorkspaceV48();
    return out;
  };
  ...
  document.addEventListener('DOMContentLoaded', function () {
    refreshWorkspaceV48();
    setTimeout(refreshWorkspaceV48, 250);
    setTimeout(refreshWorkspaceV48, 900);
  });

  if (document.readyState !== 'loading') {
    setTimeout(refreshWorkspaceV48, 0);
  }
})();
/* ===== END V48 PRODUCT WORKSPACE REDESIGN ===== */

/* ===== V49 ADMIN ROUTE GUIDE + SMART TEMPLATE PACKING ===== */
(function () {
  function q(id) { return document.getElementById(id); }

  function routeDurationV49(item) {
    var value = Number(item && (item.durationHours || item.routeDurationHours || item.hours || 0));
    if (Number.isFinite(value) && value > 0) return Math.min(24, Math.max(1, value));
    var count = Array.isArray(item && item.placeIds) ? item.placeIds.length : (Array.isArray(item && item.routePlaces) ? item.routePlaces.length : 0);
    if (count <= 2) return 3;
    if (count >= 5) return 8;
    return 6;
  }

  function routeScopeV49(item) {
    var scope = String(item && (item.routeScope || item.scope || '') || '').trim();
    if (scope) return scope;
    var hours = routeDurationV49(item);
    if (hours <= 3) return 'short';
    if (hours >= 8) return 'day';
    return 'half-day';
  }

  function routeScopeLabelV49(item) {
    var scope = routeScopeV49(item);
    var hours = routeDurationV49(item);
    if (scope === 'day') return '하루 코스';
    if (scope === 'short') return hours + '시간 루트';
    return '반나절 루트';
  }

  function preferredTimeLabelV49(value) {
    value = String(value || 'flex');
    if (value === 'morning') return '오전 추천';
    if (value === 'lunch') return '점심 전후';
    if (value === 'afternoon') return '오후 추천';
    if (value === 'evening') return '저녁 추천';
    return '시간 자유';
  }

  function ensureAdminRouteFieldsV49() {
    var templateAnchor = q('theme-template-foods');
    if (templateAnchor && !q('theme-template-duration')) {
      var wrap = document.createElement('div');
      wrap.id = 'theme-template-route-settings';
      wrap.className = 'v49-admin-route-settings full-span';
      wrap.innerHTML =
        '<label class="form-label"><span>루트 길이</span><select id="theme-template-duration"><option value="3">3시간 단기 루트</option><option value="5">5시간 반나절</option><option value="7" selected>7시간 넉넉한 반나절</option><option value="9">하루 코스</option></select></label>' +
        '<label class="form-label"><span>추천 시간대</span><select id="theme-template-preferred-time"><option value="flex" selected>상관없음</option><option value="morning">오전</option><option value="lunch">점심 전후</option><option value="afternoon">오후</option><option value="evening">저녁</option></select></label>' +
        '<label class="checkbox-row v49-ai-toggle"><input id="theme-template-ai-eligible" type="checkbox" checked /><span>AI 일정 만들 때 이 루트를 후보로 사용</span></label>';
      var label = templateAnchor.closest('.form-label') || templateAnchor.parentNode;
      label.parentNode.insertBefore(wrap, label.nextSibling);
    }

    var contentAnchor = q('content-summary');
    if (contentAnchor && !q('content-duration')) {
      var contentWrap = document.createElement('div');
      contentWrap.id = 'content-route-settings';
      contentWrap.className = 'v49-admin-route-settings full-span';
      contentWrap.innerHTML =
        '<label class="form-label"><span>루트 길이</span><select id="content-duration"><option value="0">가이드 전용</option><option value="3">3시간 단기 루트</option><option value="5">5시간 반나절</option><option value="7">7시간 넉넉한 반나절</option><option value="9">하루 코스</option></select></label>' +
        '<label class="form-label"><span>추천 시간대</span><select id="content-preferred-time"><option value="flex" selected>상관없음</option><option value="morning">오전</option><option value="lunch">점심 전후</option><option value="afternoon">오후</option><option value="evening">저녁</option></select></label>' +
        '<label class="checkbox-row v49-ai-toggle"><input id="content-ai-eligible" type="checkbox" /><span>AI 일정 만들 때 이 가이드 루트도 사용</span></label>';
      var contentLabel = contentAnchor.closest('.form-label') || contentAnchor.parentNode;
      contentLabel.parentNode.insertBefore(contentWrap, contentLabel.nextSibling);
    }
  }

  function readRouteSettingsV49(prefix) {
    var duration = q(prefix + '-duration');
    var preferred = q(prefix + '-preferred-time');
    var eligible = q(prefix + '-ai-eligible');
    var hours = Number(duration && duration.value || 0);
    return {
      durationHours: Number.isFinite(hours) && hours > 0 ? hours : 0,
      routeScope: hours >= 8 ? 'day' : hours <= 3 ? 'short' : 'half-day',
      preferredTime: preferred ? preferred.value : 'flex',
      aiEligible: eligible ? Boolean(eligible.checked) : false
    };
  }

  function setRouteSettingsV49(prefix, item) {
    var duration = q(prefix + '-duration');
    var preferred = q(prefix + '-preferred-time');
    var eligible = q(prefix + '-ai-eligible');
    if (duration) duration.value = String(item && (item.durationHours || item.routeDurationHours || '') || (prefix === 'theme-template' ? 7 : 0));
    if (preferred) preferred.value = String(item && (item.preferredTime || item.timeOfDay || '') || 'flex');
    if (eligible) eligible.checked = prefix === 'theme-template' ? item && item.aiEligible !== false : Boolean(item && item.aiEligible);
  }

  var previousBeginEditThemeTemplateV49 = beginEditThemeTemplate;
  beginEditThemeTemplate = function (id) {
    var out = previousBeginEditThemeTemplateV49.apply(this, arguments);
    ensureAdminRouteFieldsV49();
    var template = (state.themeTemplates || []).find(function (item) { return item.id === id; });
    setRouteSettingsV49('theme-template', template || {});
    return out;
  };

  var previousRenderAdminStateV49 = renderAdminState;
  renderAdminState = function () {
    var out = previousRenderAdminStateV49.apply(this, arguments);
    ensureAdminRouteFieldsV49();
    return out;
  };

  saveThemeTemplate = function () {
    ensureAdminRouteFieldsV49();
    var settings = readRouteSettingsV49('theme-template');
    var template = {
      id: state.themeTemplateEditingId || '',
      title: String((els.themeTemplateTitle && els.themeTemplateTitle.value) || '').trim(),
      regionKo: String((els.themeTemplateRegion && els.themeTemplateRegion.value) || '').trim(),
      ...
      wantedFoods: String((els.themeTemplateFoods && els.themeTemplateFoods.value) || '').split(',').map(function (item) { return item.trim(); }).filter(Boolean),
      placeIds: (state.themeTemplateDraft || []).map(function (item) { return item.id; }).filter(Boolean),
      durationHours: settings.durationHours || 7,
      routeScope: settings.routeScope || 'half-day',
      preferredTime: settings.preferredTime || 'flex',
      aiEligible: settings.aiEligible !== false,
      createdAt: (state.themeTemplates || []).find(function (item) { return item.id === state.themeTemplateEditingId; })?.createdAt || undefined
    };
    if (!template.title || !template.placeIds.length) {
      if (els.themeTemplateMessage) els.themeTemplateMessage.textContent = '템플릿 이름과 최소 1개 장소가 필요합니다.';
    ...
    postJson('/api/admin/theme-template', { template: template }).then(function (json) {
      ...
      ['themeTemplateTitle','themeTemplateRegion','themeTemplateThemes','themeTemplateFoods','themeTemplateDescription'].forEach(function (key) { if (els[key]) els[key].value = ''; });
      clearPickerValues('theme-template-themes');
      setRouteSettingsV49('theme-template', { durationHours: 7, preferredTime: 'flex', aiEligible: true });
      if (els.themeTemplateMessage) els.themeTemplateMessage.textContent = '테마 일정 템플릿을 저장했습니다. 루트 가이드와 AI 일정 후보에 반영됩니다.';
      ensureThemeTemplateToolbar();
      renderContentView();
    }).catch(function (error) {
      if (els.themeTemplateMessage) els.themeTemplateMessage.textContent = error && error.message ? error.message : '테마 일정 저장에 실패했습니다.';
  ...
  saveContentPost = function () {
    ensureAdminRouteFieldsV49();
    var settings = readRouteSettingsV49('content');
    var post = {
      title: String((els.contentTitle && els.contentTitle.value) || '').trim(),
      category: String((els.contentCategory && els.contentCategory.value) || '').trim(),
      regionKo: String((els.contentRegion && els.contentRegion.value) || '').trim(),
      ...
      routeScope: settings.routeScope || '',
      preferredTime: settings.preferredTime || 'flex',
      aiEligible: Boolean(settings.aiEligible && settings.durationHours)
    };
    if (!post.title || !post.routePlaces.length) {
      if (els.contentSaveMessage) els.contentSaveMessage.textContent = '제목과 최소 1개 경로 장소가 필요합니다.';
      return;
    ...
    postJson('/api/admin/content-post', { post: post }).then(function (json) {
      ...
      clearPickerValues('content-themes');
      clearPickerValues('content-category');
      setRouteSettingsV49('content', { durationHours: 0, preferredTime: 'flex', aiEligible: false });
      if (els.contentSaveMessage) els.contentSaveMessage.textContent = '루트 가이드를 저장했습니다.';
      setView('content');
    }).catch(function (error) {
      if (els.contentSaveMessage) els.contentSaveMessage.textContent = error && error.message ? error.message : '콘텐츠 저장에 실패했습니다.';
    });
  ...
  var previousBuildTemplateRoutePlacesV49 = buildTemplateRoutePlacesV25;
  buildTemplateRoutePlacesV25 = function (template) {
    if (Array.isArray(template && template.routePlaces) && template.routePlaces.length) {
      return uniquePointsStrict(template.routePlaces.map(function (place) {
        return pickThemeRoutePlace(place);
      }).filter(Boolean));
    }
    return previousBuildTemplateRoutePlacesV49.apply(this, arguments);
  };

  function templateToGuidePostV49(template) {
    var places = buildTemplateRoutePlacesV25(template);
    return {
      id: 'template:' + template.id,
      templateId: template.id,
      sourceType: 'template',
      title: template.title || '추천 일정',
      category: routeScopeLabelV49(template),
      summary: template.description || '관리자가 저장한 루트 템플릿입니다.',
      body: template.description || '',
      regionKo: template.regionKo || template.regionOriginal || '',
      themes: Array.isArray(template.themes) ? template.themes.slice() : [],
      routePlaces: places,
      durationHours: routeDurationV49(template),
      routeScope: routeScopeV49(template),
      preferredTime: template.preferredTime || 'flex',
      aiEligible: template.aiEligible !== false,
      wantedFoods: Array.isArray(template.wantedFoods) ? template.wantedFoods.slice() : [],
      updatedAt: template.updatedAt,
      createdAt: template.createdAt
    };
  }

  function contentToGuidePostV49(post) {
    return Object.assign({}, post, {
      sourceType: 'content',
      category: post.category || (post.durationHours ? routeScopeLabelV49(post) : '가이드'),
      durationHours: routeDurationV49(post),
      routeScope: post.routeScope || routeScopeV49(post),
      preferredTime: post.preferredTime || 'flex',
      aiEligible: Boolean(post.aiEligible)
    });
  }

  function getUnifiedGuidesV49() {
    var guides = [];
    (state.contentPosts || []).forEach(function (post) {
      if (post && Array.isArray(post.routePlaces) && post.routePlaces.length) guides.push(contentToGuidePostV49(post));
    });
    (state.themeTemplates || []).forEach(function (template) {
      var guide = templateToGuidePostV49(template);
      if (guide.routePlaces.length) guides.push(guide);
    });
  ...
  function renderRouteGuideCardV49(post) {
    var points = Array.isArray(post.routePlaces) ? post.routePlaces : [];
    var themes = Array.isArray(post.themes) ? post.themes : [];
    var routeLink = points.length > 1 ? '<a class="btn btn-ghost-sm small-inline" target="_blank" rel="noopener noreferrer" href="' + escapeHtml(buildMultiStopDirectionsUrl(points)) + '">전체 루트 열기</a>' : (points[0] && points[0].mapLink ? '<a class="btn btn-ghost-sm small-inline" target="_blank" rel="noopener noreferrer" href="' + escapeHtml(points[0].mapLink) + '">지도 열기</a>' : '');
    var placePreview = points.slice(0, 5).map(function (point, index) {
    ...
    var action = post.sourceType === 'template'
      ? '<button class="btn btn-secondary-sm btn-mini" type="button" data-add-template-day="' + escapeHtml(post.templateId) + '">일정에 추가</button>'
      : (post.aiEligible ? '<button class="btn btn-secondary-sm btn-mini" type="button" data-add-guide-route="' + escapeHtml(post.id) + '">일정에 추가</button>' : '');
    var badges = '<span class="badge soft">' + escapeHtml(post.category || '루트') + '</span>' +
      '<span class="badge match">' + escapeHtml(routeDurationV49(post) + '시간') + '</span>' +
      '<span class="badge soft">' + escapeHtml(preferredTimeLabelV49(post.preferredTime)) + '</span>' +
      (post.aiEligible ? '<span class="badge admin">AI 후보</span>' : '') +
      themes.slice(0, 3).map(function (theme) { return badge(theme); }).join('');
    return '<article class="content-post-card community-card v48-post-card v49-guide-card">' +
      '<div class="v48-post-rail"><span>' + escapeHtml(post.sourceType === 'template' ? '템플릿' : '가이드') + '</span><strong>' + escapeHtml(String(points.length || 0)) + '</strong><em>stops</em></div>' +
      '<div class="content-post-body v48-post-body">' +
        '<div class="community-card-top v48-post-top">' +
          '<div><div class="badge-row">' + badges + '</div><h3 class="result-title">' + escapeHtml(post.title || '루트 가이드') + '</h3><div class="subline">' + escapeHtml([post.regionKo || post.regionOriginal || '', post.sourceType === 'template' ? '일정 템플릿' : '관리자 가이드'].filter(Boolean).join(' · ')) + '</div></div>' +
        '</div>' +
        ((post.summary || post.body) ? '<p class="content-summary">' + escapeHtml(post.summary || post.body) + '</p>' : '') +
        (points.length ? '<div class="route-overview-card v48-route-overview">' + (typeof renderMiniRouteSvg === 'function' ? renderMiniRouteSvg(points) : '') + '<div class="route-pill-row">' + placePreview + '</div></div>' : '') +
        '<div class="action-row compact-top">' + routeLink + action + '</div>' +
      '</div>' +
    '</article>';
  }

  function renderUnifiedContentViewV49() {
    if (!els.contentList) return;
    var templateSection = q('template-schedule-section');
    ...
    var searchNeedle = normalize(state.contentFilters.search);
    var themeNeedle = normalize(state.contentFilters.theme);
    var rows = getUnifiedGuidesV49().filter(function (post) {
      var hay = normalize([post.title, post.summary, post.body, post.regionKo, post.category, post.sourceType].join(' '));
      var themes = normalize((Array.isArray(post.themes) ? post.themes.join(' ') : ''));
      var places = normalize((post.routePlaces || []).map(function (point) { return formatPointName(point); }).join(' '));
      if (searchNeedle && hay.indexOf(searchNeedle) < 0 && themes.indexOf(searchNeedle) < 0 && places.indexOf(searchNeedle) < 0) return false;
      if (themeNeedle && themes.indexOf(themeNeedle) < 0 && hay.indexOf(themeNeedle) < 0) return false;
      return true;
    });
    if (rows.length) {
      els.contentList.innerHTML = rows.map(renderRouteGuideCardV49).join('');
      bindGuideActionsV49(els.contentList);
      return;
    }
    els.contentList.innerHTML =
      '<div class="empty-state v48-community-empty">' +
        '<strong>아직 등록된 루트 가이드가 없습니다.</strong>' +
        '<span>관리자 페이지에서 장소를 저장한 뒤 루트 템플릿 또는 가이드 글로 묶으면 이 화면과 AI 일정 후보에 함께 반영됩니다.</span>' +
        '<div class="action-row compact-top"><button class="btn btn-secondary-sm" type="button" data-v49-admin-open>관리자에서 등록</button></div>' +
      '</div>';
    var adminOpen = els.contentList.querySelector('[data-v49-admin-open]');
    if (adminOpen) adminOpen.addEventListener('click', function () { setView('admin'); });
  }

  function routeTemplateFromGuideV49(post) {
    return {
      id: post.id,
      title: post.title,
      regionKo: post.regionKo || post.regionOriginal || '',
      description: post.summary || post.body || '',
      themes: Array.isArray(post.themes) ? post.themes.slice() : [],
      wantedFoods: Array.isArray(post.wantedFoods) ? post.wantedFoods.slice() : [],
      routePlaces: (post.routePlaces || []).map(function (item) { return Object.assign({}, item); }),
      durationHours: routeDurationV49(post),
      routeScope: routeScopeV49(post),
      preferredTime: post.preferredTime || 'flex',
      aiEligible: post.aiEligible !== false
    };
  }

  async function addRouteGuideToPlanV49(postId) {
    var post = getUnifiedGuidesV49().find(function (item) { return item.id === postId; });
    if (!post) return;
    ensurePlanContextFromSearch();
    state.plan = resizePlan(state.plan, getEffectivePlanDays());
    var index = findTargetDayIndexForTemplateV25();
    var usedRestaurantIds = collectUsedRestaurantIdsFromPlanV25();
    var payload = await buildTemplateDayV25(routeTemplateFromGuideV49(post), index, { region: post.regionKo || state.planContext.region || '' }, usedRestaurantIds);
    if (!payload) return;
    state.plan[index] = Object.assign({}, state.plan[index] || {}, payload.day);
    saveJson(STORAGE.plan, state.plan);
    renderPlan();
    renderPlanSummary();
    if (els.planWarnings) els.planWarnings.innerHTML = '<div class="message-box success">"' + escapeHtml(post.title) + '" 루트를 Day ' + (index + 1) + '에 추가했습니다.</div>';
    setView('plan');
  }

  function bindGuideActionsV49(scope) {
    (scope || document).querySelectorAll('[data-add-template-day]').forEach(function (button) {
      if (button.__v49Bound) return;
      button.__v49Bound = true;
      button.addEventListener('click', function () {
        addThemeTemplateToPlanV25(String(button.getAttribute('data-add-template-day') || '')).catch(function (error) {
          if (els.planWarnings) els.planWarnings.innerHTML = '<div class="message-box error">' + escapeHtml(error && error.message ? error.message : '추천 일정을 일정표에 추가하지 못했습니다.') + '</div>';
        });
      });
    });
    (scope || document).querySelectorAll('[data-add-guide-route]').forEach(function (button) {
      if (button.__v49Bound) return;
      button.__v49Bound = true;
      button.addEventListener('click', function () {
        addRouteGuideToPlanV49(String(button.getAttribute('data-add-guide-route') || '')).catch(function (error) {
          if (els.planWarnings) els.planWarnings.innerHTML = '<div class="message-box error">' + escapeHtml(error && error.message ? error.message : '루트를 일정표에 추가하지 못했습니다.') + '</div>';
        });
      });
    });
  }
  ...
  renderContentView = function () {
    renderUnifiedContentViewV49();
    if (typeof refreshWorkspaceV48 === 'function') refreshWorkspaceV48();
  };

  function getAiRouteTemplatesV49(region, selectedThemes) {
    var themes = Array.isArray(selectedThemes) ? selectedThemes.filter(Boolean) : [];
    var templates = (state.themeTemplates || []).filter(function (template) {
      if (!template || template.aiEligible === false) return false;
      var templateRegion = template.regionKo || template.regionOriginal || '';
      if (region && templateRegion && !regionMatchesLoose(region, templateRegion)) return false;
      if (!themes.length) return true;
      var templateThemes = Array.isArray(template.themes) ? template.themes : [];
      return templateThemes.some(function (theme) { return themes.indexOf(theme) >= 0; });
    });
    var guideTemplates = (state.contentPosts || []).filter(function (post) {
      if (!post || !post.aiEligible || !Array.isArray(post.routePlaces) || !post.routePlaces.length) return false;
      var postRegion = post.regionKo || post.regionOriginal || '';
      if (region && postRegion && !regionMatchesLoose(region, postRegion)) return false;
      if (!themes.length) return true;
      var postThemes = Array.isArray(post.themes) ? post.themes : [];
      return postThemes.some(function (theme) { return themes.indexOf(theme) >= 0; });
    }).map(routeTemplateFromGuideV49);
    return templates.concat(guideTemplates).sort(function (a, b) {
      var aTheme = Array.isArray(a.themes) ? a.themes.filter(function (theme) { return themes.indexOf(theme) >= 0; }).length : 0;
      var bTheme = Array.isArray(b.themes) ? b.themes.filter(function (theme) { return themes.indexOf(theme) >= 0; }).length : 0;
      if (bTheme !== aTheme) return bTheme - aTheme;
      return routeDurationV49(a) - routeDurationV49(b);
    });
  }

  function packRouteTemplatesV49(templates, days) {
    var buckets = [];
    (templates || []).forEach(function (template) {
      var hours = routeDurationV49(template);
      var placed = false;
      for (var i = 0; i < buckets.length; i += 1) {
        if (hours < 8 && buckets[i].hours + hours <= 8 && buckets[i].items.length < 2) {
          buckets[i].items.push(template);
          buckets[i].hours += hours;
          placed = true;
          break;
        }
      }
      if (!placed && buckets.length < days) {
        buckets.push({ hours: hours, items: [template] });
      }
    });
    return buckets;
  }

  function combineTemplateBucketV49(bucket, index) {
    ...
    items.forEach(function (item) {
      placeIds = placeIds.concat(Array.isArray(item.placeIds) ? item.placeIds : []);
      routePlaces = routePlaces.concat(Array.isArray(item.routePlaces) ? item.routePlaces : []);
      themes = themes.concat(Array.isArray(item.themes) ? item.themes : []);
      foods = foods.concat(Array.isArray(item.wantedFoods) ? item.wantedFoods : []);
    });
    return {
      id: 'combined-' + index,
      title: items.map(function (item) { return item.title; }).filter(Boolean).join(' + ') || ('추천 루트 ' + (index + 1)),
      regionKo: items[0] && (items[0].regionKo || items[0].regionOriginal) || '',
      description: items.map(function (item) { return item.title || item.description; }).filter(Boolean).join(' / ') + '를 같은 날에 묶었습니다.',
      themes: unique(themes),
      wantedFoods: unique(foods),
      placeIds: unique(placeIds),
      routePlaces: routePlaces,
      durationHours: bucket.hours,
      routeScope: bucket.hours >= 8 ? 'day' : 'half-day',
      preferredTime: 'flex',
      aiEligible: true
    };
  }

  applyThemeTemplatesToPlanV25 = async function (query) {
    var selectedThemes = getSelectedPlanThemes();
    var region = (query && query.region) || (state.planContext && state.planContext.region) || '';
    var templates = getAiRouteTemplatesV49(region, selectedThemes);
    if (!templates.length) return false;

    var days = getEffectivePlanDays();
    var buckets = packRouteTemplatesV49(templates, days);
    ...
    var appliedIds = [];
    for (var i = 0; i < buckets.length; i += 1) {
      var combined = combineTemplateBucketV49(buckets[i], i);
      appliedIds = appliedIds.concat((buckets[i].items || []).map(function (item) { return item.id; }));
      var payload = await buildTemplateDayV25(combined, i, query || {}, usedRestaurantIds);
      if (!payload) continue;
      state.plan[i] = Object.assign({}, state.plan[i] || {}, payload.day);
      stories.push(payload.story);
    }
    ...
    state.planContext = savePlanContextV9(Object.assign({}, state.planContext || {}, {
      wantedFoods: getPlanWantedFoodsV25(),
      selectedThemes: selectedThemes,
      appliedTemplateIds: unique(appliedIds)
    }));
    mergeStoryWithTemplatesV25(stories, (region || '선택 지역') + '의 관리자 루트 ' + buckets.length + '묶음을 일정에 우선 반영했습니다.');
    renderPlan();
    renderPlanSummary();
    if (els.planWarnings) {
      els.planWarnings.innerHTML = '<div class="message-box success">관리자 루트를 길이에 맞춰 일정에 반영했습니다. 짧은 루트는 같은 날에 함께 묶었습니다.</div>';
    }
    return true;
  };
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    setTimeout(function () {
      ensureAdminRouteFieldsV49();
      if (state && state.uiView === 'content') renderContentView();
    }, 350);
  });
...
/* ===== END V49 ADMIN ROUTE GUIDE + SMART TEMPLATE PACKING ===== */

/* ===== V50 ADMIN SERVICE DIAGNOSTICS ===== */
(function () {
  var lastServiceHealthV50 = null;

  function ensureAdminServicePanelV50() {
    var adminView = document.getElementById('admin-view');
    if (!adminView) return null;
    var panel = document.getElementById('admin-service-diagnostics');
    if (panel) return panel;

    panel = document.createElement('section');
    panel.id = 'admin-service-diagnostics';
    panel.className = 'v50-service-panel';
    panel.innerHTML =
      '<div class="v50-service-head">' +
        '<div><p class="page-kicker">서비스 진단</p><h3 class="content-card-title">Hot Pepper · Gemini 연결 상태</h3></div>' +
        '<button class="btn btn-ghost-sm" type="button" data-v50-refresh>상태 새로고침</button>' +
      '</div>' +
      '<div class="v50-service-grid">' +
        '<article class="v50-service-card" data-v50-hotpepper></article>' +
        '<article class="v50-service-card" data-v50-gemini></article>' +
      '</div>';

    var anchor = document.getElementById('admin-data-path');
    if (anchor && anchor.parentNode) {
      anchor.insertAdjacentElement('afterend', panel);
    } else {
      adminView.insertBefore(panel, adminView.children[1] || null);
    }

    var refresh = panel.querySelector('[data-v50-refresh]');
    if (refresh) {
      refresh.addEventListener('click', function () {
        loadServiceDiagnosticsV50(true);
      });
    }
    syncAdminServicePanelVisibilityV50();
    renderServiceDiagnosticsV50(lastServiceHealthV50);
    return panel;
  }

  function syncAdminServicePanelVisibilityV50() {
    var panel = document.getElementById('admin-service-diagnostics');
    if (!panel) return;
    var adminLogged = false;
    if (typeof isAdminUser === 'function') {
      adminLogged = Boolean(isAdminUser());
    } else {
      adminLogged = Boolean((state.currentUser && state.currentUser.role === 'admin') || (state.adminSession && state.adminSession.loggedIn));
    }
    panel.classList.toggle('hidden', !adminLogged);
  }

  function serviceStatusMetaV50(configured, status) {
    if (!configured) return { className: 'off', label: '미연결' };
    if (status === 'success') return { className: 'on', label: '연결됨' };
    if (status === 'demo') return { className: 'demo', label: '데모' };
    if (status === 'failed') return { className: 'failed', label: '오류' };
    if (status === 'not_configured') return { className: 'off', label: '미연결' };
    if (status === 'checking' || status === 'retrying') return { className: 'checking', label: '확인 중' };
    return { className: configured ? 'ready' : 'off', label: configured ? '키 등록됨' : '미연결' };
  }

  function purposeLabelV50(value) {
    var key = String(value || '').trim();
    if (key === 'trip-plan') return 'AI 일정 설명';
    if (key === 'translate-restaurants') return '식당 정보 번역';
    if (key === 'suggest-places') return '장소 추천 생성';
    if (key === 'unknown') return '용도 미확인';
    return key || '기록 없음';
  }

  function formatServiceTimeV50(value) {
  ...
  function escapeServiceHtmlV50(value) {
  ...
  function renderHotpepperCardV50(container, json) {
    var diagnostic = (json && json.diagnostics && json.diagnostics.hotpepper) || {};
    var configured = Boolean(json && json.hotpepperConfigured);
    var status = diagnostic.lastStatus || '';
    var meta = serviceStatusMetaV50(configured, status);
    var request = diagnostic.lastRequest || {};
    var requestText = request
      ? [
        request.hasKeyword ? '키워드 포함' : '키워드 없음',
        request.hasLocation ? '위치 기준' : '지역/키워드 기준',
        request.range ? '반경 ' + request.range : '',
        request.count ? request.count + '개 요청' : '',
      ].filter(Boolean).join(' · ')
      : '최근 검색 기록 없음';
    var checkedAt = diagnostic.lastSuccessAt || diagnostic.lastFailureAt || diagnostic.lastAttemptAt || diagnostic.updatedAt || '';
    container.innerHTML =
      '<div class="v50-service-card-head">' +
        '<div><strong>Hot Pepper</strong><span>실시간 식당 검색</span></div>' +
        '<span class="v50-status-pill ' + meta.className + '">' + escapeServiceHtmlV50(meta.label) + '</span>' +
      '</div>' +
      '<dl class="v50-service-facts">' +
        '<div><dt>최근 상태</dt><dd>' + escapeServiceHtmlV50(status || (configured ? '키 등록됨' : 'API 키 없음')) + '</dd></div>' +
        '<div><dt>최근 확인</dt><dd>' + escapeServiceHtmlV50(formatServiceTimeV50(checkedAt)) + '</dd></div>' +
        '<div><dt>최근 결과</dt><dd>' + escapeServiceHtmlV50(String(Number(diagnostic.lastResultCount || 0))) + '곳</dd></div>' +
        '<div><dt>최근 요청</dt><dd>' + escapeServiceHtmlV50(requestText) + '</dd></div>' +
      '</dl>' +
      (diagnostic.lastError ? '<p class="v50-service-error">' + escapeServiceHtmlV50(diagnostic.lastError) + '</p>' : '');
  }

  function renderGeminiCardV50(container, json) {
    var diagnostic = (json && json.diagnostics && json.diagnostics.gemini) || {};
    var configured = Boolean(json && json.geminiConfigured);
    var status = diagnostic.lastStatus || '';
    var meta = serviceStatusMetaV50(configured, status);
    var candidates = Array.isArray(json && json.geminiModels) && json.geminiModels.length
      ? json.geminiModels
      : (Array.isArray(diagnostic.modelCandidates) ? diagnostic.modelCandidates : []);
    var activeModel = diagnostic.lastSuccessModel || (json && json.activeGeminiModel) || '';
    var checkedAt = diagnostic.lastSuccessAt || diagnostic.lastFailureAt || diagnostic.lastAttemptAt || diagnostic.updatedAt || '';
    container.innerHTML =
      '<div class="v50-service-card-head">' +
        '<div><strong>Gemini</strong><span>AI 일정 · 번역 · 장소 추천</span></div>' +
        '<span class="v50-status-pill ' + meta.className + '">' + escapeServiceHtmlV50(meta.label) + '</span>' +
      '</div>' +
      '<dl class="v50-service-facts">' +
        '<div><dt>현재 사용 모델</dt><dd>' + escapeServiceHtmlV50(activeModel || (configured ? '성공 기록 없음' : 'API 키 없음')) + '</dd></div>' +
        '<div><dt>최근 사용</dt><dd>' + escapeServiceHtmlV50(purposeLabelV50(diagnostic.lastPurpose)) + '</dd></div>' +
        '<div><dt>최근 확인</dt><dd>' + escapeServiceHtmlV50(formatServiceTimeV50(checkedAt)) + '</dd></div>' +
        '<div><dt>최근 시도 모델</dt><dd>' + escapeServiceHtmlV50(diagnostic.lastAttemptModel || '기록 없음') + '</dd></div>' +
      '</dl>' +
      '<div class="v50-model-chain">' +
        '<span>후보 모델</span>' +
        '<p>' + escapeServiceHtmlV50(candidates.length ? candidates.join(' → ') : '등록된 후보 모델 없음') + '</p>' +
      '</div>' +
      (diagnostic.lastError ? '<p class="v50-service-error">' + escapeServiceHtmlV50(diagnostic.lastError) + '</p>' : '');
  }

  function renderServiceDiagnosticsV50(json, error) {
    var panel = ensureAdminServicePanelV50();
    if (!panel) return;
    syncAdminServicePanelVisibilityV50();
    var hotpepper = panel.querySelector('[data-v50-hotpepper]');
    var gemini = panel.querySelector('[data-v50-gemini]');
    if (error) {
      var message = '<div class="v50-service-empty">서비스 상태를 불러오지 못했습니다. 서버가 켜져 있는지 확인해 주세요.</div>';
      if (hotpepper) hotpepper.innerHTML = message;
      if (gemini) gemini.innerHTML = message;
      return;
    }
    if (!json) {
      var loading = '<div class="v50-service-empty">서비스 상태 확인 중...</div>';
      if (hotpepper) hotpepper.innerHTML = loading;
      if (gemini) gemini.innerHTML = loading;
      return;
    }
    if (hotpepper) renderHotpepperCardV50(hotpepper, json);
    if (gemini) renderGeminiCardV50(gemini, json);
  }

  function loadServiceDiagnosticsV50(silent) {
    ensureAdminServicePanelV50();
    if (!silent) renderServiceDiagnosticsV50(lastServiceHealthV50);
    return fetch('/api/health?serviceDiagnostics=1&ts=' + Date.now(), { cache: 'no-store' })
      .then(function (response) {
        if (!response.ok) throw new Error('health_failed');
        return response.json();
      })
      .then(function (json) {
        lastServiceHealthV50 = json;
        if (state) state.serviceHealth = json;
        renderServiceDiagnosticsV50(json);
        return json;
      })
      .catch(function (error) {
        renderServiceDiagnosticsV50(lastServiceHealthV50, error);
        return null;
      });
  }

  var previousDetectApiV50 = typeof detectApi === 'function' ? detectApi : null;
  if (previousDetectApiV50) {
    detectApi = function () {
      return Promise.resolve(previousDetectApiV50.apply(this, arguments)).then(function (result) {
        return loadServiceDiagnosticsV50(true).then(function () { return result; });
      }).catch(function (error) {
        renderServiceDiagnosticsV50(lastServiceHealthV50, error);
        throw error;
      });
  ...
  var previousRenderAdminStateV50 = typeof renderAdminState === 'function' ? renderAdminState : null;
  if (previousRenderAdminStateV50) {
    renderAdminState = function () {
      var result = previousRenderAdminStateV50.apply(this, arguments);
      ensureAdminServicePanelV50();
      syncAdminServicePanelVisibilityV50();
      return result;
    };
  ...
  var previousRenderAdminDashboardV50 = typeof renderAdminDashboard === 'function' ? renderAdminDashboard : null;
  if (previousRenderAdminDashboardV50) {
    renderAdminDashboard = function () {
      var result = previousRenderAdminDashboardV50.apply(this, arguments);
      ensureAdminServicePanelV50();
      syncAdminServicePanelVisibilityV50();
      if (!lastServiceHealthV50) loadServiceDiagnosticsV50(true);
      return result;
    };
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ensureAdminServicePanelV50();
    loadServiceDiagnosticsV50(true);
    var adminRefresh = document.getElementById('admin-refresh-button');
    if (adminRefresh && !adminRefresh.__v50ServiceBound) {
      adminRefresh.__v50ServiceBound = true;
...
/* ===== END V50 ADMIN SERVICE DIAGNOSTICS ===== */
