...
const STORAGE_BACKEND = SUPABASE_URL && SUPABASE_SECRET_KEY && createClient ? 'supabase' : 'file';
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/api/health') {
      const diagnostics = getServiceDiagnostics(loadStore());
      return sendJson(res, 200, {
        ok: true,
        hotpepperConfigured: Boolean(HOTPEPPER_API_KEY),
        geminiConfigured: Boolean(GEMINI_API_KEY),
        geminiModels: GEMINI_MODEL_CANDIDATES,
        activeGeminiModel: diagnostics.gemini.lastSuccessModel || '',
        diagnostics,
...
function ensureStoreShape(value) {
  ...
  base.serviceDiagnostics = raw.serviceDiagnostics && typeof raw.serviceDiagnostics === 'object' ? raw.serviceDiagnostics : {};
...
function compactDiagnosticError(value) {
  let text = String(value || '');
  if (GEMINI_API_KEY) text = text.split(GEMINI_API_KEY).join('[hidden]');
  if (HOTPEPPER_API_KEY) text = text.split(HOTPEPPER_API_KEY).join('[hidden]');
  return text.replace(/\s+/g, ' ').slice(0, 240);
}

function getServiceDiagnostics(store) {
  const raw = ensureStoreShape(store).serviceDiagnostics || {};
  const hotpepper = raw.hotpepper && typeof raw.hotpepper === 'object' ? raw.hotpepper : {};
  const gemini = raw.gemini && typeof raw.gemini === 'object' ? raw.gemini : {};
  return {
    hotpepper: {
      configured: Boolean(HOTPEPPER_API_KEY),
      lastStatus: String(hotpepper.lastStatus || ''),
      lastAttemptAt: String(hotpepper.lastAttemptAt || ''),
      lastSuccessAt: String(hotpepper.lastSuccessAt || ''),
      lastFailureAt: String(hotpepper.lastFailureAt || ''),
      lastResultCount: Number(hotpepper.lastResultCount || 0),
      lastHttpStatus: hotpepper.lastHttpStatus ? Number(hotpepper.lastHttpStatus) : 0,
      lastError: compactDiagnosticError(hotpepper.lastError || ''),
      lastRequest: hotpepper.lastRequest && typeof hotpepper.lastRequest === 'object' ? {
        hasKeyword: Boolean(hotpepper.lastRequest.hasKeyword),
        hasLocation: Boolean(hotpepper.lastRequest.hasLocation),
        range: Number(hotpepper.lastRequest.range || 0),
        count: Number(hotpepper.lastRequest.count || 0),
      } : null,
      updatedAt: String(hotpepper.updatedAt || ''),
    },
    gemini: {
      configured: Boolean(GEMINI_API_KEY),
      modelCandidates: GEMINI_MODEL_CANDIDATES.slice(),
      lastStatus: String(gemini.lastStatus || ''),
      lastPurpose: String(gemini.lastPurpose || ''),
      lastAttemptAt: String(gemini.lastAttemptAt || ''),
      lastSuccessAt: String(gemini.lastSuccessAt || ''),
      lastFailureAt: String(gemini.lastFailureAt || ''),
      lastAttemptModel: String(gemini.lastAttemptModel || ''),
      lastSuccessModel: String(gemini.lastSuccessModel || ''),
      lastFailureModel: String(gemini.lastFailureModel || ''),
      lastHttpStatus: gemini.lastHttpStatus ? Number(gemini.lastHttpStatus) : 0,
      lastError: compactDiagnosticError(gemini.lastError || ''),
      updatedAt: String(gemini.updatedAt || ''),
    },
  };
}

function updateServiceDiagnostic(service, patch) {
  const store = loadStore();
  store.serviceDiagnostics = store.serviceDiagnostics && typeof store.serviceDiagnostics === 'object'
    ? store.serviceDiagnostics
    : {};
  const current = store.serviceDiagnostics[service] && typeof store.serviceDiagnostics[service] === 'object'
    ? store.serviceDiagnostics[service]
    : {};
  store.serviceDiagnostics[service] = {
    ...current,
    ...(patch || {}),
...
function sanitizeThemeTemplate(template) {
  ...
  const durationHours = Number(template.durationHours || template.routeDurationHours || template.hours || 6);
  return {
    ...
    durationHours: Number.isFinite(durationHours) && durationHours > 0 ? Math.min(24, Math.max(1, durationHours)) : 6,
    routeScope: String(template.routeScope || template.scope || '').trim() || (durationHours >= 8 ? 'day' : durationHours <= 3 ? 'short' : 'half-day'),
    preferredTime: String(template.preferredTime || template.timeOfDay || '').trim() || 'flex',
    aiEligible: template.aiEligible === false ? false : true,
...
function sanitizeContentPost(post) {
  ...
  const durationHours = Number(post.durationHours || post.routeDurationHours || post.hours || 0);
  return {
    ...
    durationHours: Number.isFinite(durationHours) && durationHours > 0 ? Math.min(24, Math.max(1, durationHours)) : 0,
    routeScope: String(post.routeScope || post.scope || '').trim() || '',
    preferredTime: String(post.preferredTime || post.timeOfDay || '').trim() || 'flex',
    aiEligible: post.aiEligible === true,
...
function hashPassword(password, salt) {
  ...
  const hash = crypto.pbkdf2Sync(safePassword, safeSalt, 120000, 32, 'sha256').toString('hex');
...
function verifyPassword(user, password) {
  ...
  if (storedHash && storedHash.includes(':')) {
    ...
    const actual = hashPassword(raw, salt).split(':').slice(1).join(':');
    try {
  ...
  return String(user.password || '') === raw;
}
...
function sanitizeUserAccount(user) {
  ...
  if (!passwordHash && legacyPassword) account.password = legacyPassword;
  return account;
}
...
async function handleAuthSignup(req, res) {
  ...
  const user = sanitizeUserAccount({ email, username, passwordHash: hashPassword(password), nickname, role: 'user' });
...
async function handleAuthLogin(req, res) {
  ...
  if (!user.passwordHash && user.password) {
    ...
    store.users[user.email] = sanitizeUserAccount(user);
    saveStore(store);
    await flushStoreSave();
    user = store.users[user.email];
  }
...
async function handleAiSuggestPlaces(req, res) {
  if (!GEMINI_API_KEY) {
    updateServiceDiagnostic('gemini', {
      lastStatus: 'not_configured',
      lastPurpose: 'suggest-places',
  ...
  let gemini;
  try {
    gemini = await callGeminiWithFallback(prompt, { temperature: 0.4 }, { purpose: 'suggest-places' });
  } catch (error) {
...
async function callGeminiWithFallback(prompt, generationConfig, meta) {
  const purpose = String(meta?.purpose || 'unknown').trim() || 'unknown';
  if (!GEMINI_API_KEY) {
    updateServiceDiagnostic('gemini', {
      lastStatus: 'not_configured',
      lastPurpose: purpose,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        updateServiceDiagnostic('gemini', {
          ...
          lastFailureModel: model,
          lastHttpStatus: response.status,
          lastError: compactDiagnosticError(detail || `model_failed:${model}`),
        });
      ...
      updateServiceDiagnostic('gemini', {
        ...
        lastFailureAt: new Date().toISOString(),
        lastFailureModel: model,
        lastError: compactDiagnosticError(error && error.message ? error.message : error),
      });
    }
  }
  updateServiceDiagnostic('gemini', {
    ...
    lastPurpose: purpose,
    lastFailureAt: new Date().toISOString(),
    lastError: compactDiagnosticError(lastError && lastError.message ? lastError.message : lastError),
  });
  throw lastError;
}
...
async function handleSearch(url, res) {
  if (!HOTPEPPER_API_KEY) {
    updateServiceDiagnostic('hotpepper', {
      lastStatus: 'demo',
      lastAttemptAt: new Date().toISOString(),
      lastResultCount: 0,
      lastError: 'HOTPEPPER_API_KEY is not configured',
    });
  ...
  try {
    ...
    return sendJson(res, 502, { ok: false, error: 'hotpepper_request_failed' });
  }
...
async function handleAiRestaurantTranslation(req, res) {
  if (!GEMINI_API_KEY) {
    updateServiceDiagnostic('gemini', {
      lastStatus: 'not_configured',
      lastPurpose: 'translate-restaurants',
      lastAttemptAt: new Date().toISOString(),
      lastError: 'GEMINI_API_KEY is not configured',
    });
    return sendJson(res, 400, { ok: false, error: 'gemini_not_configured' });
  ...
  let gemini;
  try {
    gemini = await callGeminiWithFallback(prompt, { temperature: 0.2 }, { purpose: 'translate-restaurants' });
  } catch (error) {
    return sendJson(res, 502, { ok: false, error: 'gemini_request_failed', detail: String(error.message || error).slice(0, 400) });
  }
...
async function handleAiTripPlan(req, res) {
  ...
  let gemini;
  try {
    gemini = await callGeminiWithFallback(prompt, { temperature: 0.5 }, { purpose: 'trip-plan' });
  } catch (error) {
    return sendJson(res, 502, { ok: false, error: 'gemini_request_failed', detail: String(error.message || error).slice(0, 400) });
  }
