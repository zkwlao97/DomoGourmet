...
function sanitizeUserAccount(user) {
  ...
  return {
    ...
    email,
    username: String(user.username || email.split('@')[0] || '').trim().toLowerCase(),
...
function findUserByIdentifier(store, identifier) {
  ...
  if (store.users[needle]) return store.users[needle];
  const values = Object.values(store.users || {});
  return values.find((user) => String(user.username || '').trim().toLowerCase() === needle) || null;
}
...
async function handleAuthSignup(req, res) {
  ...
  if (store.users[email]) {
    return sendJson(res, 409, { ok: false, error: 'already_exists', message: '이미 존재하는 이메일입니다.' });
  }
  const duplicatedUsername = Object.values(store.users || {}).some((user) => String(user.username || '').trim().toLowerCase() === username);
  if (duplicatedUsername) {
    return sendJson(res, 409, { ok: false, error: 'already_exists', message: '이미 존재하는 아이디입니다.' });
  }
